...
#google_dataproc_workflow_template: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/google_dataproc_workflow_template")
	close({
		...
		// timeout period, any remaining jobs are cancelled, the workflow
		// is ended, and if the workflow was running on a [managed
		...
		// 1035](https://www.ietf.org/rfc/rfc1035.txt). Label **values**
		// may be empty, but, if present, must contain 1 to 63
		// characters, and must conform to [RFC
		// 1035](https://www.ietf.org/rfc/rfc1035.txt). No more than 32
	...
	#jobs: close({
		...
		// within the template. The step id is used as prefix for job id,
		// as job `goog-dataproc-workflow-step-id` label, and in
		// prerequisiteStepIds field from other steps. The id must
	...
	#parameters: close({
		...
		// reference fields using the following syntax: * Values in maps
		// can be referenced by key: * labels['key'] *
		// placement.clusterSelector.clusterLabels['key'] *
		// placement.managedCluster.labels['key'] *
		// placement.clusterSelector.clusterLabels['key'] *
		// jobs['step-id'].labels['key'] * Jobs in the jobs list can be
		// referenced by step-id: *
		...
		// jobs['step-id'].pySparkJob.pythonFileUris[0] * Items in
		// repeated fields can be referenced by a zero-based index: *
	...
	#placement: close({
		cluster_selector?: matchN(1, [_#defs."/$defs/placement/$defs/cluster_selector", list.MaxItems(1) & [..._#defs."/$defs/placement/$defs/cluster_selector"]])
		managed_cluster?: matchN(1, [_#defs."/$defs/placement/$defs/managed_cluster", list.MaxItems(1) & [..._#defs."/$defs/placement/$defs/managed_cluster"]])
	})
	...
	_#defs: "/$defs/jobs/$defs/hadoop_job": close({
		...
		// arguments, such as `-libjars` or `-Dfoo=bar`, that can be set
	...
	_#defs: "/$defs/jobs/$defs/hadoop_job/$defs/logging_config": close({
	...
	_#defs: "/$defs/jobs/$defs/hive_job": close({
		...
		// Hive server and Hadoop MapReduce (MR) tasks. Can contain Hive
		// SerDes and UDFs.
		jar_file_uris?: [...string]
	...
	_#defs: "/$defs/jobs/$defs/hive_job/$defs/query_list": close({
		...
		// QueryList to specify a HiveJob: "hiveJob": { "queryList": {
	...
	_#defs: "/$defs/jobs/$defs/pig_job": close({
		...
		// Optional. HCFS URIs of jar files to add to the CLASSPATH of the
		// Pig Client and Hadoop MapReduce (MR) tasks. Can contain Pig
		// UDFs.
		jar_file_uris?: [...string]
	...
	_#defs: "/$defs/jobs/$defs/pig_job/$defs/logging_config": close({
		// The per-package log levels for the driver. This may include
		// "root" package name to configure rootLogger. Examples:
		// 'com.google = FATAL', 'root = INFO', 'org.apache = DEBUG'
		driver_log_levels?: [string]: string
	...
	_#defs: "/$defs/jobs/$defs/pig_job/$defs/query_list": close({
		// Required. The queries to execute. You do not need to end a
		// query expression with a semicolon. Multiple queries can be
		// specified in one string by separating each with a semicolon.
		// Here is an example of a Dataproc API snippet that uses a
	...
	_#defs: "/$defs/jobs/$defs/presto_job/$defs/logging_config": close({
		// The per-package log levels for the driver. This may include
		// "root" package name to configure rootLogger. Examples:
		// 'com.google = FATAL', 'root = INFO', 'org.apache = DEBUG'
		driver_log_levels?: [string]: string
	...
	_#defs: "/$defs/jobs/$defs/presto_job/$defs/query_list": close({
		// Required. The queries to execute. You do not need to end a
		// query expression with a semicolon. Multiple queries can be
		// specified in one string by separating each with a semicolon.
		// Here is an example of a Dataproc API snippet that uses a
	...
	_#defs: "/$defs/jobs/$defs/pyspark_job/$defs/logging_config": close({
		// The per-package log levels for the driver. This may include
		// "root" package name to configure rootLogger. Examples:
		// 'com.google = FATAL', 'root = INFO', 'org.apache = DEBUG'
		driver_log_levels?: [string]: string
	...
	_#defs: "/$defs/jobs/$defs/scheduling": close({
	...
	_#defs: "/$defs/jobs/$defs/spark_job/$defs/logging_config": close({
		// The per-package log levels for the driver. This may include
		// "root" package name to configure rootLogger. Examples:
		// 'com.google = FATAL', 'root = INFO', 'org.apache = DEBUG'
		driver_log_levels?: [string]: string
	...
	_#defs: "/$defs/jobs/$defs/spark_r_job/$defs/logging_config": close({
		// The per-package log levels for the driver. This may include
		// "root" package name to configure rootLogger. Examples:
		// 'com.google = FATAL', 'root = INFO', 'org.apache = DEBUG'
		driver_log_levels?: [string]: string
	...
	_#defs: "/$defs/jobs/$defs/spark_sql_job/$defs/query_list": close({
		// Required. The queries to execute. You do not need to end a
		// query expression with a semicolon. Multiple queries can be
		// specified in one string by separating each with a semicolon.
		// Here is an example of a Dataproc API snippet that uses a
	...
	_#defs: "/$defs/parameters/$defs/validation": close({
		regex?: matchN(1, [_#defs."/$defs/parameters/$defs/validation/$defs/regex", list.MaxItems(1) & [..._#defs."/$defs/parameters/$defs/validation/$defs/regex"]])
		values?: matchN(1, [_#defs."/$defs/parameters/$defs/validation/$defs/values", list.MaxItems(1) & [..._#defs."/$defs/parameters/$defs/validation/$defs/values"]])
	})

	_#defs: "/$defs/parameters/$defs/validation/$defs/regex": close({
	...
	_#defs: "/$defs/parameters/$defs/validation/$defs/values": close({
		// Required. List of allowed values for the parameter.
		values!: [...string]
	})

	_#defs: "/$defs/placement/$defs/cluster_selector": close({
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/encryption_config": close({
		// Optional. The Cloud KMS key name to use for PD disk encryption
		// for all instances in the cluster.
		gce_pd_kms_key_name?: string
	})

	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/endpoint_config": close({
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/gce_cluster_config": close({
		...
		// neither `network_uri` nor `subnetwork_uri` is specified, the
		// "default" network of the project is used, if it exists. Cannot
		// be a "Custom Subnet Network" (see [Using
		// Subnetworks](https://cloud.google.com/compute/docs/subnetworks)
		...
		// values: PRIVATE_IPV6_GOOGLE_ACCESS_UNSPECIFIED,
		...
		// Platform services. If not specified, the [Compute Engine
		// default service
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/gce_cluster_config/$defs/node_group_affinity": close({
		...
		// or node group name are valid. Examples: *
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/gce_cluster_config/$defs/reservation_affinity": close({
		// Optional. Type of reservation to consume Possible values:
		// TYPE_UNSPECIFIED, NO_RESERVATION, ANY_RESERVATION,
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/gce_cluster_config/$defs/shielded_instance_config": close({
		// Optional. Defines whether instances have integrity monitoring
		// enabled. Integrity monitoring compares the most recent boot
		// measurements to the integrity policy baseline and returns a
		// pair of pass/fail results depending on whether they match or
		// not.
		enable_integrity_monitoring?: bool

		// Optional. Defines whether the instances have Secure Boot
		...
		// Optional. Defines whether the instance have the vTPM enabled.
		// Virtual Trusted Platform Module protects objects like keys,
		// certificates and enables Measured Boot by performing the
		// measurements needed to create a known good boot baseline,
		// called the integrity policy baseline.
		enable_vtpm?: bool
	})

	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/initialization_actions": close({
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/lifecycle_config": close({
		...
		// auto-deleted at the end of this period. Minimum value is 10
		...
		// the cluster to be deleted. Minimum value is 5 minutes; maximum
		// value is 14 days (see JSON representation of
		// [Duration](https://developers.google.com/protocol-buffers/docs/proto3#json)).
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/master_config": close({
		...
		// `image-id` Image family examples. Dataproc will use the most
		...
		// names from `cluster_name`, `num_instances`, and the instance
		// group.
		...
		// `NON_PREEMPTIBLE`. This default cannot be changed. The default
		// value for secondary instances is `PREEMPTIBLE`. Possible
		// values: PREEMPTIBILITY_UNSPECIFIED, NON_PREEMPTIBLE,
		// PREEMPTIBLE
		preemptibility?: string
	})

	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/master_config/$defs/accelerators": close({
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/master_config/$defs/disk_config": close({
		...
		// State Drive), "pd-ssd" (Persistent Disk Solid State Drive), or
		// "pd-standard" (Persistent Disk Hard Disk Drive). See [Disk
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/secondary_worker_config/$defs/accelerators": close({
		// The number of the accelerator cards of this type exposed to
		// this instance.
		accelerator_count?: number
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/security_config": close({
		kerberos_config?: matchN(1, [_#defs."/$defs/placement/$defs/managed_cluster/$defs/config/$defs/security_config/$defs/kerberos_config", list.MaxItems(1) & [..._#defs."/$defs/placement/$defs/managed_cluster/$defs/config/$defs/security_config/$defs/kerberos_config"]])
	})

	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/security_config/$defs/kerberos_config": close({
		// Optional. The admin server (IP or hostname) for the remote
		// trusted realm in a cross realm trust relationship.
		cross_realm_trust_admin_server?: string

		// Optional. The KDC (IP or hostname) for the remote trusted realm
		...
		// SSL encryption. If not provided, Dataproc will provide a
		...
		// Optional. The uri of the KMS key used to encrypt various
		// sensitive files.
		kms_key?: string
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/software_config": close({
		...
		// `distcp-default.xml` * hdfs: `hdfs-site.xml` * hive:
		// `hive-site.xml` * mapred: `mapred-site.xml` * pig:
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/worker_config/$defs/accelerators": close({
		// The number of the accelerator cards of this type exposed to
		// this instance.
		accelerator_count?: number
