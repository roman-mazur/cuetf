...
#google_compute_region_instance_group_manager: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/google_compute_region_instance_group_manager")
	close({
	...
	#instance_flexibility_policy: close({
		instance_selections?: matchN(1, [_#defs."/$defs/instance_flexibility_policy/$defs/instance_selections", [..._#defs."/$defs/instance_flexibility_policy/$defs/instance_selections"]])
	})
	...
	#update_policy: close({
		// The instance redistribution policy for regional managed
		// instance groups. Valid values are: "PROACTIVE", "NONE". If
		...
		// The instance replacement method for regional managed instance
		// groups. Valid values are: "RECREATE", "SUBSTITUTE". If
		// SUBSTITUTE (default), the group replaces VM instances with new
		// instances that have randomly generated names. If RECREATE,
		// instance names are preserved. You must also set
	...
	_#defs: "/$defs/instance_flexibility_policy/$defs/instance_selections": close({
		...
		// higher preference. MIG will first try to create a VM based on
		// the machine-type with lowest rank and fallback to next rank
		// based on availability. Machine types and instance selections
