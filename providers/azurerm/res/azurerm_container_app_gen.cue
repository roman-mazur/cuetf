...
azurerm_container_app: {
	...
	#ingress: close({
		...
		// The transport method for the Ingress. Possible values include `auto`, `http`,
		// and `http2`, `tcp`. Defaults to `auto`
		transport?: string
	})
	...
	#template: close({
		...
		// The suffix for the revision. This value must be unique for the lifetime of
		// the Resource. If omitted the service will use a hash function to create one.
	...
	_#defs: "/$defs/ingress/$defs/traffic_weight": close({
		...
		// Container App's lifetime. A default hash created by the service will be used
