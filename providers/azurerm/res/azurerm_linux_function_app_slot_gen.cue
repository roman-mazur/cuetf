...
azurerm_linux_function_app_slot: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/azurerm_linux_function_app_slot")
	close({
		...
		// The mode of the Function App Slot's client certificates requirement for
		// incoming requests. Possible values are `Required`, `Optional`, and
		...
		// consume per day. Setting this value only affects function apps in
		// Consumption Plans.
	...
	#auth_settings: close({
		...
		// Specifies a map of Login Parameters to send to the OpenID Connect
		// authorization endpoint when a user logs in.
		additional_login_parameters?: [string]: string

		// Specifies a list of External URLs that can be redirected to as part of
		// logging in or logging out of the Windows Web App.
		...
		// The number of hours after session token expiration that a session token can
		// be used to call the token refresh API. Defaults to `72` hours.
		token_refresh_extension_hours?: number

		// Should the Windows Web App durably store platform-specific security tokens
		// that are obtained during login flows? Defaults to `false`.
	...
	#auth_settings_v2: close({
		...
		// The path to the App Auth settings. **Note:** Relative Paths are evaluated
		// from the Site Root directory.
		config_file_path?: string

		// The Default Authentication Provider to use when the `unauthenticated_action`
		// is set to `RedirectToLoginPage`. Possible values include: `apple`,
		// `azureactivedirectory`, `facebook`, `github`, `google`, `twitter` and the
		// `name` of your `custom_oidc_v2` provider.
		default_provider?: string

		// The paths which should be excluded from the `unauthenticated_action` when it
		...
		// The prefix that should precede all the authentication and authorisation
		...
		// include `RedirectToLoginPage`, `AllowAnonymous`, `Return401`, and
		// `Return403`. Defaults to `RedirectToLoginPage`.
	...
	#site_config: close({
		...
		// The number of workers this function app can scale out to. Only applicable to
		// apps on the Consumption and Premium plan.
		...
		// The Client ID of the Managed Service Identity to use for connections to the
		...
		// The number of minimum instances for this Linux Function App. Only affects
		// apps on Elastic Premium plans.
		...
		// The amount of time in minutes that a node is unhealthy before being removed
		// from the load balancer. Possible values are between `2` and `10`. Defaults
		// to `10`. Only valid in conjunction with `health_check_path`
		...
		// values include: `1.0`, `1.1`, `1.2` and `1.3`. Defaults to `1.2`.
		minimum_tls_version?: string

		// The number of pre-warmed instances for this function app. Only affects apps
		// on an Elastic Premium plan.
		...
		// Configures the minimum version of TLS required for SSL requests to the SCM
		// site Possible values include: `1.0`, `1.1`, `1.2` and `1.3`. Defaults to
		// `1.2`.
	...
	_#defs: "/$defs/auth_settings/$defs/facebook": close({
		...
		// The App Secret of the Facebook app used for Facebook Login. Cannot be
		// specified with `app_secret_setting_name`.
		app_secret?: string
	...
	_#defs: "/$defs/auth_settings/$defs/github": close({
		...
		// The Client Secret of the GitHub app used for GitHub Login. Cannot be
	...
	_#defs: "/$defs/auth_settings/$defs/google": close({
		...
		// Sign-In authentication. If not specified, "openid", "profile", and "email"
	...
	_#defs: "/$defs/auth_settings/$defs/microsoft": close({
		...
		// authentication. Cannot be specified with `client_secret_setting_name`.
		client_secret?: string

		// The app setting name containing the OAuth 2.0 client secret that was created
		// for the app used for authentication. Cannot be specified with
		// `client_secret`.
		client_secret_setting_name?: string

		// The list of OAuth 2.0 scopes that will be requested as part of Microsoft
		// Account authentication. If not specified, `wl.basic` is used as the default
		// scope.
		oauth_scopes?: [...string]
	})
	...
	_#defs: "/$defs/auth_settings/$defs/twitter": close({
		...
		// Twitter application used for sign-in. Cannot be specified with
		// `consumer_secret`.
	...
	_#defs: "/$defs/auth_settings_v2/$defs/google_v2": close({
		// Specifies a list of Allowed Audiences that will be requested as part of
	...
	_#defs: "/$defs/auth_settings_v2/$defs/login": close({
		// External URLs that can be redirected to as part of logging in or logging out
		// of the app. This is an advanced setting typically only needed by Windows
		// Store application backends. **Note:** URLs within the current domain are
	...
	_#defs: "/$defs/backup/$defs/schedule": close({
		...
		// be set to `7` and `frequency_unit` should be set to `Day`).
	...
	_#defs: "/$defs/site_config/$defs/app_service_logs": close({
		...
		// The retention period for logs in days. Valid values are between `0` and
		// `99999`. Defaults to `0` (never delete).
		retention_period_days?: number
	})
	...
	_#defs: "/$defs/site_config/$defs/application_stack": close({
		...
		// The version of Python to use. Possible values include `3.14`, `3.13`, `3.12`,
		// `3.11`, `3.10`, `3.9`, `3.8`, and `3.7`.
	...
	_#defs: "/$defs/site_config/$defs/application_stack/$defs/docker": close({
