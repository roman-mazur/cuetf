...
github_repository: {
	...
	close({
		...
		// Set to 'true' to allow private forking on the repository; this is only
		// relevant if the repository is owned by an organization and is private or
		// internal.
		allow_forking?: bool
		...
		// Specifies if the repository should be archived. Defaults to 'false'. NOTE
		// Currently, the API does not support unarchiving.
		archived?: bool
		...
		// Set to 'true' to enable the GitHub Projects features on the repository. Per
		// the GitHub documentation when in an organization that has disabled
		// repository projects it will default to 'false' and will otherwise default to
		// 'true'. If you specify 'true' when it has been disabled it will return an
		// error.
		has_projects?: bool
		...
		// URL that can be provided to 'svn checkout' to check out the repository via
		// GitHub's Subversion protocol emulation.
		svn_url?: string
		...
		// 2.20+, visibility can also be 'internal'.
		visibility?: string
	...
	#template: close({
		// Whether the new repository should include all the branches from the template
		// repository (defaults to 'false', which includes only the default branch from
		// the template).
	...
	_#defs: "/$defs/security_and_analysis/$defs/advanced_security": close({
		// Set to 'enabled' to enable advanced security features on the repository. Can
		// be 'enabled' or 'disabled', This value being present when split licensing is
		// enabled will error out.
		status!: string
	})

	_#defs: "/$defs/security_and_analysis/$defs/code_security": close({
		// Set to 'enabled' to enable code security on the repository. Can be 'enabled'
		// or 'disabled'. If set to 'enabled', the repository's visibility must be
		// 'public', 'security_and_analysis[0].advanced_security[0].status' must also
		// be set to 'enabled', or your Organization must have split licensing for
		// Advanced security.
		status!: string
	})

	_#defs: "/$defs/security_and_analysis/$defs/secret_scanning": close({
		// Set to 'enabled' to enable secret scanning on the repository. Can be
		// 'enabled' or 'disabled'. If set to 'enabled', the repository's visibility
		// must be 'public', 'security_and_analysis[0].advanced_security[0].status'
		// must also be set to 'enabled', or your Organization must have split
	...
	_#defs: "/$defs/security_and_analysis/$defs/secret_scanning_ai_detection": close({
		...
		// 'security_and_analysis[0].advanced_security[0].status' must also be set to
		// 'enabled', or your Organization must have split licensing for Advanced
		// security.
		status!: string
	})

	_#defs: "/$defs/security_and_analysis/$defs/secret_scanning_non_provider_patterns": close({
		// Set to 'enabled' to enable secret scanning non-provider patterns on the
		// repository. Can be 'enabled' or 'disabled'. If set to 'enabled', the
