...
elasticstack_kibana_agentbuilder_agent: {
	...
	close({
		...
		// Tools attached to the agent. When include_dependencies is true, each entry
		// includes full tool data and workflow YAML for workflow-type tools. When
		// false, only id (composite space/tool), space_id, and tool_id are set.
		tools?: matchN(1, [close({
			...
			// The YAML configuration of the referenced workflow. Only populated for
			// workflow-type tools. Requires Elastic Stack v9.4.0 or later.
			workflow_configuration_yaml?: string

			// The ID of the referenced workflow. Only populated for workflow-type tools.
		...
		// If `true`, exports the agent along with its tools and workflows. If omitted,
		// `false` is used (tool rows only list `id`, `space_id`, and `tool_id` unless
		// this is `true`).
		...
		// Set of skill IDs assigned to the agent. Requires Elastic Stack 9.4.0 or later.
		skill_ids?: [...string]
