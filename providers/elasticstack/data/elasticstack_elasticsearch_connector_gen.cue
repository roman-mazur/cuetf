package data
	...
	close({
		...
		// Full registered configuration schema document from the
		// connector service. JSON-encoded; use `jsondecode()` to
		// inspect.
		...
		// Custom per-job-type scheduling overrides. JSON-encoded object;
		// use `jsondecode()` to inspect.
		...
		// ISO 8601 timestamp when the last access-control sync was
		// scheduled.
		...
		// ISO 8601 timestamp when the last incremental sync was
		// scheduled.
		...
		// Connector lifecycle status (for example `created`, `connected`,
		// `error`).
		status?: string

		// Opaque connector sync cursor state. JSON-encoded; use
		// `jsondecode()` to inspect.
		sync_cursor?: string
