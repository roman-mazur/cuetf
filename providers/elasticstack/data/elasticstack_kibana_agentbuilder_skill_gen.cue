...
elasticstack_kibana_agentbuilder_skill: {
	...
	close({
		...
		// Ordered list of referenced-content entries. Order is preserved as returned by the API.
		...
		// The skill ID to look up. Accepts either a bare skill id or a composite
		// `<space_id>/<skill_id>` string.
		skill_id!: string

		// An identifier for the Kibana space. If not provided, the default space is
		// used unless the `skill_id` argument supplies a composite space.
		space_id?: string
