...
elasticstack_elasticsearch_ml_datafeed: {
	...
	close({
		...
		// is limited and should be used only with low cardinality data. This should be
		// a JSON object representing the aggregations to be performed.
		aggregations?: string
		...
		chunking_config?: close({
			// The chunking mode. Can be `auto`, `manual`, or `off`. In `auto` mode, the
			// chunk size is dynamically calculated. In `manual` mode, chunking is applied
			// according to the specified `time_span`. In `off` mode, no chunking is
			// applied.
			mode!: string

			// The time span for each chunk. Only applicable and required when mode is
		...
		delayed_data_check_config?: close({
			...
			// appropriate `check_window` to be calculated when the real-time datafeed
			// runs.
			check_window?: string
		...
		// is passed verbatim to Elasticsearch. By default uses `{"match_all":
		// {"boost": 1}}`.
		query?: string
		...
		// Specifies runtime fields for the datafeed search. This should be a JSON
		// object representing the runtime field mappings.
		...
		// functions that use these script fields. This should be a JSON object
		// representing the script fields.
