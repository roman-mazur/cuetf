...
elasticstack_elasticsearch_ml_anomaly_detection_job: {
	...
	close({
		...
		analysis_config!: close({
			...
			//
			// If the anomaly detector is expecting to see data at near real-time frequency,
			// then the `bucket_span` should be set to a value around 10 times the time
			// between ingested documents. For example, if data comes every second,
			// `bucket_span` should be 10s; if data comes every 5 minutes, `bucket_span`
			// should be 50m.
			//
			// For sparse or batch data, use larger `bucket_span` values.
			bucket_span?: string

			// Detector configuration objects. Detectors identify the anomaly detection
			// functions and the fields on which they operate.
			detectors!: matchN(1, [close({
				...
				// their own history. It is used for finding unusual values in the context of
				// the split.
				by_field_name?: string

				// Custom rules enable you to customize the way detectors operate. Each rule
				// must either have a non-empty `scope` or at least one `conditions` entry.
				// Multiple conditions are combined together with a logical AND. A non-empty
				// `scope` and one or more `conditions` may both be set on the same rule; they
				...
				custom_rules?: matchN(1, [close({
					...
					// An array of numeric conditions when the rule applies. If you specify more
					// than one condition, Elasticsearch combines them together with a logical AND.
					// A rule must either have a non-empty `scope` or at least one condition. You
					// may set `scope` on the same rule.
					...
					// Maps an analysis field name (typically matching `by_field_name`,
					// `over_field_name`, or `partition_field_name` on the detector) to an ML
					// filter reference. Each `filter_id` must identify an ML filter that already
					// exists in the cluster (for example, created using the Elasticsearch ML
					// filter APIs). A rule must either have a non-empty `scope` or at least one
					// condition. You may set `conditions` on the same rule.
					scope?: [string]: close({
						// The ML filter identifier (`filter_id`) to apply.
						filter_id!: string

						// `include` applies the rule to values in the filter; `exclude` applies it to
						// values not in the filter.
						filter_type?: string
					})
				}), [...close({
					// The set of actions to be triggered when the rule applies. If more than one
				...
				// The field that the detector function analyzes. Some functions require a
				// field. Functions that don't require a field are count, rare, and freq_rare.
				field_name?: string
			...
			// For categorization jobs only. An array of regular expressions. A
			// categorization message is matched against each regex in the order they are
			// listed in the array.
			...
			// you specify a non-zero value, it must be greater than or equal to one
			// second.
			latency?: string
			...
			// in customer environments and is not subject to the support SLA of official
			// GA features.
		...
		// A text string that affects the name of the machine learning results index. Do
		// not start the value with `custom-`; Elasticsearch automatically adds this
		// prefix.
