package res
	...
	close({
		...
		//
		// This identifier can contain lowercase alphanumeric characters
		// (a-z and 0-9), hyphens, and underscores. It must start and end
		...
		analysis_config!: close({
			...
			// typically between 15m and 1h.
			//
			// If the anomaly detector is expecting to see data at near
			// real-time frequency, then the `bucket_span` should be set to a
			// value around 10 times the time between ingested documents. For
			// example, if data comes every second, `bucket_span` should be
			// 10s; if data comes every 5 minutes, `bucket_span` should be
			// 50m.
			//
			...
			detectors!: matchN(1, [close({
				...
				//
				// In particular, this property is used for analyzing the splits
				// with respect to their own history. It is used for finding
				...
				//
				// In particular, this property is used for analyzing the splits
				// with respect to the history of all splits. It is used for
				// finding unusual values in the population of all splits.
			...
			per_partition_categorization?: close({
				...
				// detector that uses the keyword `mlcategory`.
				//
