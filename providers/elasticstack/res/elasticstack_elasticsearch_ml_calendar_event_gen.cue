...
elasticstack_elasticsearch_ml_calendar_event: {
	...
	close({
		...
		// The identifier for the calendar that owns the event. Must contain lowercase
		// alphanumeric characters (a-z and 0-9), hyphens, or underscores. Must start
		...
		// When set, changes the duration of the event to the specified value in seconds
		// (decimal digits as a string; the API uses a JSON number). Requires
		// Elasticsearch **8.16** or newer. Maps to `force_time_shift` in the
		// Elasticsearch API.
		...
		// event period. When omitted, the request does not send this field and
		// Elasticsearch applies its default behavior. Explicit values require
		// Elasticsearch **8.16** or newer. Maps to `skip_model_update` in the
		// Elasticsearch API.
		skip_model_update?: bool

		// If true, results are not generated for buckets that fall inside the event
		// period. When omitted, the request does not send this field and Elasticsearch
		// applies its default behavior. Explicit values require Elasticsearch **8.16**
		// or newer. Maps to `skip_result` in the Elasticsearch API.
		skip_result?: bool
