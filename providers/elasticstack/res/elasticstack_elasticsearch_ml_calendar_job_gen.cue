...
elasticstack_elasticsearch_ml_calendar_job: {
	...
	close({
		...
		// Identifier of the ML calendar. Must contain lowercase alphanumeric characters
		// (a-z and 0-9), dots, hyphens, or underscores. Must start and end with an
		...
		// calendar, matching Elasticsearch `PUT .../jobs/{job_id}` (one value per
		// resource; not a comma-separated list).
		job_id!: string
	})
