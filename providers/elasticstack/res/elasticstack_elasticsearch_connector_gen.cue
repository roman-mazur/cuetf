package res
	...
	close({
		...
		// ID of the API key used by the connector service for
		// authorization.
		api_key_id?: string

		// ID of the connector secret holding the API key (Elastic-managed
		// connectors only).
		api_key_secret_id?: string

		// Unique connector identifier. When omitted, Elasticsearch
		// auto-generates an ID on create.
		connector_id?: string
		...
		// `<cluster_uuid>/<connector_id>`.
		id?: string

		// Destination Elasticsearch index name. When omitted,
		// Elasticsearch may assign a default on create.
		index_name?: string

		// Whether this is an Elastic-managed connector (`true`) or
		// self-managed (`false`). Defaults to `false` on the
		// Elasticsearch side when omitted.
		is_native?: bool
		...
		// Connector service type (for example `postgresql`, `mysql`,
		// `github`). New service types may be added over time; the
		// provider does not validate against a fixed enum.
		service_type!: string

		// User-supplied connector configuration values keyed by field
		// name. Each element must set exactly one of `string`, `number`,
		// `bool`, `json`, or `secret_value`. Removing a key stops
		// managing it but does not unset the value server-side.
		configuration_values?: [string]: close({
			...
			// Write-only secret configuration value. Drift is detected via
			// private-state hashing (see resource documentation).
			secret_value?: string
		...
		// Connector feature flags. Changes trigger `PUT
		// /_connector/{id}/_features`.
		features?: close({
		...
		// Ingest pipeline settings applied to synced documents. Changes
		// trigger `PUT /_connector/{id}/_pipeline`.
		pipeline?: close({
		...
		// Changes trigger `PUT /_connector/{id}/_scheduling`.
		scheduling?: close({
