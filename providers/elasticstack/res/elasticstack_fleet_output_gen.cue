...
elasticstack_fleet_output: {
	...
	close({
		...
		ssl?: close({
			...
			// The SSL verification mode. One of `certificate`, `full`, `none`, `strict`.
			verification_mode?: string
		})

		// Advanced YAML configuration. YAML settings here will be added to the output
		// section of each agent policy. Note: the Fleet API treats an omitted
		// `config_yaml` in an update request as "no change" and does not clear the
		// stored value. To clear a previously stored value, delete and re-create the
		// output.
		config_yaml?: string
		timeouts?: close({
		...
		// The Kibana space IDs where this output is available. When set, the output
		// will be created and managed within the specified space. Note: The order of
		// space IDs does not matter as this is a set.
		space_ids?: [...string]

		// When type is remote_elasticsearch, whether Fleet synchronizes integration
		// assets to the remote cluster. Subscription and version requirements apply
		// per Elastic documentation.
		sync_integrations?: bool

		// When type is remote_elasticsearch, whether to sync uninstalled integrations.
		// Only meaningful when sync_integrations is enabled.
		sync_uninstalled_integrations?: bool

		// The output type.
		type!: string

		// When type is remote_elasticsearch, whether agents using this output send data
		// to wired logs streams (preview in newer stacks).
