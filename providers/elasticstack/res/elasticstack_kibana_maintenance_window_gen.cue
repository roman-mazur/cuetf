...
elasticstack_kibana_maintenance_window: {
	...
	close({
		...
		custom_schedule!: close({
			// The duration of the schedule. It allows values in `<integer><unit>` format.
			// `<unit>` is one of `d`, `h`, `m`, or `s` for days, hours, minutes, seconds.
			// For example: `1d`, `5h`, `30m`, `5000s`.
			duration!: string

			// A set schedule over which the maintenance window applies.
			recurring!: close({
				// The end date and time of the schedule, provided in ISO 8601 format and set to
				// the UTC timezone. For example: `2025-03-12T12:00:00.000Z`.
				end?: string
				...
				// The specific days of the week (`[MO,TU,WE,TH,FR,SA,SU]`) or nth day of month
				// (`[+1MO, -3FR, +2WE, -4SA, -5SU]`) for a recurring schedule.
				on_week_day?: [...string]
			})

			// The start date and time of the schedule, provided in ISO 8601 format and set
			// to the UTC timezone. For example: `2025-03-12T12:00:00.000Z`.
			start!: string
