...
elasticstack_kibana_security_detection_rule: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/elasticstack_kibana_security_detection_rule")
	close({
		// Start date to use when checking if a term has been seen before. Supports
		// relative dates like 'now-30d'. Required for new_terms rules.
		...
		actions?: matchN(1, [close({
			// The action type used for sending notifications (e.g., .slack, .email,
			// .webhook, .pagerduty, etc.).
			action_type_id!: string

			// Conditions that affect whether the action runs. If you specify multiple
			// conditions, all conditions must be met for the action to run.
			//
			// The `query` attribute accepts a KQL string and a JSON array of Kibana filter
			// objects. Use `filters_json = jsonencode([])` when no filters are required.
			// Example: `alerts_filter = { query = { kql = "event.action : \"test\""
			// filters_json = jsonencode([]) } }`.
			alerts_filter?: close({
				// KQL query and Kibana filter DSL conditions that determine whether the action runs.
				query?: close({
					// JSON-encoded array of Kibana filter DSL objects. Use `jsonencode([])` for an empty filter list.
					filters_json?: string

					// Defines a KQL query filter that determines whether the action runs. Written
					// in Kibana Query Language (KQL).
					kql?: string
				})
			...
			frequency?: close({
				// Defines how often rules run actions. Valid values: onActionGroupChange,
				// onActiveAlert, onThrottleInterval.
				notify_when!: string

				// Action summary indicates whether we will send a summary notification about
				// all the generated alerts or notification per individual alert.
				summary!: bool
			...
			// JSON-encoded object containing the allowed connector fields, which varies
			// according to the connector type. Use `jsonencode({...})` to set this value.
			params!: string
		...
		// Anomaly score threshold above which the rule creates an alert. Valid values
		// are from 0 to 100. Required for machine_learning rules.
		...
		alert_suppression?: close({
			...
			// Strategy for handling missing fields in suppression grouping: 'suppress' -
			// only one alert will be created per suppress by bucket, 'doNotSuppress' - per
			// each document a separate alert will be created.
		...
		exceptions_list?: matchN(1, [close({
			...
			// The type of exception container. Valid values are `detection`, `endpoint`,
			// `endpoint_events`, `endpoint_host_isolation_exceptions`,
			// `endpoint_blocklists`, `endpoint_trusted_apps`, `endpoint_trusted_devices`,
			// and `rule_default`.
			type!: string
		}), [...close({
			// The exception container ID.
		...
		// Determines if the rule acts as a building block. If set, value must be
		// `default`. Building-block alerts are not displayed in the UI by default and
		// are used as a foundation for other rules.
		...
		// Machine learning job ID(s) the rule monitors for anomaly scores. Required for
		...
		// Array of Elasticsearch fields and types that must be present in source
		// indices for the rule to function properly.
		required_fields?: matchN(1, [close({
			// Indicates whether the field is ECS-compliant. This is computed by the backend
			// based on the field name and type.
			ecs?: bool
		...
		// Array of risk score mappings to override the default risk score based on
		// source event field values.
		risk_score_mapping?: matchN(1, [close({
			...
			// Risk score to use when the field matches the value (0-100). If omitted, uses
			// the rule's default risk_score.
			risk_score?: number
		...
		// Array of threat mappings that specify how to match events with threat
		// intelligence. Required for threat_match rules.
		...
		// Query and filter context array to define alert conditions as JSON. Supports
		// complex filter structures including bool queries, term filters, range
		// filters, etc. Available for all rule types.
		filters?: string
		...
		// Rule type. Supported types: query, eql, esql, machine_learning, new_terms,
		// saved_query, threat_match, threshold.
		type!: string
