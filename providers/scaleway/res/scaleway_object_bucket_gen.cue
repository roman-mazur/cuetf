...
scaleway_object_bucket: {
	...
	#lifecycle_rule: close({
		...
		// Specifies the number of days after initiating a multipart upload when the
	...
	_#defs: "/$defs/lifecycle_rule/$defs/expiration": close({
		// Specifies the date the object is to be moved or deleted. The date value must
		// be in RFC3339 full-date format e.g. `2023-08-22`
		date?: string

		// Specifies the number of days after object creation when the specific rule action takes effect
		days?: number

		// Specifies whether Scaleway Object will remove a delete marker with no
		// noncurrent versions. If set to `true`, the delete marker will be expired; if
		// set to `false` the policy takes no action
		expired_object_delete_marker?: bool
	})

	_#defs: "/$defs/lifecycle_rule/$defs/noncurrent_version_expiration": close({
		// Number of noncurrent versions Scaleway Object Storage will retain. Must be a
		// non-zero positive integer
		newer_noncurrent_versions?: number

		// Number of days an object is noncurrent before Scaleway Object Storage can
		// perform the associated action. Must be a positive integer
	...
	_#defs: "/$defs/lifecycle_rule/$defs/transition": close({
		// Specifies the date objects are transitioned to the specified storage class.
		// The date value must be in RFC3339 full-date format e.g. `2023-08-22`
		date?: string
