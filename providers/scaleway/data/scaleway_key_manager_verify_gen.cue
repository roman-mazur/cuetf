...
scaleway_key_manager_verify: {
	...
	close({
		// Digest of the original signed message. Must be generated using the same
		// algorithm specified in the key’s configuration, and encoded as a base64
		// string.
		digest!: string
		id?:     string
		...
		// Defines whether the signature is valid. Returns `true` if the signature is
		// valid for the digest and key, and `false` otherwise.
		valid?: bool
	})
}
