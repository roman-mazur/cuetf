	...
	close({
		...
		assets?: close({
			...
			config?: close({
				...
				// The contents of a _redirects file (used to apply redirects or proxy paths
				// ahead of asset serving).
				redirects?: string
		...
		bindings?: matchN(1, [close({
			...
			// "secrets_store_secret", "secret_key", "workflow", "wasm_module",
		...
		// Path to a file containing the Module or Service Worker contents of the
		// Worker. Conflicts with `content`. Must be paired with `content_sha256`.
		content_file?: string

		// SHA-256 hash of the Worker contents. Used to trigger updates when source code
		// changes. Must be provided when `content_file` is specified.
		...
		// Per-entrypoint export configuration. Keys are the export names; values
		// describe the entrypoint's kind and per-entrypoint cache behavior.
		exports?: [string]: close({
			// Per-entrypoint cache override. When present, this overrides the top-level
			// `cache_options` for this specific entrypoint.
			cache?: close({
				// Whether caching is enabled for this entrypoint.
				enabled!: bool
			})

			// The kind of entrypoint. A `type: worker` entry overrides the top-level
			// `cache_options` for this specific entrypoint.
			type!: string
		})
		...
		// Retain assets which exist for a previously uploaded Worker version; used in
		// lieu of providing a completion token. An explicit `assets` upload takes
		// precedence over `keep_assets`.
		keep_assets?: bool
		...
		// Name of the uploaded file that contains the main module (e.g. the file
		// exporting a `fetch` handler). Indicates a `module syntax` Worker.
		main_module?: string
