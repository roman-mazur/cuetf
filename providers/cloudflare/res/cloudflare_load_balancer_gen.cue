...
cloudflare_load_balancer: {
	...
	close({
		...
		// response codes. If there is another healthy origin in the same pool, the
		// request is retried once against this alternate origin.
		...
		location_strategy?: close({
			...
			// exist in the request, or its GeoIP lookup is unsuccessful.
			// - `"pop"`: Use the Cloudflare PoP location.
			// - `"resolver_ip"`: Use the DNS resolver GeoIP location. If the GeoIP lookup
			// is unsuccessful, use the Cloudflare PoP location.
		...
		// each pool's outstanding requests.
		...
		rules?: matchN(1, [close({
			...
			// routing to a pool. If a fixed_response is supplied the rule will be marked
			// as terminates.
			...
			overrides?: close({
				...
				session_affinity_attributes?: close({
					...
					// how HTTP headers on load balancing requests will be used. The supported
					// values are: - `"true"`: Load balancing requests must contain *all* of the
					// HTTP headers specified by the `headers` session affinity attribute,
					...
					// Secure attribute depending if Always Use HTTPS is enabled.
				...
				// connections. Pools with more open connections are weighted proportionately
				// less relative to others. Supported for HTTP/1 and HTTP/2 connections.
