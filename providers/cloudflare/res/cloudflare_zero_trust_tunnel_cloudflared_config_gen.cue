...
cloudflare_zero_trust_tunnel_cloudflared_config: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/cloudflare_zero_trust_tunnel_cloudflared_config")
	close({
		// Identifier.
		account_id!: string
		...
		config?: close({
			...
			ingress?: matchN(1, [close({
				...
				origin_request?: close({
					// For all L7 requests to this hostname, cloudflared will validate each
					// request's Cf-Access-Jwt-Assertion request header.
					access?: close({
						// Access applications that are allowed to reach this hostname for this Tunnel.
						// Audience tags can be identified in the dashboard or via the List Access
						// policies API.
						aud_tag!: [...string]
					...
					// This option should be used only if your certificate is not signed by
					// Cloudflare.
					ca_pool?: string
					...
					// proxying, for example, SSH or RDP. This configures what type of proxy will
					// be started. Valid options are: "" for the regular proxy and "socks" for a
					// SOCKS5 proxy.
					proxy_type?: string
				...
				// https://, unix://, tcp://, ssh://, rdp://, unix+tls://, smb://.
				// Alternatively can return a HTTP status code http_status:[code] e.g.
				// 'http_status:404'.
				service!: string
			}), [...close({
				// Public hostname for this service.
		...
		// manage the tunnel's configuration on the Zero Trust dashboard.
