...
#cloudflare_zero_trust_tunnel_warp_connector_config: {
	...
	close({
		...
		// Provider-specific configuration. Required shape depends on
		// ha_mode. For `aws`, must contain `fnr_id`. For `local`, must
		// contain `vips`. For `none` and `disabled`, must be empty or
		// omitted.
		config?: close({
