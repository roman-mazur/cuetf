...
cloudflare_zero_trust_access_application: {
	...
	close({
		...
		// authentication. This setting always overrides the organization setting for
		// WARP authentication.
		...
		// to allow for more flexibility in defining different types of domains. If
		...
		destinations?: matchN(1, [close({
			...
			// A MCP server id configured in ai-controls. Access will secure the MCP server
			// if accessed through a MCP portal.
		...
		// Configures multi-factor authentication (MFA) settings for the application.
		// Only valid for self_hosted, ssh, vnc, and rdp application types.
		mfa_config?: close({
		...
		// Optional configuration for managing an OAuth authorization flow controlled by
		// Access. When set, Access will act as the OAuth authorization server for this
		// application. This feature is currently in beta.
		oauth_configuration?: close({
			...
			dynamic_client_registration?: close({
				...
				// clients. Must use the `https` protocol. Paths may end in `/*` to match all
				// sub-paths.
				allowed_uris?: [...string]
		...
		policies?: matchN(1, [close({
			...
			// Configures multi-factor authentication (MFA) settings for this policy. For
			// infrastructure applications only `ssh_piv_key` is a supported authenticator;
			// for other application types use `totp`, `biometrics`, or `security_key`.
			mfa_config?: close({
				// The authenticators allowed for MFA.
		...
		saas_app?: close({
			...
			custom_attributes?: matchN(1, [close({
				...
				// Available values: "urn:oasis:names:tc:SAML:2.0:attrname-format:unspecified",
			...
			// should evaluate to a singular string. The output of this expression can
			// override the `name_id_format` setting.
		...
		scim_config?: close({
			...
			authentication?: close({
				...
				// Client ID used to authenticate when generating a token for authenticating
				// with the remote SCIM service.
				client_id?: string

				// Secret used to authenticate when generating a token for authenticating with
				// the remove SCIM service.
				client_secret?: string
		...
		// Available values: "self_hosted", "saas", "ssh", "vnc", "app_launcher",
		// "warp", "biso", "bookmark", "dash_sso", "infrastructure", "rdp", "mcp",
