...
cloudflare_zero_trust_organization: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/cloudflare_zero_trust_organization")
	close({
		...
		// Determines whether to deny all requests to Cloudflare-protected resources
		// that lack an associated Access application. If enabled, you must explicitly
		// configure an Access application and policy to allow traffic to your
		...
		// Requests to a subdomain in an exempted zone will block unauthenticated
		// traffic by default if there is a configured Access application and policy
		// that matches the request.
		deny_unmatched_requests_exempted_zone_names?: [...string]

		// Lock all settings as Read-Only in the Dashboard, regardless of user
		// permission. Updates may only be made via the API or Terraform for this
		...
		mfa_config?: close({
			...
			// when the AMR claim provided by the IdP the user used to authenticate
			// contains "mfa". Must be in minutes (m) or hours (h). Minimum: 0m. Maximum:
			// 720h (30 days).
		...
		mfa_ssh_piv_key_requirements?: close({
			// Defines when a PIN is required to use the SSH key. Valid values: `never` (no
			// PIN required), `once` (PIN required once per session), `always` (PIN
			...
			// `never` (no touch required), `always` (touch required for each use),
		...
		// The amount of time a user seat is inactive before it expires. When the user
		// seat exceeds the set time of inactivity, the user is removed as an active
		// seat and no longer counts against your Teams seat count. Minimum value for
