...
cloudflare_dns_firewall: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/cloudflare_dns_firewall")
	close({
		// Identifier.
		account_id!: string
		...
		// Number of IPv4 addresses to assign to the DNS Firewall cluster. Only used
		// during cluster creation and cannot be changed later.
		...
		// Maximum number of DNS queries per second that will be forwarded to your
		// upstream nameservers. The limit is enforced per server, where each server
		// receives a fraction of the configured value. The actual aggregate rate for a
		// data center may vary depending on how many servers are present. Responses
		// served from cache do not count toward this limit. Set to null to disable
		// rate limiting.
		ratelimit?: number
