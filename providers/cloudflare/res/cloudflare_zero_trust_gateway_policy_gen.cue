...
cloudflare_zero_trust_gateway_policy: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/cloudflare_zero_trust_gateway_policy")
	close({
		account_id!: string

		// Specify the action to perform when the associated traffic, identity, and
		// device posture expressions either absent or evaluate to `true`.
		// Available values: "on", "off", "allow", "block", "scan", "noscan",
		// "safesearch", "ytrestricted", "isolate", "noisolate", "override",
		// "l4_override", "egress", "resolve", "quarantine", "redirect".
		action!:     string
		created_at?: string
		...
		expiration?: close({
			...
			// follow RFC 3339 and include a UTC offset. The system accepts non-zero
			// offsets but converts them to the equivalent UTC+00:00 value and returns
			// timestamps with a trailing Z. Expiration policies ignore client timezones
		...
		// processing phase, evaluate applicable rules in ascending order of this
		...
		rule_settings?: close({
			...
			// as keys that map to arrays of header values. Settable only for `http` rules
			...
			biso_admin_controls?: close({
				...
				// absent, copying remains enabled. Applies only when version == "v2".
				...
				// pasting remains enabled. Applies only when version == "v2".
				// Available values: "enabled", "disabled", "remote_only".
				paste?: string
			...
			// via WARP IPs. Settable only for `egress` rules.
			egress?: close({
