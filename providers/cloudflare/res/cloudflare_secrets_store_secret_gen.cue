...
#cloudflare_secrets_store_secret: {
	...
	close({
		...
		// `workers`, `ai_gateway`, `dex`, and `access`. Must be listed
		...
		// The value of the secret. Maximum 64 KiB (65,536 bytes). Note
		// that this is 'write only' - no API response will provide this
		// value, it is only used to create/modify secrets.
		value!: string
	})
}
