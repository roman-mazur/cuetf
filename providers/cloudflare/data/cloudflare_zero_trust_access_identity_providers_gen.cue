...
cloudflare_zero_trust_access_identity_providers: {
	...
	close({
		...
		result?: matchN(1, [close({
			...
			// parameters for a specific provider, refer to our [developer
			// documentation](https://developers.cloudflare.com/cloudflare-one/identity/idp-integration/).
			config?: close({
				// Your companies TLD
				apps_domain?: string

				// Add a list of attribute names that will be returned in the response header
				// from the Access callback.
				...
				// A list of SAML attribute names that will be added to your signed JWT token
				// and can be used in SAML policy rules.
				attributes?: [...string]
				...
				// Indicates the type of user interaction that is required. prompt=login forces
				// the user to enter their credentials on that request, negating single-sign
				// on. prompt=none is the opposite. It ensures that the user isn't presented
				// with any interactive prompt. If the request can't be completed silently by
				// using single-sign on, the Microsoft identity platform returns an
			...
			saml_certificate_set?: close({
				...
				// Null if no rotation has occurred. Mirrors the structure of
				// `saml_certificate`.
			...
			scim_config?: close({
				...
				// Indicates how a SCIM event updates a user identity used for policy
				// evaluation. Use "automatic" to automatically update a user's identity and
				// augment it with fields from the SCIM user resource. Use "reauth" to force
				// re-authentication on group membership updates, user identity update will
				// only occur after successful re-authentication. With "reauth" identities will
				// not contain fields from the SCIM user resource. With "no_action" identities
				// will not be changed by SCIM updates in any way and users will not be
				...
				// A flag to remove a user's seat in Zero Trust when they have been
				// deprovisioned in the Identity Provider. This cannot be enabled unless
				// user_deprovision is also enabled.
				seat_deprovision?: bool

				// A read-only token generated when the SCIM integration is enabled for the
				// first time. It is redacted on subsequent requests. If you lose this you will
				// need to refresh it at /access/identity_providers/:idpID/refresh_scim_secret.
				secret?: string

				// A flag to enable revoking a user's session in Access and Gateway when they
				// have been deprovisioned in the Identity Provider.
			...
			// The type of identity provider. To determine the value for a specific
			// provider, refer to our [developer
			// documentation](https://developers.cloudflare.com/cloudflare-one/identity/idp-integration/).
			// Available values: "onetimepin", "azureAD", "saml", "centrify", "facebook",
		...
		// Indicates to Access to only retrieve identity providers that have the System
		// for Cross-Domain Identity Management (SCIM) enabled.
		scim_enabled?: string
