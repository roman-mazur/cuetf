...
cloudflare_healthchecks: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/data/cloudflare_healthchecks")
	close({
		...
		result?: matchN(1, [close({
			...
			http_config?: close({
				...
				// The expected HTTP response codes (e.g. "200") or code ranges (e.g. "2xx" for
				// all codes starting with 2) of the health check.
				...
				// Port number to connect to for the health check. Defaults to 80 if type is
				// HTTP or 443 if type is HTTPS.
				port?: number
			})
			...
			// The number of consecutive fails required from a health check before changing
			// the health to unhealthy.
			consecutive_fails?: number

			// The number of consecutive successes required from a health check before
			// changing the health to healthy.
			...
			// notifications if the origin status changes, but will increase load on the
			// origin as we check from multiple locations.
			interval?:    number
			modified_on?: string

			// A short name to identify the health check. Only alphanumeric characters,
