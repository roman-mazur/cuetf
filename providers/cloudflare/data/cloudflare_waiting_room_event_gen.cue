...
cloudflare_waiting_room_event: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/data/cloudflare_waiting_room_event")
	close({
		created_on?: string
		...
		// users will be processed with the event's configuration. The start time must
		// be at least one minute before `event_end_time`.
		event_start_time?: string
		id?:               string
		modified_on?:      string

		// A unique name to identify the event. Only alphanumeric characters, hyphens
		// and underscores are allowed.
		name?: string

		// If set, the event will override the waiting room's `new_users_per_minute`
		// property while it is active. If null, the event will inherit it. This can
		// only be set if the event's `total_active_users` property is also set.
		...
		// useful for situations when many users will join the event prequeue at the
		// same time and you want to shuffle them to ensure fairness. Naturally, it
		// makes the most sense to enable this feature when the `queueing_method`
		// during the event respects ordering such as **fifo**, or else the shuffling
		// may be unnecessary.
		shuffle_at_event_start?: bool

		// Suspends or allows an event. If set to `true`, the event is ignored and
		// traffic will be handled based on the waiting room configuration.
		suspended?: bool

		// If set, the event will override the waiting room's `total_active_users`
		// property while it is active. If null, the event will inherit it. This can
		// only be set if the event's `new_users_per_minute` property is also set.
		total_active_users?: number

		// If set, the event will override the waiting room's `turnstile_action`
		// property while it is active. If null, the event will inherit it.
