...
cloudflare_access_rule: {
	...
	close({
		...
		configuration?: close({
			// The configuration target. You must set the target to `ip` when specifying an
			// IP address in the rule.
		...
		filter?: close({
			configuration?: close({
				...
				// Defines the target value to search for in existing rules: an IP address, an
				// IP address range, or a country code, depending on the provided
			...
			// Notes: For example, the string 'attack' would match IP Access rules with
