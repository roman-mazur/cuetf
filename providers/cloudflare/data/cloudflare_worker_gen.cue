...
cloudflare_worker: {
	...
	close({
		...
		subdomain?: close({
			...
			// *.workers.dev [preview
			// URL](https://developers.cloudflare.com/workers/configuration/previews/) the
			// Worker would serve on once previews are enabled, e.g.
			// `https://<prefix>-my-worker.my-subdomain.workers.dev`. Present whenever the
			// account owns a workers.dev subdomain, regardless of whether
			// `previews_enabled` is true, so presence does not imply preview URLs are
			// currently live. Absent only when the account owns no workers.dev subdomain.
			...
			// enabled. Present whenever the account owns a workers.dev subdomain,
			// regardless of whether `enabled` is true, so presence does not imply the
			// Worker is currently live at this URL. Absent only when the account owns no
