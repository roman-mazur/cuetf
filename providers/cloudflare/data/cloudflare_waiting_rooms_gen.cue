	...
	close({
		...
		result?: matchN(1, [close({
			...
			additional_routes?: matchN(1, [close({
				...
				// will be enabled for all subpaths as well. If there are two waiting rooms on
				// the same subpath, the waiting room for the most specific path will be
			...
			// html file that will be rendered at the edge. If no custom_page_html is
			...
			// are evaluated by the Cloudflare edge:
			// 1. {{`waitTimeKnown`}} Acts like a boolean value that indicates the behavior
			// to take when wait time is not available, for instance when queue_all is
			// **true**.
			// 2. {{`waitTimeFormatted`}} Estimated wait time for the user. For example,
			// five minutes. Alternatively, you can use:
			...
			//
			// To view the full list of variables, look at the `cfWaitingRoom` object
			// described under the `json_response_enabled` property in other Waiting Room
			// API calls.
			...
			// 2. `waitTimeKnown`: Boolean indicating if the current estimated wait times
			// are accurate. If **false**, they are not available.
			// 3. `waitTime`: Valid only when `waitTimeKnown` is **true**. Integer
			// indicating the current estimated time in minutes the user will wait in the
			// waiting room. When `queueingMethod` is **random**, this is set to
			...
			// `waitTimeKnown` is **true**. Integer indicating the current estimated
			// maximum wait time for the 50% of users that gain entry the fastest (50th
			// percentile). In other words, half of the queued users are expected to let
			// into the origin website before `waitTime50Percentile` and half are expected
			// to be let in after it.
			// 6. `waitTime75Percentile`: Valid only when `queueingMethod` is **random** and
			// `waitTimeKnown` is **true**. Integer indicating the current estimated
			// maximum wait time for the 75% of users that gain entry the fastest (75th
			// percentile).
			...
			// 10. `lastUpdated`: String displaying the timestamp as an ISO 8601 string of
			// the user's last attempt to leave the waiting room and be let into the origin
			...
			// **zero**.
			// 12. `queueingMethod`: The queueing method currently used by the waiting room.
			// It is either **fifo**, **random**, **passthrough**, or **reject**.
			// 13. `isFIFOQueue`: Boolean indicating if the waiting room uses a FIFO (First-In-First-Out) queue.
			// 14. `isRandomQueue`: Boolean indicating if the waiting room uses a Random
			// queue where users gain access randomly.
			...
			// if it exists.
			// 18. `isEventPrequeueing`: Valid only when `isEventActive` is **true**.
			// Boolean indicating if an event is currently prequeueing users before it
			// starts.
			...
			// `isEventPrequeueing` is **false**, `timeUntilEventStartFormatted` will
			// display **unavailable**.
			...
			// 23. `shuffleAtEventStart`: Valid only when `isEventActive` is **true**.
			// Boolean indicating if the users in the prequeue are shuffled randomly when
			// the event starts.
			// 24. `turnstile`: Empty when turnstile isn't enabled. String displaying an
			// html tag to display the Turnstile widget. Please add the `{{{turnstile}}}`
			// tag to the `custom_html` template to ensure the Turnstile widget appears.
			...
			//
			// If `json_response_enabled` is **true** and the request hits the waiting room,
			// an example JSON response when `queueingMethod` is **fifo** and no event is
			// active could be:
			//
			// {
			// "cfWaitingRoom": {
			...
			//
			// If `json_response_enabled` is **true** and the request hits the waiting room,
			// an example JSON response when `queueingMethod` is **random** and an event is
			// active could be:
			//
			// {
			// "cfWaitingRoom": {
			...
			// 1. `fifo` **(default)**: First-In-First-Out queue where customers gain access
			// in the order they arrived.
			// 2. `random`: Random queue where customers gain access randomly, regardless of arrival time.
			// 3. `passthrough`: Users will pass directly through the waiting room and into
			// the origin website. As a result, any configured limits will not be respected
			// while this is enabled. This method can be used as an alternative to
			// disabling a waiting room (with `suspended`) so that analytics are still
			// reported. This can be used if you wish to allow all traffic normally, but
			// want to restrict traffic during a waiting room event, or vice versa.
