...
cloudflare_load_balancer_pool: {
	...
	close({
		...
		// receive traffic and are excluded from health checks. Disabling a pool will
		// cause any load balancers using it to failover to the next pool (if any).
		enabled?: bool
		filter?: close({
		...
		origin_steering?: close({
			...
			// - `"least_outstanding_requests"`: Select an origin by taking into
			// consideration origin weights, as well as each origin's number of outstanding
			// requests. Origins with more pending requests are weighted proportionately
		...
		origins?: matchN(1, [close({
			...
			// will not receive traffic and are excluded from health checks. The origin
			// will only be disabled for the current pool.
			enabled?: bool
			...
			// pool.
			// - `origin_steering.policy="least_outstanding_requests"`: Use weight to scale
			// the origin's outstanding requests.
