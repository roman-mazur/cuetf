...
cloudflare_page_shield_scripts_list: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/data/cloudflare_page_shield_scripts_list")
	close({
		// The direction used to sort returned scripts.
		// Available values: "asc", "desc".
		direction?: string

		// When true, excludes scripts seen in a `/cdn-cgi` path from the returned
		// scripts. The default value is true.
		exclude_cdn_cgi?: bool

		// When true, excludes duplicate scripts. We consider a script duplicate of
		// another if their javascript
		// content matches and they share the same url host and zone hostname. In such
		// case, we return the most
		...
		// Filters the returned scripts using a comma-separated list of scripts
