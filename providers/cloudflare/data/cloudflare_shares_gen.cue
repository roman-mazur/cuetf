...
#cloudflare_shares: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/data/cloudflare_shares")
	close({
		// Account identifier.
		account_id!: string
		...
		// Filter shares by tag. Each value is either `key=value` (matches
		// shares whose tags contain that key/value pair) or `key` alone
		// (matches shares that have any value for that key). May be
		// repeated; multiple `tag` parameters are ANDed together.
		// Maximum 20 `tag` parameters per request.
		tag?: [...string]
		...
		result?: matchN(1, [close({
			...
			// A list of resources that are part of the share. This field is
			// only included when requested via the 'include_resources'
			// parameter.
