...
cloudflare_magic_transit_site_lans: {
	...
	close({
		...
		result?: matchN(1, [close({
			...
			// on. only one LAN can be set as the ha_link.
			ha_link?: bool
			nat?: close({
			...
			// If the site is not configured in high availability mode, this configuration
			// is optional (if omitted, use DHCP). However, if in high availability mode,
			...
			static_addressing?: close({
				...
				dhcp_server?: close({
					...
					dhcp_options?: matchN(1, [close({
						// DHCP option number (1-254). Options 0 and 255 are reserved by RFC 2132.
						// Options 3, 6, and 51 are not allowed because they conflict with
						// connector-managed configuration.
						code?: number

						// The type of the option value. text: a string (max 255 bytes). hex:
						// colon-separated hex bytes (e.g. "01:04:aa:bb:cc", max 255 bytes). ip: an
						// IPv4 address (e.g. "10.20.30.40"). byte: an unsigned integer 0-255 (1 byte).
						// short: an unsigned integer 0-65535 (2 bytes). integer: an unsigned integer
