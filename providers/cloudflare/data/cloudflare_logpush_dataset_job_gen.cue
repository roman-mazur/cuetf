...
cloudflare_logpush_dataset_job: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/data/cloudflare_logpush_dataset_job")
	close({
		...
		// "audit_logs", "audit_logs_v2", "biso_user_actions", "casb_findings",
		// "device_posture_results", "dex_application_tests",
		// "dex_device_state_events", "dlp_forensic_copies", "dns_firewall_logs",
		// "dns_logs", "email_security_alerts", "email_security_post_delivery_events",
		// "firewall_events", "gateway_dns", "gateway_http", "gateway_network",
		// "http_requests", "ipsec_logs", "magic_bgp_logs", "magic_ids_detections",
		// "mcp_portal_logs", "mnm_flow_logs", "nel_reports", "network_analytics_logs",
		// "page_shield_events", "sinkhole_http_logs", "spectrum_events", "ssh_logs",
		// "turnstile_events", "warp_config_changes", "warp_toggle_changes",
		// "websocket_analytics", "workers_trace_events", "zaraz_events",
		...
		// pushed. Additional configuration parameters supported by the destination may
		// be included.
		...
		// hasn't run yet then the field will be empty.
		...
		// must be between `5 MB` and `1 GB`, or `0` to disable it. Note that you
		// cannot set a minimum file size; this means that log files may be much
		// smaller than this batch size.
		max_upload_bytes?: number

		// The maximum interval in seconds for log batches. This setting must be between
		// 30 and 300 seconds (5 minutes), or `0` to disable it. Note that you cannot
		// specify a minimum interval for log batches; this means that log files may be
		// sent in shorter intervals than this.
		max_upload_interval_seconds?: number

		// The maximum number of log lines per batch. This setting must be between 1000
		// and 1,000,000 lines, or `0` to disable it. Note that you cannot specify a
		// minimum number of log lines per batch; this means that log files may contain
		// many fewer lines than this.
