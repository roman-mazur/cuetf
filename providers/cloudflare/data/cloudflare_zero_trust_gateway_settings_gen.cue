...
cloudflare_zero_trust_gateway_settings: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/data/cloudflare_zero_trust_gateway_settings")
	close({
		account_id?: string
		created_at?: string
		id?:         string
		...
		settings?: close({
			...
			// responses so returned record TTLs do not exceed this value. Null means no
			// cap. Each DNS location can inherit, override, or disable it through the
			// location `max_ttl` setting.
			max_ttl_secs?: number
			...
			antivirus?: close({
				...
				notification_settings?: close({
					...
					// Specify a URL that directs users to more information. If unset, the
			...
			block_page?: close({
				...
				// Specify the subject line for emails created from the block page when the mode
				// is customized_block_page.
				...
				// Specify whether to suppress detailed information at the bottom of the block
				// page when the mode is customized_block_page.
			...
			// Configures user email settings for firewall policies. When you enable this,
			// the system standardizes email addresses in the identity portion of the rule
			// to match extended email variants in firewall policies. When you disable this
			// setting, the system matches email addresses exactly as you provide them.
			...
			inspection?: close({
				// Define the proxy inspection mode. 1. static: Gateway applies static
				// inspection to HTTP on TCP(80). With TLS decryption on, Gateway inspects
				// HTTPS traffic on TCP(443) and UDP(443). 2. dynamic: Gateway applies protocol
				// detection to inspect HTTP and HTTPS traffic on any port. TLS decryption must
				// remain on to inspect HTTPS traffic.
