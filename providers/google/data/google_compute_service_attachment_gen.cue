...
google_compute_service_attachment: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/data/google_compute_service_attachment")
	close({
		...
		// Service Connect connections can be established to this service attachment
		...
		//
		// If unspecified, the default propagated connection limit is 250. To explicitly
		// send a zero value, set 'send_propagated_connection_limit_if_zero = true'.
		...
		//
		// If false, connection policy update will only affect existing PENDING PSC
		// endpoints. Existing ACCEPTED/REJECTED endpoints will remain untouched
		// regardless how the connection policy is modified .
		// If true, update will affect both PENDING and ACCEPTED/REJECTED PSC endpoints.
		// For example, an ACCEPTED PSC endpoint will be moved to REJECTED if its
		// project is added to the reject list.
		...
		// When false, setting propagated_connection_limit to zero causes the provider
		// to use to the API's default value.
