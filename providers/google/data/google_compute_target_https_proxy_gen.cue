...
google_compute_target_https_proxy: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/data/google_compute_target_https_proxy")
	close({
		// URLs to certificate manager certificate resources that are used to
		// authenticate connections between users and the load balancer.
		// Certificate manager certificates only apply when the load balancing scheme is
		...
		// associated with the given target proxy. This field is only supported for
		// EXTERNAL and EXTERNAL_MANAGED load balancing schemes.
		...
		// This field will be ignored when inserting a TargetHttpsProxy. An up-to-date
		// fingerprint must be provided in order to
		...
		// specified, Google manages whether QUIC is used. Default value: "NONE"
		// Possible values: ["NONE", "ENABLE", "DISABLE"]
		...
		// Currently, you may specify up to 15 SSL certificates. sslCertificates do not
		...
		// This applies to TLS 1.3 connections over TCP (HTTP/2) as well as over UDP
		// (QUIC/h3). Possible values: ["STRICT", "PERMISSIVE", "UNRESTRICTED",
		// "DISABLED"]
