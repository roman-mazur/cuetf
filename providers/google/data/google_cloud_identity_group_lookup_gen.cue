...
google_cloud_identity_group_lookup: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/data/google_cloud_identity_group_lookup")
	close({
	...
	#group_key: close({
		// The ID of the entity. For Google-managed entities, the id should be the email
		// address of an existing group or user.
		// For external-identity-mapped entities, the id must be a string conforming to
		// the Identity Source's requirements.
		...
		// If specified, the EntityKey represents an external-identity-mapped group. The
