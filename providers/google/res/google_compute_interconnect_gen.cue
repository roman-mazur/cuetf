...
google_compute_interconnect: {
	...
	close({
		...
		// [Output Only] List of features that are available on this Interconnect
		// connection based on the provisioned hardware and configuration.
		// Possible values include:
		// - 'IF_MACSEC': Indicates the Interconnect connection is provisioned on MACsec
		// capable hardware ports. If this feature is not present, the connection does
		// not support MACsec, and any attempt to enable it will fail.
		// - 'IF_L2_FORWARDING': Indicates the Interconnect connection can be used for
		// Layer 2 (L2) traffic forwarding. If not present, the connection cannot be
		// used with L2 forwarding attachments.
		// - 'IF_CROSS_SITE_NETWORK': Indicates the Interconnect connection is
		// provisioned for Cross-Site Networking.
		...
		// crossconnect. This field is required for Dedicated and Partner Interconnect,
		...
		// - DEDICATED: A dedicated physical interconnection with the customer. Possible
		// values: ["DEDICATED", "PARTNER", "IT_PRIVATE"]
		...
		// - LINK_TYPE_ETHERNET_400G_LR4: A 400G Ethernet with LR4 optics Possible
		...
		// List of features to request for this Interconnect connection. This field is
		// only applicable during Interconnect creation and cannot be modified later.
		// Possible values include:
		// - 'IF_MACSEC': Provisions the connection on hardware ports that support
		// MACsec (Media Access Control Security). If not specified, the system may
		// allocate non-MACsec capable ports if available.
		// - 'IF_L2_FORWARDING': Provisions the connection for Layer 2 (L2) traffic
		// forwarding. If not specified, the connection defaults to Layer 3 (L3)
		// traffic forwarding.
		// - 'IF_CROSS_SITE_NETWORK': Provisions the connection exclusively for Cross-Site Networking.
		// Note: 'MACSEC' is a legacy value for compatibility reasons and has the same
		// effect as 'IF_MACSEC'. 'IF_MACSEC' is preferred. Possible values: ["MACSEC",
		// "CROSS_SITE_NETWORK", "IF_MACSEC", "IF_L2_FORWARDING"]
		...
		// A list of the URLs of all CrossSiteNetwork WireGroups configured to use this
