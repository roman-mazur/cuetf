...
google_cloudfunctions_function: {
	...
	close({
		...
		// Memory (in MB), available to the function. Default value is 256. Possible
		// values include 128, 256, 512, 1024, etc.
		available_memory_mb?: number

		// A set of key/value environment variable pairs available during build time.
		build_environment_variables?: [string]: string

		// The fully-qualified name of the service account to be used for the build step
		// of deploying this function
		...
		// Docker Registry to use for storing the function's Docker images. Allowed
		// values are ARTIFACT_REGISTRY (default) and CONTAINER_REGISTRY.
		docker_registry?: string

		// User managed repository created in Artifact Registry optionally with a
		// customer managed encryption key. If specified, deployments will use Artifact
		// Registry for storing images built with Cloud Build.
		...
		// The security level for the function. Defaults to SECURE_OPTIONAL. Valid only
		...
		// String value that controls what traffic can reach the function. Allowed
		// values are ALLOW_ALL and ALLOW_INTERNAL_ONLY. Changes to this field will
		...
		// A set of key/value label pairs to assign to the function. Label keys must
		...
		// Timeout (in seconds) for the function. Default value is 60 seconds. Cannot be
		// more than 540 seconds.
		timeout?: number

		// Boolean variable. Any HTTP request (of a supported type) to the endpoint will
		// trigger function execution. Supported HTTP request types are: POST, PUT,
		// GET, DELETE, and OPTIONS. Endpoint is returned as https_trigger_url. Cannot
		// be used with trigger_bucket and trigger_topic.
		trigger_http?: bool
		...
		// through it. Allowed values are ALL_TRAFFIC and PRIVATE_RANGES_ONLY. Defaults
		// to PRIVATE_RANGES_ONLY. If unset, this field preserves the previously set
		// value.
	...
	#event_trigger: close({
		...
		// See the documentation on calling Cloud Functions for a full reference of
		// accepted triggers.
		event_type!: string

		// The name or partial URI of the resource from which to observe events. For
		// example, "myBucket" or "projects/my-project/topics/my-topic"
		resource!: string
	})
	...
	#secret_environment_variables: close({
		...
		// Project identifier (due to a known limitation, only project number is
		// supported by this field) of the project that contains the secret. If not
		// set, it will be populated with the function's project, assuming that the
		// secret exists in the same project as of the function.
		project_id?: string
	...
	#secret_volumes: close({
		...
		// paths: "/etc/secrets" Restricted mount paths: "/cloudsql", "/dev/log",
		// "/pod", "/proc", "/var/log".
		mount_path!: string
	...
	#source_repository: close({
		// The URL pointing to the hosted repository where the function was defined at
