...
google_managed_kafka_connect_cluster: {
	...
	close({
		...
		// of the connect cluster's name. This value is structured like:
	...
	#capacity_config: close({
		// The memory to provision for the cluster in bytes. The CPU:memory ratio
		// (vCPU:GiB) must be between 1:1 and 1:8. Minimum: 3221225472 (3 GiB).
		memory_bytes!: string
	...
	_#defs: "/$defs/gcp_config/$defs/access_config/$defs/network_configs": close({
		// Additional DNS domain names from the subnet's network to be made visible to
		// the Connect Cluster. When using MirrorMaker2, it's necessary to add the
		// bootstrap address's dns domain name of the target cluster to make it visible
		// to the connector. For example:
