...
google_app_engine_flexible_app_version: {
	...
	close({
		...
		// in the API request, Terraform will not detect any changes made outside of
		// the Terraform config.
		env_variables?: [string]: string
		id?: string

		// A list of the types of messages that this application is able to receive.
		// Possible values: ["INBOUND_SERVICE_MAIL", "INBOUND_SERVICE_MAIL_BOUNCE",
		// "INBOUND_SERVICE_XMPP_ERROR", "INBOUND_SERVICE_XMPP_MESSAGE",
		// "INBOUND_SERVICE_XMPP_SUBSCRIBE", "INBOUND_SERVICE_XMPP_PRESENCE",
		...
		// App Engine Appspot service account as
		// default if this field is neither provided in app.yaml file nor through CLI flag.
		service_account?: string

		// Current serving status of this version. Only the versions with a SERVING
		// status create instances and can be billed. Default value: "SERVING" Possible
		// values: ["SERVING", "STOPPED"]
		serving_status?: string

		// Relative name of the version within the service. For example, 'v1'. Version
	...
	#api_config: close({
		...
		// Default value: "AUTH_FAIL_ACTION_REDIRECT" Possible values:
		...
		// "LOGIN_OPTIONAL" Possible values: ["LOGIN_OPTIONAL", "LOGIN_ADMIN",
		// "LOGIN_REQUIRED"]
		login?: string
	...
	#automatic_scaling: close({
		...
		// information from a new instance.
	...
	#deployment: close({
		...
		//
		// By default, the rollout strategy for Endpoints is "FIXED". This means that
		// Endpoints starts up with a particular configuration ID.
		// When a new configuration is rolled out, Endpoints must be given the new
		// configuration ID. The configId field is used to give the configuration ID
		// and is required in this case.
		//
		// Endpoints also has a rollout strategy called "MANAGED". When using this,
		...
		// Endpoints rollout strategy. If FIXED, configId must be specified. If MANAGED,
		// configId must be omitted. Default value: "FIXED" Possible values: ["FIXED",
		// "MANAGED"]
	...
	#handlers: close({
		...
		// Actions to take when the user is not logged in. Possible values:
		// ["AUTH_FAIL_ACTION_REDIRECT", "AUTH_FAIL_ACTION_UNAUTHORIZED"]
		auth_fail_action?: string

		// Methods to restrict access to a URL based on login status. Possible values:
		// ["LOGIN_OPTIONAL", "LOGIN_ADMIN", "LOGIN_REQUIRED"]
		login?: string

		// 30x code to use when performing redirects for the secure field. Possible
		// values: ["REDIRECT_HTTP_RESPONSE_CODE_301",
		...
		// characters must be escaped, but should not contain groupings.
	...
	#manual_scaling: close({
		...
		//
		// **Note:** When managing the number of instances at runtime through the App
		// Engine Admin API or the (now deprecated) Python 2
		// Modules API set_num_instances() you must use 'lifecycle.ignore_changes =
	...
	#network: close({
		...
		// Network, then only network name should be specified (not the subnetworkName)
		...
		// Network, then the subnetworkName must be specified and the IP address is
		// created from the IPCidrRange of the subnetwork.
		// If specified, the subnetwork must exist in the same region as the App Engine
	...
	_#defs: "/$defs/deployment/$defs/container": close({
		...
		// be fully qualified and include a tag or digest.
	...
	_#defs: "/$defs/handlers/$defs/static_files": close({
		...
		// declared in static file handlers are
		// uploaded as static data and are only served to end users; they cannot be read
		// by the application. If enabled,
