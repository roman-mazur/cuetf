...
google_compute_network: {
	...
	close({
		...
		// The BGP best selection algorithm to be employed. MODE can be LEGACY or
		// STANDARD. Possible values: ["LEGACY", "STANDARD"]
		bgp_best_path_selection_mode?: string

		// Choice of the behavior of inter-regional cost and MED in the BPS algorithm.
		// Possible values: ["DEFAULT", "ADD_COST_TO_MED"]
		bgp_inter_region_cost?: string

		// If set to 'true', the 'bgp_always_compare_med' field will be cleared.
		// If set to 'false' (the default), 'bgp_always_compare_med' will be set to the
		...
		// Note that packets larger than 1500 bytes (standard Ethernet) can be subject
		// to TCP-MSS clamping or dropped
		// with an ICMP 'Fragmentation-Needed' message if the packets are routed to the
		...
		// Default value: "AFTER_CLASSIC_FIREWALL" Possible values:
