...
google_bigtable_app_profile: {
	...
	close({
		...
		// If true, read/write requests are routed to the nearest cluster in the
		// instance, and will fail over to the nearest cluster that is available
		// in the event of transient errors or delays. Clusters in a region are
		// considered equidistant. Choosing this option sacrifices read-your-writes
		...
		// Must be used with multi-cluster routing. If true, then this app profile will
		// use row affinity sticky routing. With row affinity, Bigtable will route
		// single row key requests based on the row key, rather than randomly. Instead,
		// each row key will be assigned to a cluster by Cloud Bigtable, and will stick
		// to that cluster. Choosing this option improves read-your-writes consistency
		// for most requests under most circumstances, without sacrificing
		// availability. Consistency is not guaranteed, as requests may still fail over
		// between clusters in the event of errors or latency.
		row_affinity?: bool
		project?:      string
	})
	...
	#standard_isolation: close({
		// The priority of requests sent using this app profile. Possible values:
