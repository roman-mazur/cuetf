...
google_pubsub_subscription: {
	...
	close({
		...
		//
		// - The message sent to a subscriber is guaranteed not to be resent before the
		...
		// These copies are considered distinct by Pub/Sub and have distinct messageId
		// values
		...
		// Pub/Sub automatically acknowledges the messages that don't match the filter.
		// You can filter messages
		// by their attributes. The maximum length of a filter is 256 bytes. After
	...
	#bigquery_config: close({
		// When true and use_topic_schema or use_table_schema is true, any fields that
		// are a part of the topic schema or message schema that
		// are not part of the BigQuery table schema are dropped when writing to
		// BigQuery. Otherwise, the schemas must be kept in sync
		...
		// When true, write the subscription name, messageId, publishTime, attributes,
		// and orderingKey to additional columns in the table.
		// The subscription name, messageId, and publishTime fields are put in their own
	...
	#cloud_storage_config: close({
		...
		// User-provided name for the Cloud Storage bucket. The bucket must be created
		// by the user. The bucket name must be without any prefix like "gs://".
		bucket!: string
		...
		// file is created. Min 1 KB, max 10 GiB.
		// The maxBytes limit may be exceeded in cases where messages are larger than the limit.
		max_bytes?: number

		// The maximum duration that can elapse before a new Cloud Storage file is
		// created. Min 1 minute, max 10 minutes, default 5 minutes.
		// May not exceed the subscription's acknowledgement deadline.
		// A duration in seconds with up to nine fractional digits, ending with 's'. Example: "3.5s".
		max_duration?: string

		// The maximum messages that can be written to a Cloud Storage file before a new
		// file is created. Min 1000 messages.
		max_messages?: number
	...
	_#defs: "/$defs/cloud_storage_config/$defs/avro_config": close({
		...
		// When true, write the subscription name, messageId, publishTime, attributes,
		// and orderingKey as additional fields in the output.
		write_metadata?: bool
	})
