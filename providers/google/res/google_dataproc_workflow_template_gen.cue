...
google_dataproc_workflow_template: {
	...
	close({
		...
		// cancelled, the workflow is ended, and if the workflow was running on a
		// [managed
	...
	#parameters: close({
		...
		// that references the zone field of a workflow template's cluster selector
		// would be specified as `placement.clusterSelector.zone`. Also, field paths
		...
		// jobs['step-id'].pySparkJob.pythonFileUris[0] * Items in repeated fields can
		// be referenced by a zero-based index: * jobs['step-id'].sparkJob.args[0] *
		// Other examples: * jobs['step-id'].hadoopJob.properties['key'] *
		// jobs['step-id'].hadoopJob.args[0] *
		// jobs['step-id'].hiveJob.scriptVariables['key'] *
		// jobs['step-id'].hadoopJob.mainJarFileUri * placement.clusterSelector.zone It
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/gce_cluster_config": close({
		...
		// if it exists. Cannot be a "Custom Subnet Network" (see [Using
		...
		// Optional. The type of IPv6 access for a cluster. Possible values:
		// PRIVATE_IPV6_GOOGLE_ACCESS_UNSPECIFIED, INHERIT_FROM_SUBNETWORK, OUTBOUND,
		// BIDIRECTIONAL
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/gce_cluster_config/$defs/reservation_affinity": close({
		// Optional. Type of reservation to consume Possible values: TYPE_UNSPECIFIED,
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/gce_cluster_config/$defs/shielded_instance_config": close({
		...
		// integrity policy baseline and returns a pair of pass/fail results depending
		// on whether they match or not.
		enable_integrity_monitoring?: bool

		// Optional. Defines whether the instances have Secure Boot enabled. Secure Boot
		...
		// Optional. Defines whether the instance have the vTPM enabled. Virtual Trusted
		// Platform Module protects objects like keys, certificates and enables
		// Measured Boot by performing the measurements needed to create a known good
		// boot baseline, called the integrity policy baseline.
		enable_vtpm?: bool
	})
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/master_config": close({
		...
		// * `projects/[project_id]/global/images/[image-id]` * `image-id` Image family
		...
		// `n1-standard-2` **Auto Zone Exception**: If you are using the Dataproc [Auto
		// Zone
		...
		// standard cluster [master_config](#FIELDS.master_config) groups, **must be
		// set to 1**.
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/master_config/$defs/accelerators": close({
		...
		// * `nvidia-tesla-k80` **Auto Zone Exception**: If you are using the Dataproc
		// [Auto Zone
		// Placement](https://cloud.google.com/dataproc/docs/concepts/configuring-clusters/auto-zone#using_auto_zone_placement)
	...
	_#defs: "/$defs/placement/$defs/managed_cluster/$defs/config/$defs/software_config": close({
		...
		// capacity-scheduler: `capacity-scheduler.xml` * core: `core-site.xml` *
		// distcp: `distcp-default.xml` * hdfs: `hdfs-site.xml` * hive: `hive-site.xml`
		// * mapred: `mapred-site.xml` * pig: `pig.properties` * spark:
		// `spark-defaults.conf` * yarn: `yarn-site.xml` For more information, see
		// [Cluster
