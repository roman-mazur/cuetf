...
google_clouddeploy_custom_target_type: {
	...
	close({
		...
		// The weak etag of the 'CustomTargetType' resource. This checksum is computed
	...
	#custom_actions: close({
		...
		// The Skaffold custom action responsible for render operations. If not provided
		// then Cloud Deploy will perform the render operations via 'skaffold render'.
	...
	_#defs: "/$defs/custom_actions/$defs/include_skaffold_modules/$defs/google_cloud_storage": close({
		...
		// Cloud Storage source paths to copy recursively. For example, providing
		// 'gs://my-bucket/dir/configs/*' will result in Skaffold copying all files
		// within the 'dir/configs' directory in the bucket 'my-bucket'.
		source!: string
	})
	...
	_#defs: "/$defs/tasks/$defs/deploy/$defs/container": close({
		// Args is the container arguments to use. This overrides the default arguments
		// defined in the container image.
		args?: [...string]

		// Command is the container entrypoint to use. This overrides the default
		// entrypoint defined in the container image.
		command?: [...string]
