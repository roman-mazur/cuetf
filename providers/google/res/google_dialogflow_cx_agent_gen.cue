...
google_dialogflow_cx_agent: {
	...
	close({
		...
		// If set to 'true', Terraform will delete the chat engine associated with the
		// agent when the agent is destroyed.
		// Otherwise, the chat engine will persist.
		//
		// This virtual field addresses a critical dependency chain: 'agent' -> 'engine'
		// -> 'data store'. The chat engine is automatically
		// provisioned when a data store is linked to the agent, meaning Terraform
		// doesn't have direct control over its lifecycle as a managed
		// resource. This creates a problem when both the agent and data store are
		// managed by Terraform and need to be destroyed. Without
		// delete_chat_engine_on_destroy set to true, the data store's deletion would
		// fail because the unmanaged chat engine would still be
		...
		//
		// Data store can be linked to an agent through the 'knowledgeConnectorSettings'
		// field of a
		...
		// The description of this agent. The maximum length is 500 characters. If
		...
		// Enable training multi-lingual models for this agent. These models will be
		// trained on all the languages supported by the agent.
		...
		//
		// ~> **Note:** The first time you are deploying an Agent in your project you
		// must configure location settings.
		// This is a one time step but at the moment you can only [configure location
		// settings](https://cloud.google.com/dialogflow/cx/docs/concept/region#location-settings)
		// via the Dialogflow CX console.
		// Another options is to use global location so you don't need to manually
		// configure location settings.
		location!: string

		// Indicates whether the agent is locked for changes. If the agent is locked,
		// modifications to the agent will be rejected except for [agents.restore][].
		locked?: bool
	...
	#answer_feedback_settings: close({
		...
		// to Dialogflow responses. Feature works only if interaction logging is enabled
	...
	#client_certificate_settings: close({
		...
		// The name of the SecretManager secret version resource storing the private key
		// encoded in PEM format. Format:
	...
	#personalization_settings: close({
		...
		// example { "age": "$session.params.age" }.
	...
	_#defs: "/$defs/advanced_settings/$defs/audio_export_gcs_destination": close({
		// The Google Cloud Storage URI for the exported objects. Whether a full object
