...
google_network_services_gateway: {
	...
	close({
		...
		// When deleting a gateway of type 'SECURE_WEB_GATEWAY', this boolean option
		// will also delete auto generated router by the gateway creation.
		// If there is no other gateway of type 'SECURE_WEB_GATEWAY' remaining for that
		// region and network it will be deleted.
		...
		// A fully-qualified GatewaySecurityPolicy URL reference. Defines how a server
		// should apply security policy to inbound (VM to Proxy) initiated connections.
		...
		// The routing mode of the Gateway. This field is configurable only for gateways
		// of type SECURE_WEB_GATEWAY. This field is required for gateways of type
		// SECURE_WEB_GATEWAY. Possible values: ["NEXT_HOP_ROUTING_MODE",
		// "EXPLICIT_ROUTING_MODE"]
		routing_mode?: string
		project?:      string

		// Immutable. Scope determines how configuration across multiple Gateway instances are merged.
		// The configuration for multiple Gateway instances with the same scope will be
		// merged as presented as a single coniguration to the proxy/load balancer.
		//
		// Max length 64 characters. Scope should start with a letter and can only have
		...
		// A fully-qualified ServerTLSPolicy URL reference. Specifies how TLS traffic is
		...
		// Immutable. The type of the customer managed gateway. Possible values:
