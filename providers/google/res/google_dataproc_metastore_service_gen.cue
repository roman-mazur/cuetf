...
google_dataproc_metastore_service: {
	...
	#hive_metastore_config: close({
		...
		// defaults to 'THRIFT'. Default value: "THRIFT" Possible values: ["THRIFT",
		// "GRPC"]
	...
	#maintenance_window: close({
		// The day of week, when the window starts. Possible values: ["MONDAY",
	...
	#scaling_config: close({
		...
		// Metastore instance sizes. Possible values: ["EXTRA_SMALL", "SMALL", "MEDIUM",
		// "LARGE", "EXTRA_LARGE"]
		instance_size?: string

		// Scaling factor, in increments of 0.1 for values less than 1.0, and increments
	...
	#scheduled_backup: close({
		...
		// The scheduled interval in Cron format, see https://en.wikipedia.org/wiki/Cron
		// The default is empty: scheduled backup is not enabled. Must be specified to
		...
		// Specifies the time zone to be used when interpreting cronSchedule. Must be a
		// time zone name from the time zone database
	...
	#telemetry_config: close({
		// The output format of the Dataproc Metastore service's logs. Default value:
		// "JSON" Possible values: ["LEGACY", "JSON"]
		log_format?: string
	})
	...
	_#defs: "/$defs/hive_metastore_config/$defs/auxiliary_versions": close({
		...
		// If keys are present in both the auxiliary version's overrides and the primary
		// version's overrides, the value from the auxiliary version's overrides takes
		// precedence.
		config_overrides?: [string]: string
		key!: string
	...
	_#defs: "/$defs/hive_metastore_config/$defs/kerberos_config": close({
		...
		// form gs://{bucket_name}/path/to/krb5.conf, although the file does not need
	...
	_#defs: "/$defs/network_config/$defs/consumers": close({
		...
		// and used as the Dataproc Metastore service's endpoint.
		// It is accessible to hosts in the subnet and to all hosts in a subnet in the
		// same region and same network.
		// There must be at least one IP address available in the subnet's primary
		// range. The subnet is specified in the following form:
