...
google_network_services_lb_edge_extension: {
	...
	close({
		...
		// At least one forwarding rule is required. Only one LbEdgeExtension resource
		// can be associated with a forwarding rule.
		...
		// balancing scheme. Possible values: ["EXTERNAL_MANAGED"]
	...
	_#defs: "/$defs/extension_chains/$defs/extensions": close({
		...
		// This field is required for the LbEdgeExtension resource and only supports the
