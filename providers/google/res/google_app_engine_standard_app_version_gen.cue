...
google_app_engine_standard_app_version: {
	...
	close({
		...
		// Defaults to F1 for AutomaticScaling and B2 for ManualScaling and
		// BasicScaling. If no scaling is specified, AutomaticScaling is chosen.
	...
	_#defs: "/$defs/automatic_scaling/$defs/standard_scheduler_settings": close({
		...
		//
		// **Note:** Starting from March 2025, App Engine sets the maxInstances default
		// for standard environment deployments to 20. This change doesn't impact
		// existing apps. To override the default, specify a new value between 0 and
		// 2147483647, and deploy a new version or redeploy over an existing version.
		// To disable the maxInstances default configuration setting, specify the
		...
		// Target CPU utilization ratio to maintain when scaling. Should be a value in
		// the range [0.50, 0.95], zero, or a negative value.
		target_cpu_utilization?: number

		// Target throughput utilization ratio to maintain when scaling. Should be a
		// value in the range [0.50, 0.95], zero, or a negative value.
	...
	_#defs: "/$defs/handlers/$defs/static_files": close({
		...
		// declared in static file handlers are uploaded as
		// static data and are only served to end users; they cannot be read by the
		// application. If enabled, uploads are charged
