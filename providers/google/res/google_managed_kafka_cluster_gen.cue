...
google_managed_kafka_cluster: {
	...
	close({
		...
		// is structured like: 'my-cluster-id'.
		cluster_id!: string
		...
		// characters, and numbers. Values must contain only hyphens (-), underscores (
	...
	#gcp_config: close({
		...
		// The Cloud KMS Key name to use for encryption. The key must be located in the
		// same region as the cluster and cannot be changed. Must be in the format
	...
	#rebalance_config: close({
		...
		// 'NO_REBALANCE'. Possible values: 'MODE_UNSPECIFIED', 'NO_REBALANCE',
	...
	#tls_config: close({
		...
		// principal names for Kafka ACLs. This field corresponds exactly to the
		// ssl.principal.mapping.rules broker config and matches the format and syntax
		// defined in the Apache Kafka documentation. Setting or modifying this field
		// will trigger a rolling restart of the Kafka brokers to apply the change. An
		// empty string means that the default Kafka behavior is used. Example:
	...
	_#defs: "/$defs/gcp_config/$defs/access_config/$defs/network_configs": close({
		// Name of the VPC subnet from which the cluster is accessible. Both broker and
		// bootstrap server IP addresses and DNS entries are automatically created in
		// the subnet. There can only be one subnet per network, and the subnet must be
		// located in the same region as the cluster. The project may differ. The name
	...
	_#defs: "/$defs/tls_config/$defs/trust_config/$defs/cas_configs": close({
		// The name of the CA pool to pull CA certificates from. The CA pool does not
		// need to be in the same project or location as the Kafka cluster. Must be in
		// the format 'projects/PROJECT_ID/locations/LOCATION/caPools/CA_POOL_ID.
		ca_pool!: string
	})
}
