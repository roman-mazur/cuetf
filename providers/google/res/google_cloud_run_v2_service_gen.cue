...
google_cloud_run_v2_service: {
	...
	close({
		...
		// diagnostics information in case the Service does not reach its Serving
		...
		// revision is active. Possible values: ["INGRESS_TRAFFIC_ALL",
		...
		// The generation of this Service currently serving traffic. See comments in
		// reconciling for additional information on reconciliation process in Cloud
		...
		//
		// When a new Service is created, or an existing one is updated, Cloud Run will
		...
		// matches the Service, or there was an error, and reconciliation failed. This
		// state can be found in terminalCondition.state.
		//
		...
		// be found in terminalCondition and conditions.
		reconciling?: bool
		project?:     string

		// The Condition of this Service, containing its readiness status, and detailed
		// error information in case it did not reach a serving state. See comments in
		// reconciling for additional information on reconciliation process in Cloud
		// Run.
		...
		// Detailed status information for corresponding traffic targets. See comments
	...
	#build_config: close({
		...
		// Cloud Run function. The format of this field is
	...
	#scaling: close({
		// Total instance count for the service in manual scaling mode. This number of
		// instances is divided among all revisions with specified traffic based on the
		// percent of traffic they are receiving.
	...
	#template: close({
		...
		// If not specified or 0, defaults to 80 when requested CPU >= 1 and defaults to
		// 1 when requested CPU < 1.
	...
	_#defs: "/$defs/template/$defs/containers": close({
		...
		// Base image for this container. If set, it indicates that the service is
		...
		// Containers which should be started before this container. If specified the
		// container will wait to start until all containers with the listed names are
		// healthy.
		depends_on?: [...string]
	...
	_#defs: "/$defs/template/$defs/containers/$defs/liveness_probe": close({
		...
		// startup probe is 240. Must be greater or equal than timeoutSeconds
		period_seconds?: number

		// Number of seconds after which the probe times out. Defaults to 1 second.
		// Minimum value is 1. Maximum value is 3600. Must be smaller than
		// periodSeconds. More info:
		// https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#container-probes
	...
	_#defs: "/$defs/template/$defs/containers/$defs/resources": close({
		// Determines whether CPU is only allocated during requests. True by default if
		// the parent 'resources' field is not set. However, if
	...
	_#defs: "/$defs/template/$defs/scaling": close({
		...
		// the region and specified instance size.
		max_instance_count?: number

		// Minimum number of serving instances that this resource should have. Defaults
		// to 0. Must not be greater than maximum instance count.
		min_instance_count?: number
	})
