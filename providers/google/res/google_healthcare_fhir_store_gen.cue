...
google_healthcare_fhir_store: {
	...
	close({
		...
		// this value set to ENABLED by default after a notification period. Warning:
		// turning on this flag causes processing existing resources to fail if they
		// contain references to non-existent resources. Possible values:
		...
		// If false, uses the FHIR specification default handling=lenient which ignores
		// unrecognized search parameters.
		// The handling can always be changed from the default on an individual API call
		...
		// immutable after FHIR store
		// creation. The default value is false, meaning that the API will enforce
		// referential integrity and fail the
		// requests that will result in inconsistent state in the FHIR store. When this
		// field is set to true, the API
		...
		// not be changed after the creation
		// of FHIR store. If set to false, which is the default behavior, all write
		// operations will cause historical
		// versions to be recorded automatically. The historical versions can be fetched
		// through the history APIs, but
		// cannot be updated. If set to true, no historical versions will be kept. The
		// server will send back errors for
		...
		// insert historical resource
		// versions into the FHIR store. Importing resource histories creates resource
		// interactions that appear to have
		// occurred in the past, which clients may not want to allow. If set to false,
		// history bundles within an import
		// will fail with an error.
		//
		// ** Changing this property may recreate the FHIR store (removing all data) **
		//
		// ** This property can be changed manually in the Google Cloud Healthcare admin
		// console without recreating the FHIR store **
		enable_history_import?: bool

		// Whether this FHIR store has the updateCreate capability. This determines if
		// the client can use an Update
		// operation to create a new resource with a client-specified ID. If false, all
		// IDs are server-assigned through
		// the Create operation and attempts to Update a non-existent resource will
		// return errors. Please treat the audit
		// logs with appropriate levels of care if client-specified resource IDs contain
		// sensitive data such as patient
		// identifiers, those IDs will be part of the FHIR resource path recorded in
		// Cloud audit logs and Cloud Pub/Sub
		// notifications.
		...
		// maximum 128 bytes, and must
		// conform to the following PCRE regular expression: [\p{Ll}\p{Lo}][\p{Ll}\p{Lo}\p{N}_-]{0,62}
		//
		// Label values are optional, must be between 1 and 63 characters long, have a
		// UTF-8 encoding of maximum 128
	...
	#notification_config: close({
		...
		// PubsubMessage.Data will contain the resource name. PubsubMessage.MessageId is
		// the ID of this message.
		// It is guaranteed to be unique within the topic. PubsubMessage.PublishTime is
		// the time at which the message
		// was published. Notifications are only sent if the topic is non-empty. Topic
		// names must be scoped to a
		// project. service-PROJECT_NUMBER@gcp-sa-healthcare.iam.gserviceaccount.com
		// must have publisher permissions on the given
		// Cloud Pub/Sub topic. Not having adequate permissions will cause the calls
	...
	#notification_configs: close({
		...
		// Note that setting this to true does not guarantee that all resources will be
		// sent in the format of
		// full FHIR resource. When a resource change is too large or during heavy
		// traffic, only the resource name will be
		...
		// resource. Note that setting this to
		// true does not guarantee that all previous resources will be sent in the
		// format of full FHIR resource. When a
		// resource change is too large or during heavy traffic, only the resource name
		// will be sent. Clients should always
		// check the "payloadType" label from a Pub/Sub message to determine whether it
		// needs to fetch the full previous
	...
	#stream_configs: close({
		...
		// resource types. The server treats
	...
	#validation_config: close({
		...
		// enabledImplementationGuides is empty or omitted, then incoming resources are
		// only required to conform to the base FHIR profiles. Otherwise, a resource
		// must conform to at least one profile listed in the global property of one of
		// the enabled ImplementationGuides.
	...
	_#defs: "/$defs/stream_configs/$defs/bigquery_destination/$defs/schema_config": close({
		...
		// example, concept in the CodeSystem
		// resource is a recursive structure; when the depth is 2, the CodeSystem table
		// will have a column called
		// concept.concept but not concept.concept.concept. If not specified or set to
		// 0, the server will use the default
		...
		// FHIR data being exported, with no additional simplification. Default value:
		// "ANALYTICS" Possible values: ["ANALYTICS", "ANALYTICS_V2", "LOSSLESS"]
		schema_type?: string
	})
	...
	_#defs: "/$defs/stream_configs/$defs/bigquery_destination/$defs/schema_config/$defs/last_updated_partition_config": close({
		...
		// Type of partitioning. Possible values: ["PARTITION_TYPE_UNSPECIFIED", "HOUR",
		// "DAY", "MONTH", "YEAR"]
		type!: string
	})
}
