...
google_workstations_workstation_config: {
	...
	close({
		...
		// action to restore full functionality. Details can be found in the conditions
		// field.
		degraded?: bool
		...
		// Disables support for plain TCP connections in the workstation. By default the
		// service supports TCP connections via a websocket relay. Setting this option
		// to true disables that relay, which prevents the usage of services that
		// require plain tcp connections, such as ssh. When enabled, all communication
		...
		// recently received any user traffic. A value of 0 indicates that this
		// instance should never time out from idleness. Defaults to 20 minutes.
		...
		// CreateWorkstation API calls on the user issuing the API request.
		...
		// region. If set, exactly two zones within the workstation cluster's region
		// must be specified—for example, '['us-central1-a', 'us-central1-f']'.
		// If this field is empty, two default zones within the region are used.
		// Immutable after the workstation configuration is created.
		replica_zones?: [...string]
		project?: string

		// How long to wait before automatically stopping a workstation after it was
		// started. A value of 0 indicates that workstations using this configuration
		// should never time out from running duration. Must be greater than 0 and less
	...
	#allowed_ports: close({
		// Starting port number for the current range of ports. Valid ports are 22, 80,
		// and ports within the range 1024-65535.
		first?: number

		// Ending port number for the current range of ports. Valid ports are 22, 80,
		// and ports within the range 1024-65535.
		last?: number
	})
	...
	_#defs: "/$defs/ephemeral_directories/$defs/gce_pd": close({
		...
		// Whether the disk is read only. If true, the disk may be shared by multiple
		// VMs and 'sourceSnapshot' must be set.
		read_only?: bool

		// Name of the disk image to use as the source for the disk.
		//
		// Must be empty 'sourceSnapshot' is set.
		// Updating 'sourceImage' will update content in the ephemeral directory after
	...
	_#defs: "/$defs/host/$defs/gce_instance": close({
		...
		// added based on feature usage. When specified, users of workstations under
		// this configuration must have 'iam.serviceAccounts.actAs' on the service
		// account.
	...
	_#defs: "/$defs/host/$defs/gce_instance/$defs/boost_configs": close({
		...
		// Workstations supports, see the list of available machine types
		// https://cloud.google.com/workstations/docs/available-machine-types. Defaults
		// to e2-standard-4.
		machine_type?: string
	...
	_#defs: "/$defs/persistent_directories/$defs/gce_hd": close({
		...
		// deleted. Possible values: ["DELETE", "RETAIN"]
	...
	_#defs: "/$defs/persistent_directories/$defs/gce_pd": close({
		...
		// image must support this file system type. Must be empty if 'sourceSnapshot'
		...
		// with this configuration. Must be empty if 'sourceSnapshot' is set.
		// Valid values are '10', '50', '100', '200', '500', or '1000'. Defaults to
		// '200'. If less than '200' GB, the 'diskType' must be 'pd-balanced' or
		// 'pd-ssd'.
		size_gb?: number

		// Name of the snapshot to use as the source for the disk. This can be the
		// snapshot's 'self_link', 'id', or a string in the format of
		// 'projects/{project}/global/snapshots/{snapshot}'. If set, 'sizeGb' and
		// 'fsType' must be empty. Can only be updated if it has an existing value.
		source_snapshot?: string
	})
}
