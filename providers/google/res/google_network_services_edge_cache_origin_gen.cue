...
google_network_services_edge_cache_origin: {
	...
	close({
		...
		// attempt is made when a cache fill fails with one of the retryConditions.
		//
		// Once maxAttempts to this origin have failed the failoverOrigin will be used,
		// if one is specified. That failoverOrigin may specify its own maxAttempts,
		// retryConditions and failoverOrigin to control its own cache fill failures.
		//
		// The total number of allowed attempts to cache fill across this and failover
		// origins is limited to four.
		// The total time allowed for cache fill attempts across this and failover
		...
		//
		// This address will be used as the origin for cache requests - e.g. FQDN:
		// media-backend.example.com, IPv4: 35.218.1.1, IPv6: 2607:f8b0:4012:809::200e,
		// Cloud Storage: gs://bucketname
		//
		// When providing an FQDN (hostname), it must be publicly resolvable (e.g. via
		// Google public DNS) and IP addresses must be publicly routable. It must not
		// contain a protocol (e.g., https://) and it must not contain any slashes.
		// If a Cloud Storage bucket is provided, it must be in the canonical
		...
		// The protocol to use to connect to the configured origin. Defaults to HTTP2,
		...
		// configured retryCondition(s),
		// the origin request will be retried up to maxAttempts times. The
		// failoverOrigin, if configured, will then be used to satisfy the request.
		//
		...
		// - HTTP_5XX: Retry if the origin responds with any 5xx response code, or if
		...
		// - RETRIABLE_4XX: Retry for retriable 4xx response codes, which include HTTP
		// 409 (Conflict) and HTTP 429 (Too Many Requests)
		// - NOT_FOUND: Retry if the origin returns a HTTP 404 (Not Found). This can be
		// useful when generating video content, and the segment is not available yet.
		// - FORBIDDEN: Retry if the origin returns a HTTP 403 (Forbidden). Possible
		// values: ["CONNECT_FAILURE", "HTTP_5XX", "GATEWAY_ERROR", "RETRIABLE_4XX",
		// "NOT_FOUND", "FORBIDDEN"]
	...
	#aws_v4_authentication: close({
		...
		// project, secret, and version you require.
	...
	#flex_shielding: close({
		...
		//
		// You must specify exactly one FlexShieldingRegion. Possible values:
		// ["AFRICA_SOUTH1", "ME_CENTRAL1"]
	...
	#timeout: close({
		...
		// Defaults to 15 seconds. The timeout must be a value between 1s and 30s.
		//
		// The readTimeout is capped by the responseTimeout. All reads of the HTTP
		// connection/stream must be completed by the deadline set by the
		// responseTimeout.
		//
		// If the response headers have already been written to the connection, the
		// response will be truncated and logged.
		read_timeout?: string

		// The maximum duration to wait for the last byte of a response to arrive when
		// reading from the HTTP connection/stream.
		//
		// Defaults to 30 seconds. The timeout must be a value between 1s and 120s.
		//
		// The responseTimeout starts after the connection has been established.
		//
		// This also applies to HTTP Chunked Transfer Encoding responses, and/or when an
		// open-ended Range request is made to the origin. Origins that take longer to
		// write additional bytes to the response than the configured responseTimeout
		// will result in an error being returned to the client.
		//
		// If the response headers have already been written to the connection, the
		// response will be truncated and logged.
