...
google_container_aws_cluster: {
	...
	close({
		...
		// The AWS region where the cluster runs. Each Google Cloud region supports a
		// subset of nearby AWS regions. You can call to list all supported AWS regions
		// within a given Google Cloud region.
		aws_region!: string
	...
	#binary_authorization: close({
		// Mode of operation for Binary Authorization policy evaluation. Possible
	...
	#control_plane: close({
		...
		// The list of subnets where control plane replicas will run. A replica will be
		// provisioned on each subnet and up to three values can be provided. Each
		// subnet must be in a different AWS Availability Zone (AZ).
		subnet_ids!: [...string]

		// Optional. A set of AWS resource tags to propagate to all underlying managed
		// AWS resources. Specify at most 50 pairs containing alphanumerics, spaces,
		// and symbols (.+-=_:@/). Keys can be up to 127 Unicode characters. Values can
	...
	#networking: close({
		// Disable the per node pool subnet security group rules on the control plane
		// security group. When set to true, you must also provide one or more security
		// groups that ensure node pools are able to send requests to the control plane
		// on TCP/443 and TCP/8132. Failure to do so may result in unavailable node
		// pools.
		per_node_pool_sg_rules_disabled?: bool

		// All pods in the cluster are assigned an RFC1918 IPv4 address from these
		// ranges. Only a single range is supported. This field cannot be changed after
		// creation.
		...
		// The VPC associated with the cluster. All component clusters (i.e. control
		// plane and node pools) run on a single VPC. This field cannot be changed
		// after creation.
		vpc_id!: string
	})
	...
	_#defs: "/$defs/control_plane/$defs/aws_services_authentication": close({
		// The Amazon Resource Name (ARN) of the role that the Anthos Multi-Cloud API
		// will assume when managing AWS resources on your account.
		role_arn!: string

		// Optional. An identifier for the assumed role session. When unspecified, it
	...
	_#defs: "/$defs/control_plane/$defs/main_volume": close({
		...
		// used to encrypt AWS EBS volumes. If not specified, the default Amazon
		// managed key associated to the AWS region where this cluster runs will be
		// used.
		kms_key_arn?: string

		// Optional. The size of the volume, in GiBs. When unspecified, a default value
		// is provided. See the specific reference in the parent resource.
		size_gib?: number

		// Optional. The throughput to provision for the volume, in MiB/s. Only valid if
		// the volume type is GP3. If volume type is gp3 and throughput is not
		// specified, the throughput will defaults to 125.
		throughput?: number

		// Optional. Type of the EBS volume. When unspecified, it defaults to GP2
		// volume. Possible values: VOLUME_TYPE_UNSPECIFIED, GP2, GP3
		volume_type?: string
	})
	...
	_#defs: "/$defs/control_plane/$defs/proxy_config": close({
		...
		// The version string of the AWS Secret Manager secret that contains the HTTP(S)
		// proxy configuration.
