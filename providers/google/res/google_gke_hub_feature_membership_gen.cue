...
google_gke_hub_feature_membership: {
	...
	_#defs: "/$defs/configmanagement/$defs/config_sync": close({
		...
		// Service Account is no longer needed for exporting Config Sync metrics:
	...
	_#defs: "/$defs/configmanagement/$defs/config_sync/$defs/oci": close({
		...
		// Type of secret configured for access to the OCI Image. Must be one of
		// gcenode, gcpserviceaccount or none. The validation of this is
		// case-sensitive.
		secret_type?: string
	...
	_#defs: "/$defs/configmanagement/$defs/policy_controller/$defs/monitoring": close({
		// Specifies the list of backends Policy Controller will export to. Specifying
		// an empty value `[]` disables metrics export.
		backends?: [...string]
	})
	...
	_#defs: "/$defs/policycontroller/$defs/policy_controller_hub_config": close({
		...
		// INSTALL_SPEC_UNSPECIFIED, INSTALL_SPEC_NOT_INSTALLED, INSTALL_SPEC_ENABLED,
	...
	_#defs: "/$defs/policycontroller/$defs/policy_controller_hub_config/$defs/policy_content/$defs/template_library": close({
		...
		// cluster. Possible values: INSTALLATION_UNSPECIFIED, NOT_INSTALLED, ALL
		installation?: string
	})
}
