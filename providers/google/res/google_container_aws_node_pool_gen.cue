...
google_container_aws_node_pool: {
	...
	close({
		...
		// Optional. Annotations on the node pool. This field has the same restrictions
		...
		// Output only. The lifecycle state of the node pool. Possible values:
	...
	#config: close({
		...
		// Optional. The IDs of additional security groups to add to nodes in this pool.
		// The manager will automatically create security groups with minimum rules
	...
	#kubelet_config: close({
		...
		// Optional. The maximum number of PIDs in each pod running on the node. The
		// limit scales automatically based on underlying machine size if left unset.
	...
	_#defs: "/$defs/config/$defs/autoscaling_metrics_collection": close({
		// The frequency at which EC2 Auto Scaling sends aggregated data to AWS
	...
	_#defs: "/$defs/config/$defs/taints": close({
		// The taint effect. Possible values: EFFECT_UNSPECIFIED, NO_SCHEDULE,
		// PREFER_NO_SCHEDULE, NO_EXECUTE
		effect!: string
	...
	_#defs: "/$defs/update_settings/$defs/surge_settings": close({
		// Optional. The maximum number of nodes that can be created beyond the current
		// size of the node pool during the update process.
		max_surge?: number

		// Optional. The maximum number of nodes that can be simultaneously unavailable
		// during the update process. A node is considered unavailable if its status is
		// not Ready.
