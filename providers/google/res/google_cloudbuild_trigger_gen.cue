...
google_cloudbuild_trigger: {
	...
	close({
		...
		// Either a filename or build template must be provided. Set this only when
		// using trigger_template or github.
	...
	#bitbucket_server_trigger_config: close({
		...
		// Key of the project that the repo is in. For example: The key for
		// https://mybitbucket.server/projects/TEST/repos/test-repo is "TEST".
		project_key!: string

		// Slug of the repository. A repository slug is a URL-friendly version of a
		// repository name, automatically generated by Bitbucket for use in the URL.
		// For example, if the repository name is 'test repo', in the URL it would
		// become 'test-repo' as in
	...
	#git_file_source: close({
		...
		// Possible values: ["UNKNOWN", "CLOUD_SOURCE_REPOSITORIES", "GITHUB",
		// "BITBUCKET_SERVER"]
		repo_type!: string

		// The fully qualified resource name of the Repo API repository. The fully
		// qualified resource name of the Repo API repository.
		...
		// filename (optional). This field respects the same syntax/resolution as
		// described here: https://git-scm.com/docs/gitrevisions
	...
	_#defs: "/$defs/bitbucket_server_trigger_config/$defs/pull_request": close({
		...
		// comment /gcbrun. Possible values: ["COMMENTS_DISABLED", "COMMENTS_ENABLED",
	...
	_#defs: "/$defs/build/$defs/artifacts/$defs/maven_artifacts": close({
		...
		//
		// Artifact in the workspace specified by path will be uploaded to Artifact
	...
	_#defs: "/$defs/build/$defs/artifacts/$defs/npm_packages": close({
		...
		//
		// Npm package in the workspace specified by path will be zipped and uploaded to
	...
	_#defs: "/$defs/build/$defs/artifacts/$defs/python_packages": close({
		...
		//
		// Files in the workspace matching any path pattern will be uploaded to Artifact
		// Registry with this location as a prefix.
		repository?: string
	})
	...
	_#defs: "/$defs/build/$defs/options": close({
		...
		//
		// NOTE this is always enabled for triggered builds and cannot be overridden in
		...
		// Possible values: ["STREAM_DEFAULT", "STREAM_ON", "STREAM_OFF"]
		log_streaming_option?: string

		// Option to specify the logging mode, which determines if and where build logs
		// are stored. Possible values: ["LOGGING_UNSPECIFIED", "LEGACY", "GCS_ONLY",
	...
	_#defs: "/$defs/build/$defs/source/$defs/repo_source": close({
		// Regex matching branches to build. Exactly one a of branch name, tag, or
	...
	_#defs: "/$defs/developer_connect_event_config/$defs/pull_request": close({
		...
		// Configure builds to run whether a repository owner or collaborator need to
		// comment '/gcbrun'. Possible values: ["COMMENTS_DISABLED",
	...
	_#defs: "/$defs/github/$defs/pull_request": close({
		...
		// collaborator. Possible values: ["COMMENTS_DISABLED", "COMMENTS_ENABLED",
		// "COMMENTS_ENABLED_FOR_EXTERNAL_CONTRIBUTORS_ONLY"]
		comment_control?: string
