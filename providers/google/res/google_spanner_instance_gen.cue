...
google_spanner_instance: {
	...
	close({
		...
		// if unset or NONE, no default backup schedule will be created for new
		// databases within the instance. Possible values: ["NONE", "AUTOMATIC"]
		...
		// The edition selected for this instance. Different editions provide different
		// capabilities at different price points. Possible values:
		...
		// variants, that can affect aspects like:
		// usage restrictions, quotas and billing. Currently this is used to distinguish
		// FREE_INSTANCE vs PROVISIONED instances.
		// When configured as FREE_INSTANCE, the field 'edition' should not be
		// configured. Possible values: ["PROVISIONED", "FREE_INSTANCE"]
		...
		// The number of nodes allocated to this instance. Exactly one of either
		// num_nodes, processing_units or
		...
		// processing_units or autoscaling_config must be present in terraform except
		// when instance_type = FREE_INSTANCE.
	...
	_#defs: "/$defs/autoscaling_config/$defs/autoscaling_targets": close({
		...
		// This number is on a scale from 0 (no utilization) to 100 (full utilization).
		// The valid range is [10, 90] inclusive.
