...
google_dialogflow_cx_intent: {
	...
	close({
		...
		// for an agent. When you create an agent, a Default Negative Intent is created
		// automatically.
		// The Default Negative Intent cannot be deleted; deleting the
		// 'google_dialogflow_cx_intent' resource does nothing to the underlying GCP
		// resources.
		//
		// ~> Avoid having multiple 'google_dialogflow_cx_intent' resources linked to
		// the same agent with 'is_default_negative_intent = true' because they will
		// compete to control a single Default Negative Intent resource in GCP.
		...
		// for an agent. When you create an agent, a Default Welcome Intent is created
		// automatically.
		...
		//
		// ~> Avoid having multiple 'google_dialogflow_cx_intent' resources linked to
		// the same agent with 'is_default_welcome_intent = true' because they will
		// compete to control a single Default Welcome Intent resource in GCP.
		is_default_welcome_intent?: bool

		// Indicates whether this is a fallback intent. Currently only default fallback
		// intent is allowed in the agent, which is added upon agent creation.
		// Adding training phrases to fallback intent is useful in the case of requests
		// that are mistakenly matched, since training phrases assigned to fallback
		// intents act as negative examples that triggers no-match event.
		...
		// Dialogflow defined labels include: * sys-head * sys-contextual The above
		// labels do not require value. "sys-head" means the intent is a head intent.
		...
		// to 500,000, which corresponds to the Normal priority in the console.
	...
	#parameters: close({
		...
		// The unique identifier of the parameter. This field is used by training
		// phrases to annotate their parts.
		id!: string
		...
		// name during logging.
		// Note: the parameter content is subject to redaction if either parameter level
