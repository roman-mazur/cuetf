...
google_compute_attached_disk: {
	...
	close({
		...
		// self link. If only the name is used then zone and project must be defined as
		// properties on the resource or provider.
		instance!: string

		// The disk interface used for attaching this disk. One of SCSI or NVME. (This
		// field is only used for specific cases, please don't specify this field
		// without advice from Google.)
		interface?: string
		...
		// The project that the referenced compute instance is a part of. If instance is
		// referenced by its self_link the project defined in the link will take
		// precedence.
		project?: string

		// The zone that the referenced compute instance is located within. If instance
		// is referenced by its self_link the zone defined in the link will take
		// precedence.
		zone?: string
	})
