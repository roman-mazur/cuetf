...
google_access_context_manager_service_perimeter_egress_policy: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/google_access_context_manager_service_perimeter_egress_policy")
	close({
		...
		// The perimeter etag is internally used to prevent overwriting the list of
		// policies on PATCH calls. It is retrieved from the same GET perimeter API
		// call that's used to get the current list of policies. The policy defined in
		// this resource is added or removed from that list, and then this etag is sent
		// with the PATCH call along with the updated policies.
		etag?: string
		id?:   string
	...
	_#defs: "/$defs/egress_from/$defs/sources": close({
		psc_endpoint?: matchN(1, [_#defs."/$defs/egress_from/$defs/sources/$defs/psc_endpoint", list.MaxItems(1) & [..._#defs."/$defs/egress_from/$defs/sources/$defs/psc_endpoint"]])
	...
	_#defs: "/$defs/egress_from/$defs/sources/$defs/psc_endpoint": close({
