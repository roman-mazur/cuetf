...
google_netapp_storage_pool: {
	...
	close({
		...
		// Flag indicating that the hot-tier threshold will be auto-increased by 10% of
		// the hot-tier when it hits 100%. Default is true.
		// The increment will kick in only if the new size after increment is still less
		...
		// It should be less than the minimum storage pool size and cannot be more than
		// the current storage pool size. It cannot be decreased once set.
		...
		// When enabled, the volumes uses Active Directory as LDAP name service for
		// UID/GID lookups. Required to enable extended group support for NFSv3,
		...
		// If not specified during creation, the mode defaults to DEFAULT. Possible
		// values: ["MODE_UNSPECIFIED", "DEFAULT", "ONTAP"]
		mode?: string
		...
		// Specifies the replica zone for regional Flex pools. 'zone' and 'replica_zone'
		// values can be swapped to initiate a
		// [zone
		...
		// The scale type of the storage pool. Defaults to 'SCALE_TYPE_DEFAULT' if not
		// specified. Possible values: ["SCALE_TYPE_UNSPECIFIED", "SCALE_TYPE_DEFAULT",
		// "SCALE_TYPE_SCALEOUT"]
		scale_type?: string
		...
		// be calculated based on the totalThroughputMibps
		total_iops?: string
		...
		// This field is used to control whether the pool supports FILE based volumes
		// only or UNIFIED (both FILE and BLOCK) volumes.
		// If not specified during creation, it defaults to FILE. Possible values:
		// ["STORAGE_POOL_TYPE_UNSPECIFIED", "FILE", "UNIFIED"]
		type?: string
		...
		// Specifies the active zone for regional Flex pools. 'zone' and 'replica_zone'
		// values can be swapped to initiate a
		// [zone
		// switch](https://cloud.google.com/netapp/volumes/docs/configure-and-use/storage-pools/edit-or-delete-storage-pool#switch_active_and_replica_zones).
