...
google_app_engine_domain_mapping: {
	...
	close({
		...
		// By default, overrides are rejected. Default value: "STRICT" Possible values:
		// ["STRICT", "OVERRIDE"]
	...
	#ssl_settings: close({
		...
		// remove SSL support.
		// By default, a managed certificate is automatically created for every domain
		// mapping. To omit SSL support
		// or to configure SSL manually, specify 'SslManagementType.MANUAL' on a
		// 'CREATE' or 'UPDATE' request. You must be
		// authorized to administer the 'AuthorizedCertificate' resource to manually map
		// it to a DomainMapping resource.
		// Example: 12345.
		certificate_id?: string

		// ID of the managed 'AuthorizedCertificate' resource currently being
		// provisioned, if applicable. Until the new
		// managed certificate has been successfully provisioned, the previous SSL state
		// will be preserved. Once the
		// provisioning process completes, the 'certificateId' field will reflect the
		// new managed certificate and this
		// field will be left empty. To remove SSL support while there is still a
		// pending managed certificate, clear the
		...
		// If 'MANUAL', 'certificateId' must be manually specified in order to configure
