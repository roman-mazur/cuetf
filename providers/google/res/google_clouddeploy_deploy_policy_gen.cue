...
google_clouddeploy_deploy_policy: {
	...
	_#defs: "/$defs/rules/$defs/rollout_restriction": close({
		...
		// actions will be restricted. Possible values: ["ADVANCE", "APPROVE",
		// "CANCEL", "CREATE", "IGNORE_JOB", "RETRY_JOB", "ROLLBACK",
		// "TERMINATE_JOBRUN"]
		actions?: [...string]

		// ID of the rule. This id must be unique in the 'DeployPolicy' resource to
		// which this rule belongs. The format is 'a-z{0,62}'.
		id!: string

		// What invoked the action. If left empty, all invoker types will be restricted.
		// Possible values: ["USER", "DEPLOY_AUTOMATION"]
		invokers?: [...string]
	})
	...
	_#defs: "/$defs/rules/$defs/rollout_restriction/$defs/time_windows/$defs/one_time_windows/$defs/end_time": close({
		...
		// less than or equal to 59. An API may allow the value 60 if it allows
		// leap-seconds.
		seconds?: number
	})
	...
	_#defs: "/$defs/rules/$defs/rollout_restriction/$defs/time_windows/$defs/weekly_windows": close({
		...
		// Days of week. If left empty, all days of the week will be included. Possible
