...
google_data_loss_prevention_job_trigger: {
	...
	close({
		...
		// Whether the trigger is currently active. Default value: "HEALTHY" Possible
		// values: ["PAUSED", "HEALTHY", "CANCELLED"]
		status?: string
	...
	_#defs: "/$defs/inspect_job/$defs/actions/$defs/deidentify": close({
		...
		// files with these filetypes will be transformed.
		//
		// If empty, all supported files will be transformed. Supported types may be
		// automatically added over time.
		//
		// If a file type is set in this field that isn't supported by the Deidentify
		// action then the job will fail and will not be successfully created/started.
		// Possible values: ["IMAGE", "TEXT_FILE", "CSV", "TSV"]
	...
	_#defs: "/$defs/inspect_job/$defs/actions/$defs/deidentify/$defs/transformation_config": close({
		...
		// If this template is specified, it will serve as the de-identify template for
		// structured content such as delimited files and tables.
	...
	_#defs: "/$defs/inspect_job/$defs/actions/$defs/save_findings/$defs/output_config": close({
		...
		// Only for use with external storage. Possible values: ["BASIC_COLUMNS",
		// "GCS_COLUMNS", "DATASTORE_COLUMNS", "BIG_QUERY_COLUMNS", "ALL_COLUMNS"]
	...
	_#defs: "/$defs/inspect_job/$defs/inspect_config": close({
		...
		// https://cloud.google.com/dlp/docs/likelihood for more info Default value:
		// "POSSIBLE" Possible values: ["VERY_UNLIKELY", "UNLIKELY", "POSSIBLE",
		// "LIKELY", "VERY_LIKELY"]
	...
	_#defs: "/$defs/inspect_job/$defs/inspect_config/$defs/custom_info_types": close({
		...
		// returned. It still can be used for rules matching. Possible values:
		// ["EXCLUSION_TYPE_EXCLUDE"]
		exclusion_type?: string

		// Likelihood to return for this CustomInfoType. This base value can be altered
		// by a detection rule if the finding meets the criteria
		// specified by the rule. Default value: "VERY_LIKELY" Possible values:
	...
	_#defs: "/$defs/inspect_job/$defs/inspect_config/$defs/custom_info_types/$defs/info_type": close({
		...
		// CustomInfoType, or one of the names
	...
	_#defs: "/$defs/inspect_job/$defs/inspect_config/$defs/rule_set/$defs/rules/$defs/exclusion_rule/$defs/exclude_by_hotword/$defs/proximity": close({
		// Number of characters after the finding to consider. Either this or
		// window_before must be specified
		window_after?: number

		// Number of characters before the finding to consider. Either this or
		// window_after must be specified
	...
	_#defs: "/$defs/inspect_job/$defs/storage_config/$defs/big_query_options": close({
		...
		// If not set, or if set to 0, all rows will be scanned. Only one of rowsLimit
		// and rowsLimitPercent can be
		// specified. Cannot be used in conjunction with TimespanConfig.
		rows_limit?: number

		// Max percentage of rows to scan. The rest are omitted. The number of rows scanned is rounded down.
		// Must be between 0 and 100, inclusively. Both 0 and 100 means no limit. Defaults to 0. Only one of
		// rowsLimit and rowsLimitPercent can be specified. Cannot be used in
		// conjunction with TimespanConfig.
		rows_limit_percent?: number

		// How to sample rows if not all rows are scanned. Meaningful only when used in
		// conjunction with either
		// rowsLimit or rowsLimitPercent. If not specified, rows are scanned in the
		// order BigQuery reads them.
		// If TimespanConfig is set, set this to an empty string to avoid using the
		// default value. Default value: "TOP" Possible values: ["TOP", "RANDOM_START"]
	...
	_#defs: "/$defs/inspect_job/$defs/storage_config/$defs/cloud_storage_options": close({
		...
		// of bytes scanned is rounded down.
		// Must be between 0 and 100, inclusively. Both 0 and 100 means no limit.
		bytes_limit_per_file_percent?: number

		// List of file type groups to include in the scan. If empty, all files are
		// scanned and available data
		// format processors are applied. In addition, the binary content of the
		// selected files is always scanned as well.
		// Images are scanned only as binary if the specified region does not support
		// image inspection and no fileTypes were specified. Possible values:
		// ["BINARY_FILE", "TEXT_FILE", "IMAGE", "WORD", "PDF", "AVRO", "CSV", "TSV",
		// "POWERPOINT", "EXCEL"]
		file_types?: [...string]
		...
		// in conjunction with bytesLimitPerFile.
	...
	_#defs: "/$defs/inspect_job/$defs/storage_config/$defs/cloud_storage_options/$defs/file_set": close({
		...
		//
		// If the url ends in a trailing slash, the bucket or directory represented by
		// the url will be scanned
		// non-recursively (content in sub-directories will not be scanned). This means
		// that 'gs://mybucket/' is
		// equivalent to 'gs://mybucket/*', and 'gs://mybucket/directory/' is equivalent
	...
	_#defs: "/$defs/inspect_job/$defs/storage_config/$defs/timespan_config/$defs/timestamp_field": close({
		...
		// for data sources like Datastore and BigQuery.
		//
		// For BigQuery: Required to filter out rows based on the given start and end
		// times. If not specified and the table was
		// modified between the given start and end times, the entire table will be
		// scanned. The valid data types of the timestamp
		// field are: INTEGER, DATE, TIMESTAMP, or DATETIME BigQuery column.
		//
		// For Datastore. Valid data types of the timestamp field are: TIMESTAMP.
		// Datastore entity will be scanned if the
	...
	_#defs: "/$defs/triggers/$defs/schedule": close({
		...
		//
		// A scheduled start time will be skipped if the previous execution has not
		// ended when its scheduled time occurs.
		//
		// This value must be set to a time duration greater than or equal to 1 day and
