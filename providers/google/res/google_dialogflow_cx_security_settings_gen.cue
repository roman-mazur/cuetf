...
google_dialogflow_cx_security_settings: {
	...
	close({
		...
		// empty, Dialogflow replaces sensitive info with [redacted] text.
		...
		// Format: projects/<Project ID>/locations/<Location
		// ID>/inspectTemplates/<Template ID> OR organizations/<Organization
		// ID>/locations/<Location ID>/inspectTemplates/<Template ID>
		inspect_template?: string

		// The location these settings are located in. Settings can only be applied to
		// an agent in the same location.
		// See [Available
		...
		// Possible values: ["DIALOGFLOW_HISTORY"]
		purge_data_types?: [...string]
		project?: string

		// Defines what types of data to redact. If not set, defaults to not redacting any kind of data.
		// * REDACT_DISK_STORAGE: On data to be written to disk or similar devices that
		// are capable of holding data even if power is disconnected. This includes
		// data that are temporarily saved on disk. Possible values:
		// ["REDACT_DISK_STORAGE"]
		...
		// persisted. Possible values: ["REDACT_WITH_SERVICE"]
		redaction_strategy?: string

		// Defines how long we retain persisted data that contains sensitive info. Only
		// one of 'retention_window_days' and 'retention_strategy' may be set.
		...
		// when the corresponding Dialogflow session ends. Possible values:
