...
google_clouddeploy_automation: {
	...
	_#defs: "/$defs/rules/$defs/repair_rollout_rule": close({
		...
		// list, or for all jobs if unspecified or empty. The phase that includes the
		// job must match the phase ID specified in sourcePhase. This value must
	...
	_#defs: "/$defs/rules/$defs/repair_rollout_rule/$defs/repair_phases/$defs/retry": close({
		...
		// Backoff mode will be ignored if wait is 0. Possible values:
		// ["BACKOFF_MODE_UNSPECIFIED", "BACKOFF_MODE_LINEAR",
		// "BACKOFF_MODE_EXPONENTIAL"]
		backoff_mode?: string

		// Optional. How long to wait for the first retry. Default is 0, and the maximum
		// value is 14d. A duration in seconds with up to nine fractional digits,
		// ending with 's'. Example: '3.5s'.
		wait?: string
	})

	_#defs: "/$defs/rules/$defs/repair_rollout_rule/$defs/repair_phases/$defs/rollback": close({
		// Optional. The starting phase ID for the Rollout. If unspecified, the Rollout
