...
google_os_config_patch_deployment: {
	...
	#patch_config: close({
		...
		// 'SKIPPED'. An instance cannot be patched for two reasons:
		...
		// ('mig_instances_allowed' is false).
		// 2. The patch job is reported as 'SUCCEEDED' if it completes without errors,
		// even if some instances are 'SKIPPED'.
		// 3. The patch job is reported as 'COMPLETED_WITH_INACTIVE_VMS' if it completes
		// without errors, but does not patch instances that are 'INACTIVE'.
	...
	#recurring_schedule: close({
		...
		// The time that the recurring schedule becomes effective. Defaults to
		// createTime of the patch deployment.
		// A timestamp in RFC3339 UTC "Zulu" format, accurate to nanoseconds. Example:
		// "2014-10-02T15:01:23.045123456Z".
		start_time?: string
	})
	...
	_#defs: "/$defs/patch_config/$defs/apt": close({
		...
		// dist-upgrade instead. Possible values: ["DIST", "UPGRADE"]
		type?: string
	})
	...
	_#defs: "/$defs/patch_config/$defs/post_step/$defs/linux_exec_step_config": close({
		...
		// be executed directly, which will likely only succeed for scripts with shebang
		// lines. Possible values: ["SHELL", "POWERSHELL"]
		interpreter?: string
	...
	_#defs: "/$defs/patch_config/$defs/post_step/$defs/linux_exec_step_config/$defs/gcs_object": close({
		...
		// Generation number of the Cloud Storage object. This is used to ensure that
	...
	_#defs: "/$defs/patch_config/$defs/windows_update": close({
		...
		// updates are applied. Possible values: ["CRITICAL", "SECURITY", "DEFINITION",
		// "DRIVER", "FEATURE_PACK", "SERVICE_PACK", "TOOL", "UPDATE_ROLLUP", "UPDATE"]
	...
	_#defs: "/$defs/patch_config/$defs/zypper": close({
		...
		// will be installed using 'zypper patch patch:' command.
	...
	_#defs: "/$defs/recurring_schedule/$defs/monthly": close({
		...
		// One day of the month. 1-31 indicates the 1st to the 31st day. -1 indicates
		// the last day of the month.
		// Months without the target day will be skipped. For example, a schedule to run
	...
	_#defs: "/$defs/recurring_schedule/$defs/weekly": close({
		// IANA Time Zone Database time zone, e.g. "America/New_York". Possible values:
