...
google_sql_database_instance: {
	...
	#clone: close({
		...
		// (SQL Server only, use with point_in_time) clone only the specified databases
		...
		// specified zone. If no zone is specified, clone to the same zone as the
		// source instance.
	...
	#point_in_time_restore_context: close({
		...
		// For example: "google-managed-services-default". If you set this, then Cloud
		// SQL creates the IP address for the cloned instance in the allocated range.
		// This range must comply with [RFC 1035](https://tools.ietf.org/html/rfc1035)
		// standards. Specifically, the name must be 1-63 characters long and match the
		// regular expression [a-z]([-a-z0-9]*[a-z0-9])?.
		allocated_ip_range?: string

		// The Google Cloud Backup and Disaster Recovery Datasource URI. For example:
		...
		// Point-in-time recovery of an instance to the specified zone. If no zone is
		// specified, then clone to the same primary zone as the source instance.
		preferred_zone?: string
	...
	#replica_configuration: close({
		...
		// Specifies if a SQL Server replica is a cascadable replica. A cascadable
		// replica is a SQL Server cross region replica that supports replica(s) under
		// it.
		...
		// in encoded in the client_certificate.
		client_key?: string

		// The number of seconds between connect retries. MySQL's default is 60 seconds.
		connect_retry_interval?: number

		// Path to a SQL file in Google Cloud Storage from which replica instances are
		// created. Format is gs://bucket/filename.
		dump_file_path?: string

		// Specifies if the replica is the failover target. If the field is set to true
		// the replica will be designated as a failover replica. If the master instance
		// fails, the replica instance will be promoted as the new master instance. Not
	...
	#replication_cluster: close({
		...
		// disaster recovery (DR) replica. The standard format of this field is
		// "your-project:your-instance". You can also set this field to
		// "your-instance", but cloud SQL backend will convert it to the aforementioned
		// standard format.
		failover_dr_replica_name?: string

		// If set, this field indicates this instance has a private service access (PSA)
		// DNS endpoint that is pointing to the primary instance of the cluster. If
		// this instance is the primary, then the DNS endpoint points to this instance.
		// After a switchover or replica failover operation, this DNS endpoint points
		// to the promoted instance. This is a read-only field, returned to the user as
		// information. This field can exist even if a standalone instance doesn't have
		// a DR replica yet or the DR replica is deleted.
	...
	#settings: close({
		...
		// Enables the enforcement of Cloud SQL Auth Proxy or Cloud SQL connectors for
		// all the connections. If enabled, all the direct connections are rejected.
		...
		// Provisioned number of I/O operations per second for the data disk. This field
		// is only used for HYPERDISK_BALANCED disk types.
		data_disk_provisioned_iops?: number

		// Provisioned throughput measured in MiB per second for the data disk. This
		// field is only used for HYPERDISK_BALANCED disk types.
		...
		// The maximum size, in GB, to which storage capacity can be automatically
		...
		// be retained based on the backups retention setting.
		...
		// The time_zone to be used by the database engine (supported only for SQL
		// Server), in SQL Server timezone format.
		time_zone?: string
	...
	_#defs: "/$defs/settings/$defs/active_directory_config": close({
		// The secret manager key storing the administrator credential. (e.g.,
		...
		// The organizational unit distinguished name. This is the full hierarchical
		// path to the organizational unit.
	...
	_#defs: "/$defs/settings/$defs/backup_configuration": close({
		...
		// True if binary logging is enabled. If settings.backup_configuration.enabled
		...
		// from 1-7. (For PostgreSQL Enterprise Plus instances, from 1 to 35.)
	...
	_#defs: "/$defs/settings/$defs/deny_maintenance_period": close({
		// End date before which maintenance will not take place. The date is in format
		// yyyy-mm-dd i.e., 2020-11-01, or mm-dd, i.e., 11-01
		end_date!: string

		// Start date after which maintenance will not take place. The date is in format
		// yyyy-mm-dd i.e., 2020-11-01, or mm-dd, i.e., 11-01
		start_date!: string
	...
	_#defs: "/$defs/settings/$defs/final_backup_config": close({
		...
		// The valid range is between 1 and 365. For instances managed by BackupDR, the
		// valid range is between 1 day and 99 years. The final backup will be purged
	...
	_#defs: "/$defs/settings/$defs/insights_config": close({
		...
		// queries combined. Between 0 and 20. Default to 5. For Enterprise Plus
		// instances, from 0 to 200.
		query_plans_per_minute?: number

		// Maximum query length stored in bytes. Between 256 and 4500. Default to 1024.
		// For Enterprise Plus instances, from 1024 to 100000.
	...
	_#defs: "/$defs/settings/$defs/ip_configuration": close({
		...
		// Whether Google Cloud services such as BigQuery are allowed to access data in
		// this Cloud SQL instance over a private IP connection. SQLSERVER database
		// type is not supported.
		enable_private_path_for_google_cloud_services?: bool

		// Whether this Cloud SQL instance should be assigned a public IPV4 address. At
		// least ipv4_enabled must be enabled or a private_network must be configured.
		ipv4_enabled?: bool

		// The VPC network from which the Cloud SQL instance is accessible for private
		// IP. For example, projects/myProject/global/networks/default. Specifying a
		// network enables private IP. At least ipv4_enabled must be enabled or a
		// private_network must be configured. This setting can be updated, but it
	...
	_#defs: "/$defs/settings/$defs/ip_configuration/$defs/psc_config": close({
		...
		// Name of network attachment resource used to authorize a producer service to
		// connect a PSC interface to the consumer's VPC. For example:
	...
	_#defs: "/$defs/settings/$defs/ip_configuration/$defs/psc_config/$defs/psc_auto_connections": close({
		...
		// that includes both the host project and the network name. The consumer host
	...
	_#defs: "/$defs/settings/$defs/location_preference": close({
		// A Google App Engine application whose zone to remain in. Must be in the same
	...
	_#defs: "/$defs/settings/$defs/maintenance_window": close({
		...
		// Receive updates after one week (canary) or after two weeks (stable) or after
		// five weeks (week5) of notification.
		update_track?: string
	})
	...
	_#defs: "/$defs/settings/$defs/read_pool_auto_scale_config": close({
		...
		// Maximum number of nodes in the read pool. If set to lower than current node
		// count, node count will be updated.
		max_node_count?: number

		// Minimum number of nodes in the read pool. If set to higher than current node
		// count, node count will be updated.
	...
	_#defs: "/$defs/settings/$defs/sql_server_audit_config": close({
		...
		// How long to keep generated audit files. A duration in seconds with up to nine
		// fractional digits, terminated by 's'. Example: "3.5s"..
		retention_interval?: string

		// How often to upload generated audit files. A duration in seconds with up to
