...
google_clouddomains_registration: {
	...
	close({
		...
		// The list of contact notices that the caller acknowledges. Possible value is
		...
		// Output only. Set of options for the contactSettings.privacy field that this
	...
	#management_settings: close({
		// The desired renewal method for this Registration. The actual renewalMethod is
		// automatically updated to reflect this choice.
		// If unset or equal to RENEWAL_METHOD_UNSPECIFIED, the actual renewalMethod is
		// treated as if it were set to AUTOMATIC_RENEWAL.
		// You cannot use RENEWAL_DISABLED during resource creation, and you can update
		// the renewal status only when the Registration
		// resource has state ACTIVE or SUSPENDED.
		//
		// When preferredRenewalMethod is set to AUTOMATIC_RENEWAL, the actual
		// renewalMethod can be set to RENEWAL_DISABLED in case of
		// problems with the billing account or reported domain abuse. In such cases,
		// check the issues field on the Registration. After
		// the problem is resolved, the renewalMethod is automatically updated to
		...
		// preferredRenewalMethod is set to AUTOMATIC_RENEWAL,
		// the actual renewalMethod can be equal to RENEWAL_DISABLED—for example, when
		// there are problems with the billing account
		// or reported domain abuse. In such cases, check the issues field on the
		// Registration. After the problem is resolved, the
		// renewalMethod is automatically updated to preferredRenewalMethod in a few hours.
		renewal_method?: string

		// Controls whether the domain can be transferred to another registrar. Values
	...
	_#defs: "/$defs/contact_settings/$defs/admin_contact/$defs/postal_address": close({
		...
		// Because values in addressLines do not have type information and may sometimes
		// contain multiple values in a single
		// field (e.g. "Austin, TX"), it is important that the line order is clear. The
		// order of address lines should be
		// "envelope order" for the country/region of the address. In places where this
		// can vary (e.g. Japan), address_language
		// is used to make it explicit (e.g. "ja" for large-to-small ordering and
		// "ja-Latn" or "en" for small-to-large). This way,
		// the most specific line of an address can be selected based on the language.
		address_lines?: [...string]

		// Highest administrative subdivision which is used for postal addresses of a
		// country or region. For example, this can be a state,
		// a province, an oblast, or a prefecture. Specifically, for Spain this is the
		// province and not the autonomous community
		// (e.g. "Barcelona" and not "Catalonia"). Many countries don't use an
		// administrative area in postal addresses. E.g. in Switzerland
		// this should be left unpopulated.
		administrative_area?: string

		// Generally refers to the city/town portion of the address. Examples: US city,
		// IT comune, UK post town. In regions of the world
		// where localities are not well defined or do not fit into this structure well,
		...
		// Postal code of the address. Not all countries use or require postal codes to
		// be present, but where they are used,
		// they may trigger additional validation with other parts of the address (e.g.
		// state/zip validation in the U.S.A.).
		postal_code?: string

		// The recipient at the address. This field may, under certain circumstances,
		// contain multiline information. For example,
		// it might contain "care of" information.
		recipients?: [...string]

		// Required. CLDR region code of the country/region of the address. This is
		// never inferred and it is up to the user to
