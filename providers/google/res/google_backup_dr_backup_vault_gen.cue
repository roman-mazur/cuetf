...
google_backup_dr_backup_vault: {
	...
	close({
		...
		// 'WITHIN_ORGANIZATION' if not provided during creation. Default value:
		// "WITHIN_ORGANIZATION" Possible values: ["ACCESS_RESTRICTION_UNSPECIFIED",
		// "WITHIN_PROJECT", "WITHIN_ORGANIZATION", "UNRESTRICTED",
		...
		// How a backup's enforced retention end time is inherited. Default value is
		// 'INHERIT_VAULT_RETENTION' if not provided during creation. Possible values:
		// ["BACKUP_RETENTION_INHERITANCE_UNSPECIFIED", "INHERIT_VAULT_RETENTION",
		...
		// If set, the following restrictions against deletion of the backup vault
		// instance can be overridden:
		// * deletion of a backup vault instance containing no backups, but still
		// containing empty datasources.
		...
		// The Resource name of the Cloud KMS key to be used to encrypt new backups. The
		// key must be in the same location as the backup vault. The key must be a
		// Cloud KMS CryptoKey.
		kms_key_name?: string
	})
