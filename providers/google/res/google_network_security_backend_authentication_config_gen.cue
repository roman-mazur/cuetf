...
google_network_security_backend_authentication_config: {
	...
	close({
		...
		// A BackendService uses the chain of trust represented by this TrustConfig, if
		...
		// Well known roots to use for server certificate validation. If set to NONE,
		// the BackendService will only validate server certificates against roots
		// specified in TrustConfig.
		// If set to PUBLIC_ROOTS, the BackendService uses a set of well-known public
		...
		// Validation with these roots is only considered when the TlsSettings.sni field
		// in the BackendService is set. The well-known roots are a set of root CAs
		// managed by Google. CAs in this set can be added or removed without notice.
		// Possible values: ["NONE", "PUBLIC_ROOTS"]
