...
google_vertex_ai_endpoint: {
	...
	close({
		...
		// dedicatedEndpointEnabled is true. Format:
		...
		// EndpointService.UndeployModel respectively. Models can also be deployed and
		// undeployed using the [Cloud
		...
		// Endpoint if monitoring is enabled by CreateModelDeploymentMonitoringJob.
		// Format:
		// 'projects/{project}/locations/{location}/modelDeploymentMonitoringJobs/{model_deployment_monitoring_job}'
		model_deployment_monitoring_job?: string

		// The resource name of the Endpoint. The name must be numeric with no leading
		// zeros and can be at most 10 digits.
		name!: string
		...
		// project number, as in '12345', and '{network}' is network name. Only one of
		// the fields, 'network' or 'privateServiceConnectConfig', can be set.
		network?: string
		...
		// The traffic percentage values must add up to 100, or map must be empty if the
		// Endpoint is to not accept any traffic at a moment. See
		// the 'deployModel'
		...
		//
		// ~> **Note:** To set the map to empty, set '"{}"', apply, and then remove the
	...
	_#defs: "/$defs/predict_request_response_logging_config/$defs/bigquery_destination": close({
		// BigQuery URI to a project or table, up to 2000 characters long. When only the
		// project is specified, the Dataset and Table is created. When the full table
		// reference is specified, the Dataset must exist and table must not exist.
		// Accepted forms: - BigQuery path. For example: 'bq://projectId' or
