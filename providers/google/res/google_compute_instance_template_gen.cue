...
google_compute_instance_template: {
	...
	close({
		...
		// name. Max length is 54 characters. Prefixes with lengths longer than 37
		// characters will use a shortened UUID that will be more prone to collisions.
		name_prefix?: string
		...
		// specific region. Defaults to the region of the Provider if no value is
		// given.
		region?: string
	...
	#advanced_machine_features: close({
		...
		// processor runs code. Valid values for the level of PMU are "STANDARD",
		// "ENHANCED", and "ARCHITECTURAL".
	...
	#confidential_instance_config: close({
		...
		// Defines whether the instance should have confidential compute enabled. Field
	...
	#disk: close({
		...
		// operating system running within the instance. If not specified, the server
		// chooses a default device name to apply to this disk.
		device_name?: string
		...
		// The mode in which to attach this disk, either READ_WRITE or READ_ONLY. If you
		// are attaching or creating a boot disk, this must read-write mode.
		mode?: string

		// Indicates how many IOPS to provision for the disk. This sets the number of
		// I/O operations per second that the disk can handle. For more details, see
		// the [Extreme persistent disk
		...
		// Indicates how much throughput to provision for the disk, in MB/s. This sets
		// the amount of data that can be read or written from the disk per second.
		// Values must greater than or equal to 1. For more details, see the [Hyperdisk
		// documentation](https://cloud.google.com/compute/docs/disks/hyperdisks).
		...
		// google_compute_disk) to attach. ~> Note: Either source or source_image is
		// required when creating a new instance except for when creating a local SSD.
		source?: string

		// The image from which to initialize this disk. This can be one of: the image's
		// self_link, projects/{project}/global/images/{image},
		// projects/{project}/global/images/family/{family}, global/images/{image},
		// global/images/family/{family}, family/{family}, {project}/{family},
		// {project}/{image}, {family}, or {image}. ~> Note: Either source or
		// source_image is required when creating a new instance except for when
	...
	#network_interface: close({
		...
		// Indicates whether igmp query is enabled on the network interface or not. If
		...
		// One of EXTERNAL, INTERNAL to indicate whether the IP can be accessed from the
		// Internet. This field is always inherited from its subnetwork.
		ipv6_access_type?: string

		// An IPv6 internal network address for this network interface. If not
		// specified, Google Cloud will automatically assign an internal IPv6 address
		...
		// The private IP address to assign to the instance. If empty, the address will
		// be automatically assigned.
		network_ip?: string

		// The type of vNIC to be used on this interface. Possible values:GVNIC,
		// VIRTIO_NET, MRDMA, IRDMA and IDPF
		nic_type?: string
		...
		// interface. Both Rx and Tx queues will be set to this number. It will be
		...
		// The ID of the project in which the subnetwork belongs. If it is not provided,
		// the provider project is used.
	...
	#scheduling: close({
		...
		// terminated by Compute Engine (not terminated by a user). This defaults to
		// true.
	...
	#service_account: close({
		// The service account e-mail address. If not given, the default Google Compute
		// Engine service account is used.
		email?: string

		// A list of service scopes. Both OAuth2 URLs and gcloud short names are
		// supported. To allow full access to all Cloud APIs, use the cloud-platform
		// scope.
		scopes!: [...string]
	})

	#shielded_instance_config: close({
		// Compare the most recent boot measurements to the integrity policy baseline
		// and return a pair of pass/fail results depending on whether they match or
		// not. Defaults to true.
		enable_integrity_monitoring?: bool

		// Verify the digital signature of all boot components, and halt the boot
		// process if signature verification fails. Defaults to false.
		enable_secure_boot?: bool

		// Use a virtualized trusted platform module, which is a specialized computer
		// chip you can use to encrypt objects like keys and certificates. Defaults to
		// true.
		enable_vtpm?: bool
	})
	...
	_#defs: "/$defs/disk/$defs/source_image_encryption_key": close({
		...
		// base64 to either encrypt or decrypt this resource. Only one of
		// kms_key_self_link, rsa_encrypted_key and raw_key may be set.
		raw_key?: string

		// Specifies an RFC 4648 base64 encoded, RSA-wrapped 2048-bit customer-supplied
		// encryption key to either encrypt or decrypt this resource. Only one of
		// kms_key_self_link, rsa_encrypted_key and raw_key may be set.
	...
	_#defs: "/$defs/network_interface/$defs/access_config": close({
		// The IP address that will be 1:1 mapped to the instance's network ip. If not
		// given, one will be generated.
		nat_ip?: string

		// The networking tier used for configuring this instance template. This field
		// can take the following values: PREMIUM, STANDARD, FIXED_STANDARD. If this
	...
	_#defs: "/$defs/network_interface/$defs/alias_ip_range": close({
		...
		// by system or used by other network interfaces. At the time of writing only a
		// netmask (e.g. /24) may be supplied, with a CIDR format resulting in an API
		// error.
		ip_cidr_range!: string

		// The subnetwork secondary range name specifying the secondary range from which
		// to allocate the IP CIDR range for this alias IP range. If left unspecified,
	...
	_#defs: "/$defs/network_interface/$defs/ipv6_access_config": close({
		...
		// The service-level to be provided for IPv6 traffic when the subnet has an
		// external subnet. Only PREMIUM tier is valid for IPv6
		network_tier!: string
	...
	_#defs: "/$defs/reservation_affinity/$defs/specific_reservation": close({
		...
		// your reservation as the only value.
		key!: string
