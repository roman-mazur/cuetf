...
google_logging_metric: {
	...
	close({
		...
		// REGEXP_EXTRACT(field, regex). The argument are 1. field - The name of the log
		// entry field from which
		// the value is to be extracted. 2. regex - A regular expression using the Google RE2 syntax
		// (https://github.com/google/re2/wiki/Syntax) with a single capture group to
		// extract data from the specified
		// log entry field. The value of the field is converted to a string before
		// applying the regex. It is an
	...
	#metric_descriptor: close({
		...
		// Some combinations of metricKind and valueType might not be supported.
		// For counter metrics, set this to INT64. Possible values: ["BOOL", "INT64",
		// "DOUBLE", "STRING", "DISTRIBUTION", "MONEY"]
		value_type!: string
	})
