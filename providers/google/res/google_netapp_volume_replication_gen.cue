...
google_netapp_volume_replication: {
	...
	close({
		...
		// Create time of the active directory. A timestamp in RFC3339 UTC "Zulu"
		// format. Examples: "2023-06-22T09:13:01.617Z".
		create_time?: string

		// A destination volume is created as part of replication creation. The
		// destination volume will not became
		// under Terraform management unless you import it manually. If you delete the
		// replication, this volume
		// will remain.
		...
		// Only replications with mirror_state=MIRRORED can be stopped. A replication in
		// mirror_state=TRANSFERRING
		// currently receives an update and stopping the update might be undesirable.
		// Set this parameter to true
		// to stop anyway. All data transferred to the destination will be discarded and
		...
		// - false: The replication relationship is not healthy. It has missed the most
		// recent scheduled transfer.
		healthy?: bool
		...
		// Name of region for this resource. The resource needs to be created in the
		// region of the destination volume.
		location!: string

		// Indicates the state of the mirror between source and destination volumes.
		// Depending on the amount of data
		// in your source volume, PREPARING phase can take hours or days. mirrorState =
		// MIRRORED indicates your baseline
		// transfer ended and destination volume became accessible read-only.
		// TRANSFERRING means a MIRRORED volume
		...
		// Set to false to stop/break the mirror. Stopping the mirror makes the
		// destination volume read-write
		...
		// Reverting a replication can swap source and destination volume roles. This
		// field indicates if the 'location' hosts
		// the source or destination volume. For resume and revert and resume operations
		// it is critical to understand
		// which volume is the source volume, since it will overwrite changes done to
		...
		// Indicates the state of replication resource. State of the mirror itself is
		// indicated in mirrorState.
		state?: string
		...
		// Replication resource state is independent of mirror_state. With enough data,
	...
	#destination_volume_parameters: close({
		...
		// Share name for destination volume. If not specified, name of source volume's
		// share name will be used.
		share_name?: string

		// Name of an existing storage pool for the destination volume with format:
		// 'projects/{{project}}/locations/{{location}}/storagePools/{{poolId}}'
		storage_pool!: string

		// Name for the destination volume to be created. If not specified, the name of
