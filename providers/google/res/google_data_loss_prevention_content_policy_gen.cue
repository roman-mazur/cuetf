...
google_data_loss_prevention_content_policy: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/google_data_loss_prevention_content_policy")
	close({
		default_action?: matchN(1, [#default_action, list.MaxItems(1) & [...#default_action]])
		failed_to_scan_supported_file_type?: matchN(1, [#failed_to_scan_supported_file_type, list.MaxItems(1) & [...#failed_to_scan_supported_file_type]])
		input_too_large?: matchN(1, [#input_too_large, list.MaxItems(1) & [...#input_too_large]])
		inspect_config?: matchN(1, [#inspect_config, list.MaxItems(1) & [...#inspect_config]])
		logging_configs?: matchN(1, [#logging_configs, [...#logging_configs]])
		rules!: matchN(1, [#rules, [_, ...] & [...#rules]])
		timeouts?: #timeouts
		unsupported_file_type?: matchN(1, [#unsupported_file_type, list.MaxItems(1) & [...#unsupported_file_type]])
		...
		// Output only field. Will return the last 100 errors.
		errors?: [...close({
			details?: [...close({
				code?: number
				details?: [...{
	...
	#failed_to_scan_supported_file_type: close({
		// If set, the verdict will be returned to the user.
		// Possible values: ["ALLOW", "BLOCK"] Possible values: ["ALLOW", "BLOCK"]
		return_verdict?: string
	})

	#input_too_large: close({
		// If set, the verdict will be returned to the user.
		// Possible values: ["ALLOW", "BLOCK"] Possible values: ["ALLOW", "BLOCK"]
		return_verdict?: string
	})
	...
	#inspect_config: close({
		...
		// other content will be included. Possible values: ["CONTENT_TEXT",
		// "CONTENT_IMAGE"]
	...
	#logging_configs: close({
		log_to_big_query?: matchN(1, [_#defs."/$defs/logging_configs/$defs/log_to_big_query", list.MaxItems(1) & [..._#defs."/$defs/logging_configs/$defs/log_to_big_query"]])
	})

	#rules: close({
		action!: matchN(1, [_#defs."/$defs/rules/$defs/action", list.MaxItems(1) & [_, ...] & [..._#defs."/$defs/rules/$defs/action"]])
		conditions?: matchN(1, [_#defs."/$defs/rules/$defs/conditions", [..._#defs."/$defs/rules/$defs/conditions"]])
	})
	...
	_#defs: "/$defs/logging_configs/$defs/log_to_big_query": close({
	...
	_#defs: "/$defs/rules/$defs/conditions": close({
		info_type_condition?: matchN(1, [_#defs."/$defs/rules/$defs/conditions/$defs/info_type_condition", list.MaxItems(1) & [..._#defs."/$defs/rules/$defs/conditions/$defs/info_type_condition"]])
	})

	_#defs: "/$defs/rules/$defs/conditions/$defs/info_type_condition": close({
		any_info_type?: matchN(1, [_#defs."/$defs/rules/$defs/conditions/$defs/info_type_condition/$defs/any_info_type", list.MaxItems(1) & [..._#defs."/$defs/rules/$defs/conditions/$defs/info_type_condition/$defs/any_info_type"]])
		info_types?: matchN(1, [_#defs."/$defs/rules/$defs/conditions/$defs/info_type_condition/$defs/info_types", list.MaxItems(1) & [..._#defs."/$defs/rules/$defs/conditions/$defs/info_type_condition/$defs/info_types"]])

		// The minimum number of findings required for this condition to be met. Defaults to 1.
		min_count?: number
	})

	_#defs: "/$defs/rules/$defs/conditions/$defs/info_type_condition/$defs/any_info_type": close({})

	_#defs: "/$defs/rules/$defs/conditions/$defs/info_type_condition/$defs/info_types": close({
		// List of info type names.
		info_type_names!: [...string]
	})
}
