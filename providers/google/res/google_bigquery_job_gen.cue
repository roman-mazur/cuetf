...
google_bigquery_job: {
	...
	close({
		...
		// The ID of the job. The ID must contain only letters (a-z, A-Z), numbers
		// (0-9), underscores (_), or dashes (-). The maximum length is 1,024
		// characters.
		job_id!: string

		// Job timeout in milliseconds. If this time limit is exceeded, BigQuery may
		...
		// The status of this job. Examine this value when polling an asynchronous job
	...
	#copy: close({
		...
		// CREATE_NEVER: The table must already exist. If it does not, a 'notFound'
		// error is returned in the job result.
		// Creation, truncation and append actions occur as one atomic update upon job
		// completion Default value: "CREATE_IF_NEEDED" Possible values:
		...
		// Creation, truncation and append actions occur as one atomic update upon job
		// completion. Default value: "WRITE_EMPTY" Possible values: ["WRITE_TRUNCATE",
	...
	#extract: close({
		...
		// The compression type to use for exported files. Possible values include GZIP,
		// DEFLATE, SNAPPY, and NONE.
		// The default value is NONE. DEFLATE and SNAPPY are only supported for Avro.
		compression?: string

		// The exported file format. Possible values include CSV, NEWLINE_DELIMITED_JSON
		// and AVRO for tables and SAVED_MODEL for models.
		...
		// When extracting data in CSV format, this defines the delimiter to use between
		// fields in the exported data.
		// Default is ','
	...
	#load: close({
		...
		// and if there are too many bad records,
		...
		// encoded string to split the
		// data in its raw, binary state. BigQuery also supports the escape sequence
		// "\t" to specify a tab separator.
		// The default value is a comma (',').
		field_delimiter?: string

		// Indicates if BigQuery should allow extra values that are not represented in the table schema.
		// If true, the extra values are ignored. If false, records with extra columns
		// are treated as bad records,
		...
		// If sourceFormat is set to newline-delimited JSON, indicates whether it should
		// be processed as a JSON variant such as GeoJSON.
		// For a sourceFormat other than JSON, omit this field. If the sourceFormat is
		// newline-delimited JSON: - for newline-delimited
		// GeoJSON: set to GEOJSON.
		json_extension?: string

		// The maximum number of bad records that BigQuery can ignore when running the
		// job. If the number of bad records exceeds this value,
		// an invalid error is returned in the job result. The default value is 0, which
		// requires that all records are valid.
		max_bad_records?: number

		// Specifies a string that represents a null value in a CSV file. For example,
		// if you specify "\N", BigQuery interprets "\N" as a null value
		// when loading a CSV file. The default value is the empty string. If you set
		// this property to a custom value, BigQuery throws an error if an
		// empty string is present for all data types except for STRING and BYTE. For
		// STRING and BYTE columns, BigQuery interprets the empty string as
		// an empty value.
		null_marker?: string
		...
		// properties are specified, BigQuery loads all properties.
		// If any named property isn't found in the Cloud Datastore backup, an invalid
		// error is returned in the job result.
		...
		// the load job if a schema is autodetected or
		// supplied in the job configuration. Schema update options are supported in two
		// cases: when writeDisposition is WRITE_APPEND;
		...
		// For normal tables, WRITE_TRUNCATE will always overwrite the schema. One or
		// more of the following values are specified:
		...
		// skipLeadingRows unspecified - Autodetect tries to detect headers in the first
		// row. If they are not detected,
		...
		// headers in row N. If headers are not detected,
		// row N is just skipped. Otherwise row N is used to extract column names for the detected schema.
		skip_leading_rows?: number

		// The format of the data files. For CSV files, specify "CSV". For datastore
		// backups, specify "DATASTORE_BACKUP".
		// For newline-delimited JSON, specify "NEWLINE_DELIMITED_JSON". For Avro,
		// specify "AVRO". For parquet, specify "PARQUET".
	...
	#query: close({
		...
		// However, you must still set destinationTable when result size exceeds the
		...
		// If true and query uses legacy SQL dialect, flattens all nested and repeated
		...
		// Limits the billing tier for this job. Queries that have resource usage beyond
		// this tier will fail (without incurring a charge).
		// If unspecified, this will be set to your project default.
		maximum_billing_tier?: number

		// Limits the bytes billed for this job. Queries that will have bytes billed
		// beyond this limit will fail (without incurring a charge).
		// If unspecified, this will be set to your project default.
		...
		// Specifies a priority for the query. Default value: "INTERACTIVE" Possible
		// values: ["INTERACTIVE", "BATCH"]
		priority?: string

		// SQL query text to execute. The useLegacySql field can be used to indicate
		...
		// ('DELETE', 'UPDATE', 'MERGE', 'INSERT') must specify 'create_disposition =
		...
		// Whether to look for the result in the query cache. The query cache is a
		// best-effort cache that will be flushed whenever
		// tables in the query are modified. Moreover, the query cache is only available
		// when a query does not have a destination table specified.
	...
	_#defs: "/$defs/load/$defs/parquet_options": close({
		// If sourceFormat is set to PARQUET, indicates whether to use schema inference
		// specifically for Parquet LIST logical type.
		enable_list_inference?: bool

		// If sourceFormat is set to PARQUET, indicates whether to infer Parquet ENUM
		// logical type as STRING instead of BYTES by default.
		enum_as_string?: bool
	})

	_#defs: "/$defs/load/$defs/time_partitioning": close({
		// Number of milliseconds for which to keep the storage for a partition. A
	...
	_#defs: "/$defs/query/$defs/script_options": close({
		...
		// used to populate the schema and query results of the script job. Possible
		// values: ["LAST", "FIRST_SELECT"]
	...
	_#defs: "/$defs/query/$defs/user_defined_function_resources": close({
		...
		// Providing a inline code resource is equivalent to providing a URI for a file
