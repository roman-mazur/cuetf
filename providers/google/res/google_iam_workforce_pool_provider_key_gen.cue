...
google_iam_workforce_pool_provider_key: {
	...
	close({
		...
		// Note that the key may get purged before this time if the total limit of keys
		// per provider is exceeded.
		expire_time?: string
		id?:          string

		// The ID to use for the key, which becomes the final component of the resource
		// name. This value must be 4-32 characters, and may contain the characters
		// [a-z0-9-].
		key_id!: string
