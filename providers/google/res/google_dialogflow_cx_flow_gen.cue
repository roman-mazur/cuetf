...
google_dialogflow_cx_flow: {
	...
	close({
		...
		// Flow](https://cloud.google.com/dialogflow/cx/docs/concept/flow#start) for an
		// agent. When you create an agent, the Default Start Flow is created
		// automatically.
		// The Default Start Flow cannot be deleted; deleting the
		// 'google_dialogflow_cx_flow' resource does nothing to the underlying GCP
		// resources.
		//
		// ~> Avoid having multiple 'google_dialogflow_cx_flow' resources linked to the
		// same agent with 'is_default_start_flow = true' because they will compete to
		// control a single Default Start Flow resource in GCP.
		...
		// groups][Page.transition_route_groups]. Transition route groups defined in
	...
	#nlu_settings: close({
		...
		// If the returned score value is less than the threshold value, then a no-match
		// event will be triggered. The score values range from 0.0 (completely
		// uncertain) to 1.0 (completely certain). If set to 0.0, the default of 0.3 is
		// used.
		...
		// Possible values: ["MODEL_TRAINING_MODE_AUTOMATIC",
