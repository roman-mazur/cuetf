...
google_network_services_endpoint_policy: {
	...
	close({
		...
		// A URL referring to a ClientTlsPolicy resource. ClientTlsPolicy can be set to
		...
		// traffic at the identified backends.
		...
		// configuration. Possible values: ["SIDECAR_PROXY", "GRPC_SERVER"]
		type!: string
	...
	#traffic_port_selector: close({
		// List of ports. Can be port numbers or port range (example, [80-90] specifies
		// all ports from 80 to 90, including 80 and 90) or named ports or * to specify
		// all ports. If the list is empty, all ports are selected.
		ports!: [...string]
	})
