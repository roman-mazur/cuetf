...
google_iap_settings: {
	...
	_#defs: "/$defs/access_settings/$defs/oauth_settings": close({
		...
		// an appropriate access policy if you want to restrict access to users outside
		// your domain.
		login_hint?: string
	...
	_#defs: "/$defs/access_settings/$defs/reauth_settings": close({
		...
		// * 'ENROLLED_SECOND_FACTORS': User can use any enabled 2nd factor. Possible
		// values: ["LOGIN", "SECURE_KEY", "ENROLLED_SECOND_FACTORS"]
		method!: string
		...
		// Possible values: ["MINIMUM", "DEFAULT"]
		policy_type!: string
	})
	...
	_#defs: "/$defs/application_settings/$defs/attribute_propagation_settings": close({
		...
		// If set to true, attributes returned from the expression will be propagated in
		// the set output credentials.
		enable?: bool
		...
		// additional_claims": { "my_attribute": ["value1", "value2"] } Possible values:
		// ["HEADER", "JWT", "RCTOKEN"]
