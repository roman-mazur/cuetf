...
google_dataproc_gdc_spark_application: {
	...
	close({
		...
		// The annotations to associate with this application. Annotations may be used
		...
		// List of container image uris for additional file dependencies. Dependent
		// files are sequentially copied from each image. If a file with the same name
		// exists in 2 images then the file from later image is used.
		...
		// The labels to associate with this application. Labels may be used for
		...
		// URL for a monitoring UI for this application (for eventual Spark PHS/UI
		// support) Out of scope for private GA
		...
		// The Kubernetes namespace in which to create the application. This namespace
		// must already exist on the cluster.
		namespace?: string

		// An HCFS URI pointing to the location of stdout and stdout of the application
		// Mainly useful for Pantheon and gcloud Not in scope for private GA
		output_uri?: string
		...
		// Whether the application is currently reconciling. True if the current state
		// of the resource does not match the intended state, and the system is working
		// to reconcile them, whether or not the change was user initiated.
		reconciling?: bool
	...
	#pyspark_application_config: close({
		...
		// file types: .py, .egg, and .zip.
		python_file_uris?: [...string]
	})

	#spark_application_config: close({
		// HCFS URIs of archives to be extracted into the working directory of each
		// executor. Supported file types: '.jar', '.tar', '.tar.gz', '.tgz', and
		// '.zip'.
		archive_uris?: [...string]

		// The arguments to pass to the driver. Do not include arguments that can be set
		// as application properties, such as '--conf', since a collision can occur
		// that causes an incorrect application submission.
		args?: [...string]
		...
		// The name of the driver main class. The jar file that contains the class must
		// be in the classpath or specified in 'jar_file_uris'.
		main_class?: string
