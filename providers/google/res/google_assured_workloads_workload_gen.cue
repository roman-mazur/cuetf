...
google_assured_workloads_workload: {
	...
	close({
		...
		// Optional. Input only. The billing account used for the resources which are
		...
		// Possible values: COMPLIANCE_REGIME_UNSPECIFIED, IL4, CJIS, FEDRAMP_HIGH,
		...
		// but which are currently disallowed by the ResourceUsageRestriction org
		// policy. Invoke workloads.restrictAllowedResources endpoint to allow your
		...
		// Possible values: KAJ_ENROLLMENT_STATE_UNSPECIFIED,
		...
		// Optional. Partner regime associated with this workload. Possible values:
		// PARTNER_UNSPECIFIED, LOCAL_CONTROLS_BY_S3NS,
		...
		// Optional. Input only. Billing account necessary for purchasing services from
		// Sovereign Partners. This field is required for creating SIA/PSN/CNTXT
		...
		// SAA enrollment response is queried during workloads.get call. In failure
	...
	#kms_settings: close({
		...
		// Required. Input only. Immutable. will be advanced by this period when the Key
	...
	#resource_settings: close({
		...
		// Resource identifier. For a project this represents projectId. If the project
		...
		// ENCRYPTION_KEYS_PROJECT) Possible values: RESOURCE_TYPE_UNSPECIFIED,
	...
	#workload_options: close({
		// Indicates type of KAJ enrollment for the workload. Currently, only
		// specifiying KEY_ACCESS_TRANSPARENCY_OFF is implemented to not enroll in
		// KAT-level KAJ enrollment for Regional Controls workloads. Possible values:
		// KAJ_ENROLLMENT_TYPE_UNSPECIFIED, FULL_KAJ, EKM_ONLY,
