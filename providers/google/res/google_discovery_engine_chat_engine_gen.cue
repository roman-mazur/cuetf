...
google_discovery_engine_chat_engine: {
	...
	close({
		...
		// The data stores associated with this engine. Multiple DataStores in the same
		// Collection can be associated here. All listed DataStores must be
		// 'SOLUTION_TYPE_CHAT'.
		...
		// The industry vertical that the chat engine registers. Vertical on Engine has
		// to match vertical of the DataStore linked to the engine. Default value:
		// "GENERIC" Possible values: ["GENERIC"]
	...
	#chat_engine_config: close({
		...
		// The resource name of an existing Dialogflow agent to link to this Chat
		// Engine. Format:
	...
	_#defs: "/$defs/chat_engine_config/$defs/agent_creation_config": close({
		...
		// Agent location for Agent creation, currently supported values: global/us/eu,
		// it needs to be the same region as the Chat Engine.
		location?: string
