...
google_managed_kafka_acl: {
	...
	close({
		...
		// The ID to use for the acl, which will become the final component of the acl's
		// name. The structure of 'aclId' defines the Resource Pattern (resource_type,
		// resource_name, pattern_type) of the acl. 'aclId' is structured like one of
		// the following:
		...
		// The name of the acl. The 'ACL_ID' segment is used when connecting directly to
		// the cluster. Must be in the format
		...
		// The acl resource name derived from the name. For cluster resource_type, this
		// is always "kafka-cluster". Can be the wildcard literal "*".
