...
google_network_services_lb_route_extension: {
	...
	close({
		...
		// All backend services and forwarding rules referenced by this extension must
		// share the same load balancing scheme.
		// For more information, refer to [Choosing a load
		// balancer](https://cloud.google.com/load-balancing/docs/backend-service) and
	...
	_#defs: "/$defs/extension_chains/$defs/extensions": close({
		...
		// If 'supported_events' includes 'REQUEST_BODY', but 'request_body_send_mode'
		// is unset, the default value 'STREAMED' is used.
		// When this field is set to 'FULL_DUPLEX_STREAMED', 'supported_events' must
		// include both 'REQUEST_BODY' and 'REQUEST_TRAILERS'.
		// This field can be set only when the 'service' field of the extension points
		// to a 'BackendService'.
		// Only 'FULL_DUPLEX_STREAMED' mode is supported for 'LbRouteExtension'
		// resources. Possible values: ["BODY_SEND_MODE_UNSPECIFIED",
		// "BODY_SEND_MODE_STREAMED", "BODY_SEND_MODE_FULL_DUPLEX_STREAMED"]
		...
		// This field is optional for the LbRouteExtension resource. If unspecified,
		...
		// must be between 10-1000 milliseconds.
		// A duration in seconds with up to nine fractional digits, ending with 's'. Example: "3.5s".
		timeout?: string
	})

	_#defs: "/$defs/extension_chains/$defs/match_condition": close({
		// A Common Expression Language (CEL) expression that is used to match requests
