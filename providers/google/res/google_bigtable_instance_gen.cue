...
google_bigtable_instance: {
	...
	close({
		...
		// or terraform destroy that would delete the instance will fail. When the
		// field is set to false, deleting the instance is allowed.
		...
		// When deleting a BigTable instance, this boolean option will delete all
		// backups within the instance.
		force_destroy?: bool
		id?:            string
		...
		// The name (also called Instance Id in the Cloud Console) of the Cloud Bigtable
		// instance. Must be 6-33 characters and must only contain hyphens, lowercase
		// letters and numbers.
		name!: string
		...
		// A map of Resource Manager Tags. Keys can be either the numeric tag key ID
		// (tagKeys/123) or the namespaced name (project/tag-key). Values can be the
		// numeric tag value ID (tagValues/456) or the namespaced value
	...
	#cluster: close({
		...
		// The ID of the Cloud Bigtable cluster. Must be 6-30 characters and must only
		...
		// The node scaling factor of this cluster. One of "NodeScalingFactor1X" or
		// "NodeScalingFactor2X". Defaults to "NodeScalingFactor1X".
		...
		// The zone to create the Cloud Bigtable cluster in. Each cluster must have a
		// different zone in the same region. Zones that support Bigtable instances are
		// noted on the Cloud Bigtable locations page.
		zone?: string
	})
	...
	_#defs: "/$defs/cluster/$defs/autoscaling_config": close({
		...
		// The target storage utilization for autoscaling, in GB, for each node in a
		// cluster. This number is limited between 2560 (2.5TiB) and 5120 (5TiB) for a
		// SSD cluster and between 8192 (8TiB) and 16384 (16 TiB) for an HDD cluster.
		// If not set, whatever is already set for the cluster will not change, or if
		// the cluster is just being created, it will use the default value of 2560 for
