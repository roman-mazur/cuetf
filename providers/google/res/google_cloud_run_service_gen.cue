...
google_cloud_run_service: {
	...
	#metadata: close({
		...
		// If terraform plan shows a diff where a server-side annotation is added, you
		// can add it to your config
		// or apply the lifecycle.ignore_changes rule to the metadata.0.annotations field.
		//
		// Annotations with 'run.googleapis.com/' and 'autoscaling.knative.dev' are
		// restricted. Use the following annotation
		// keys to configure features on a Service:
		//
		// - 'run.googleapis.com/binary-authorization-breakglass' sets the [Binary
		// Authorization
		...
		// - 'run.googleapis.com/minScale' sets the [minimum number of container
		// instances](https://cloud.google.com/sdk/gcloud/reference/run/deploy#--min)
		// of the Service.
		// - 'run.googleapis.com/scalingMode' sets the type of scaling mode for the
		// service. The supported values for scaling mode are "manual" and "automatic".
		// If not provided, it defaults to "automatic".
		// - 'run.googleapis.com/manualInstanceCount' sets the total instance count for
		// the service in manual scaling mode. This number of instances is divided
	...
	_#defs: "/$defs/template/$defs/metadata": close({
		...
		// - 'run.googleapis.com/cpu-throttling' sets whether to throttle the CPU when
		// the container is not actively serving
		// requests. See https://cloud.google.com/sdk/gcloud/reference/run/deploy#--[no-]cpu-throttling.
		// - 'run.googleapis.com/encryption-key-shutdown-hours' sets the number of hours
		// to wait before an automatic shutdown
		...
		// - 'run.googleapis.com/startup-cpu-boost' sets whether to allocate extra CPU
		// to containers on startup.
		...
		// for the Revision.
		// - 'run.googleapis.com/vpc-access-egress' sets the outbound traffic to send
		// through the VPC connector for this resource.
		...
		// for the Revision.
		// - 'run.googleapis.com/health-check-disabled' disabled health checking
	...
	_#defs: "/$defs/template/$defs/spec": close({
		...
		// GPU](https://cloud.google.com/run/docs/configuring/services/gpu) required by
		// the Revision to run.
	...
	_#defs: "/$defs/template/$defs/spec/$defs/containers/$defs/env/$defs/value_from/$defs/secret_key_ref": close({
		...
		// assumed to be in the same project.
	...
	_#defs: "/$defs/template/$defs/spec/$defs/containers/$defs/ports": close({
		...
		// "http1" (HTTP/1) and "h2c" (HTTP/2 end-to-end). Defaults to "http1".
		name?: string
	...
	_#defs: "/$defs/template/$defs/spec/$defs/volumes/$defs/csi": close({
		// Unique name representing the type of file system to be created. Cloud Run
		// supports the following values:
		// * gcsfuse.run.googleapis.com: Mount a Google Cloud Storage bucket using
		// GCSFuse. This driver requires the
		...
		// * bucketName: The name of the Cloud Storage Bucket that backs this volume.
		// The Cloud Run Service identity must have access to this bucket.
	...
	_#defs: "/$defs/template/$defs/spec/$defs/volumes/$defs/empty_dir": close({
		...
		// the node's default medium. Must be an empty string (default) or Memory.
		medium?: string
