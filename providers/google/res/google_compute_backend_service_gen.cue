...
google_compute_backend_service: {
	...
	close({
		...
		// client's Accept-Encoding header. Possible values: ["AUTOMATIC", "DISABLED"]
		...
		// back to EXTERNAL or PREPARE can be used to migrate all traffic back to
		// EXTERNAL. Possible values: ["PREPARE", "TEST_BY_PERCENTAGE",
		// "TEST_ALL_TRAFFIC"]
		...
		// by a URL map that is bound to target gRPC proxy. Possible values: ["HTTP",
		// "HTTPS", "HTTP2", "TCP", "SSL", "UDP", "GRPC", "UNSPECIFIED", "H2C"]
		protocol?: string
		project?:  string
		...
		// Can only be set if load balancing scheme is EXTERNAL, EXTERNAL_MANAGED,
		...
		// "HEADER_FIELD", "HTTP_COOKIE", "STRONG_COOKIE_AFFINITY"]
	...
	#backend: close({
		...
		// sending traffic to backends
		// with default preference. This field cannot be set when loadBalancingScheme is
		// set to 'EXTERNAL'. The possible values are:
		// - PREFERRED: Backends with this preference level will be filled up to their
		// capacity limits first,
		// based on RTT.
		// - DEFAULT: If preferred backends don't have enough capacity, backends in this
		// layer would be used and
		// traffic would be assigned based on the load balancing algorithm you use. This
		// is the default Possible values: ["PREFERRED", "DEFAULT"]
		preference?: string
	})
	...
	#cdn_policy: close({
		...
		// into a small number of requests
		// to the origin.
	...
	#log_config: close({
		...
		// service and "logConfig.optionalMode"
	...
	_#defs: "/$defs/cdn_policy/$defs/negative_caching_policy": close({
		...
		// status code. The maximum allowed value is 1800s
		// (30 minutes), noting that infrequently accessed objects may be evicted from
	...
	_#defs: "/$defs/locality_lb_policies/$defs/policy": close({
		...
		// Maglev is not as stable as ring hash but has faster table lookup
		// build times and host selection times. For more information about
		// Maglev, refer to https://ai.google/research/pubs/pub44824 Possible values:
	...
	_#defs: "/$defs/security_settings/$defs/aws_v4_authentication": close({
		...
		// queried with a REST API GET request.
		access_key?: string
		...
		// the name of the region your cloud uses to host your origin.
