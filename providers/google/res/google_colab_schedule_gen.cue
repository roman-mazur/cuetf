...
google_colab_schedule: {
	...
	close({
		...
		// reached. If set to true, new runs will be queued instead of skipped. Default
		// to false.
		allow_queueing?: bool

		// Whether to backfill missed runs when the schedule is resumed from PAUSED
		// state. If set to true, all missed runs will be scheduled. New runs will be
		...
		// Desired state of the Colab Schedule. Set this field to 'ACTIVE' to
		// start/resume the schedule, and 'PAUSED' to pause the schedule.
		...
		// scheduled_run_count >= max_run_count. Must be in the RFC 3339
		...
		// Specifies the maximum number of active runs that can be executed concurrently
		// for this Schedule. This limits the number of runs that can be in a
		// non-terminal state at the same time. Currently, this field is only supported
		...
		// This is the limit for starting the scheduled requests and not the execution
		// of the notebook execution jobs created by the requests.
		max_concurrent_run_count!: string

		// Maximum run count of the schedule. If specified, The schedule will be
		// completed when either startedRunCount >= maxRunCount or when endTime is
		...
		// Timestamp when this Schedule should schedule the next run. Having a
		// next_run_time in the past means the runs are being started behind schedule.
	...
	_#defs: "/$defs/create_notebook_execution_job_request/$defs/notebook_execution_job": close({
		...
		// Max running time of the execution job in seconds (default 86400s / 24 hrs). A
		...
		// Possible values: JOB_STATE_QUEUED JOB_STATE_PENDING JOB_STATE_RUNNING
		// JOB_STATE_SUCCEEDED JOB_STATE_FAILED JOB_STATE_CANCELLING
		// JOB_STATE_CANCELLED JOB_STATE_PAUSED JOB_STATE_EXPIRED JOB_STATE_UPDATING
	...
	_#defs: "/$defs/create_notebook_execution_job_request/$defs/notebook_execution_job/$defs/custom_environment_spec/$defs/machine_spec": close({
		...
		// Possible values: NVIDIA_TESLA_K80 NVIDIA_TESLA_P100 NVIDIA_TESLA_V100
		// NVIDIA_TESLA_P4 NVIDIA_TESLA_T4 NVIDIA_TESLA_A100 NVIDIA_A100_80GB NVIDIA_L4
		// NVIDIA_H100_80GB NVIDIA_H100_MEGA_80GB NVIDIA_H200_141GB NVIDIA_B200
		// NVIDIA_GB200 NVIDIA_RTX_PRO_6000 TPU_V2 TPU_V3 TPU_V4_POD TPU_V5_LITEPOD
		accelerator_type?: string

		// The Nvidia GPU partition size. When specified, the requested accelerators
		// will be partitioned into smaller GPU partitions. For example, if the request
		// is for 8 units of NVIDIA A100 GPUs, and gpu_partition_size="1g.10gb", the
		// service will create 8 * 7 = 56 partitioned MIG instances. The partition size
		// must be a value supported by the requested accelerator. Refer to [Nvidia GPU
		...
		// The type of the machine. See the [list of machine types supported for
	...
	_#defs: "/$defs/create_notebook_execution_job_request/$defs/notebook_execution_job/$defs/custom_environment_spec/$defs/machine_spec/$defs/reservation_affinity": close({
		...
		// Specifies the reservation affinity type. Possible values: NO_RESERVATION
		// ANY_RESERVATION SPECIFIC_RESERVATION SPECIFIC_THEN_ANY_RESERVATION
	...
	_#defs: "/$defs/create_notebook_execution_job_request/$defs/notebook_execution_job/$defs/encryption_spec": close({
		// Resource name of the Cloud KMS key used to protect the resource. The Cloud
	...
	_#defs: "/$defs/create_pipeline_job_request/$defs/pipeline_job": close({
		...
		// Compute Engine default service account in the project will be used. See
	...
	_#defs: "/$defs/create_pipeline_job_request/$defs/pipeline_job/$defs/runtime_config": close({
		...
		// directory of the pipeline. It is used by the system to generate the paths of
		// output artifacts. The artifact paths are generated with a sub-path pattern
		// '{job_id}/{task_id}/{output_key}' under the specified output directory. The
		// service account specified in this pipeline must have the
		...
		// 2.1.0, such as pipelines built using Kubeflow Pipelines SDK 1.9 or higher
		// and the v2 DSL.
