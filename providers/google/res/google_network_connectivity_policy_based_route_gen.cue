...
google_network_connectivity_policy_based_route: {
	...
	close({
		...
		// Possible values: ["DEFAULT_ROUTING"]
		next_hop_other_routes?: string

		// The priority of this policy-based route. Priority is used to break ties in
	...
	#filter: close({
		...
		// applies to. Default is "0.0.0.0/0".
		dest_range?: string

		// The IP protocol that this policy-based route applies to. Valid values are
		...
		// The source IP range of outgoing packets that this policy-based route applies
		// to. Default is "0.0.0.0/0".
		src_range?: string
	})

	#interconnect_attachment: close({
		// Cloud region to install this policy-based route on for Interconnect
		// attachments. Use 'all' to install it on all Interconnect attachments.
		region!: string
	})
	...
	#virtual_machine: close({
		// A list of VM instance tags that this policy-based route applies to. VM
		// instances that have ANY of tags specified here will install this PBR.
		tags!: [...string]
	})
}
