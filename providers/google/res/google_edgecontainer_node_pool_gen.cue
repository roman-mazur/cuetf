...
google_edgecontainer_node_pool: {
	...
	#local_disk_encryption: close({
		...
		// The Cloud KMS CryptoKeyVersion currently in use for protecting node local
		// disks. Only applicable if kmsKey is set.
		kms_key_active_version?: string

		// Availability of the Cloud KMS CryptoKey. If not KEY_AVAILABLE, then nodes may
		// go offline as they cannot access their local data.
