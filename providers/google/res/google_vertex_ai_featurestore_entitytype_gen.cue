...
google_vertex_ai_featurestore_entitytype: {
	...
	close({
		...
		// The name of the EntityType. This value may be up to 60 characters, and valid
	...
	_#defs: "/$defs/monitoring_config/$defs/categorical_threshold_config": close({
		// Specify a threshold value that can trigger the alert. For categorical
		// feature, the distribution distance is calculated by L-inifinity norm. Each
		// feature must have a non-zero threshold if they need to be monitored.
		// Otherwise no alert will be triggered for that feature. The default value is
		// 0.3.
		value!: number
	})
	...
	_#defs: "/$defs/monitoring_config/$defs/import_features_analysis": close({
		...
		// each [entityTypes.importFeatureValues][] operation. The value must be one of
		// the values below:
		...
		// * MOST_RECENT_SNAPSHOT_STATS: Use the statistics generated by the most recent
		// snapshot analysis if exists.
		...
		// analysis. The value must be one of the values below:
		...
		// config: by default disables import features analysis for all Features under
		// it.
		state?: string
	})

	_#defs: "/$defs/monitoring_config/$defs/numerical_threshold_config": close({
		// Specify a threshold value that can trigger the alert. For numerical feature,
		// the distribution distance is calculated by Jensen–Shannon divergence. Each
		// feature must have a non-zero threshold if they need to be monitored.
		// Otherwise no alert will be triggered for that feature. The default value is
		// 0.3.
		value!: number
	...
	_#defs: "/$defs/monitoring_config/$defs/snapshot_analysis": close({
		...
		// interval. The value indicates number of days. The default value is 1.
		// If both
		// FeaturestoreMonitoringConfig.SnapshotAnalysis.monitoring_interval_days and
		// [FeaturestoreMonitoringConfig.SnapshotAnalysis.monitoring_interval][] are
		// set when creating/updating EntityTypes/Features,
		// FeaturestoreMonitoringConfig.SnapshotAnalysis.monitoring_interval_days will
		// be used.
		monitoring_interval_days?: number

		// Customized export features time window for snapshot analysis. Unit is one
		// day. The default value is 21 days. Minimum value is 1 day. Maximum value is
		// 4000 days.
