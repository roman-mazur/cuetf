...
google_vertex_ai_deployment_resource_pool: {
	...
	close({
		...
		// The resource name of deployment resource pool. The maximum length is 63
	...
	#dedicated_resources: close({
		...
		// is not provided, will use min_replica_count as the default value. The value
		// of this field impacts the charge against Vertex CPU and GPU quotas.
		// Specifically, you will be charged for max_replica_count * number of cores in
		// the selected machine type) and (max_replica_count * number of GPUs per
		...
		// deployed on. This value must be greater than or equal to 1. If traffic
		// against the DeployedModel increases, it may dynamically be deployed onto
		// more replicas, and as traffic decreases, some of these extra replicas may be
		// freed.
	...
	_#defs: "/$defs/dedicated_resources/$defs/machine_spec": close({
		...
		// accelerator_count. See possible values
