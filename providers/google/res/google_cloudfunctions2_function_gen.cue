...
google_cloudfunctions2_function: {
	...
	#event_trigger: close({
		...
		// Retried execution is charged as any other execution. Possible values:
		// ["RETRY_POLICY_UNSPECIFIED", "RETRY_POLICY_DO_NOT_RETRY",
		// "RETRY_POLICY_RETRY"]
		retry_policy?: string
	...
	#service_config: close({
		...
		// When false, GCF honors the existing traffic configuration of the underlying
		// Cloud Run service. If that configuration is set to route to LATEST (the
		// default), the new deployment will become LATEST and intercept the traffic.
		// To prevent traffic from shifting, you must manually pin the existing service
		// to a specific revision name in Cloud Run before deploying.
		all_traffic_on_latest_revision?: bool

		// The number of CPUs used in a single container instance. Default value is
		// calculated from available memory.
		...
		// VPC_EGRESS_PRIVATE_RANGES_ONLY. Possible values: ["VPC_EGRESS_ALL_TRAFFIC",
		...
		// Available ingress settings. Defaults to "ALLOW_ALL" if unspecified. Default
		// value: "ALLOW_ALL" Possible values: ["ALLOW_ALL", "ALLOW_INTERNAL_ONLY",
		...
		// ["VPC_CONNECTOR_EGRESS_SETTINGS_UNSPECIFIED", "PRIVATE_RANGES_ONLY",
		// "ALL_TRAFFIC"]
	...
	_#defs: "/$defs/event_trigger/$defs/event_filters": close({
		...
		// 'gcloud eventarc providers describe' command to learn more about events and
		// their attributes.
		// Do not filter for the 'type' attribute here, as this is already achieved by
		// the resource's 'event_type' attribute.
		attribute!: string
		...
		// If the operator field is set as 'match-path-pattern', this value can be a
		// path pattern instead of an exact value.
		value!: string
	})

	_#defs: "/$defs/service_config/$defs/direct_vpc_network_interface": close({
		// The name of the VPC network to which the function will be connected. Specify
		// either a VPC network or a subnet, or both. If you specify only a network,
		// the subnet uses the same name as the network.
		network?: string

		// The name of the VPC subnetwork that the Cloud Function resource will get IPs
		// from. Specify either a VPC network or a subnet, or both. If both network and
	...
	_#defs: "/$defs/service_config/$defs/secret_environment_variables": close({
		...
		// of the project that contains the secret. If not set, it will be populated
		// with the function's project assuming that the secret exists in the same
	...
	_#defs: "/$defs/service_config/$defs/secret_volumes": close({
		...
		// setting the mountPath as /etc/secrets would mount the secret value files
	...
	_#defs: "/$defs/service_config/$defs/secret_volumes/$defs/versions": close({
		...
		// mountPath as '/etc/secrets' and path as secret_foo would mount the secret
