...
google_vertex_ai_index_endpoint_deployed_index: {
	...
	close({
		...
		// The user specified ID of the DeployedIndex. The ID can be up to 128
		// characters long and must start with a letter and only contain letters,
		...
		// 'prod'). If not set, we will use the 'default' deployment group.
		...
		// Also, one deployment_group (except 'default') can only be used with the same
		...
		// when what the Index contains is being changed) the DeployedIndex may be
		// asynchronously updated in the background to reflect these changes. If this
		// timestamp's value is at least the
		...
		// this DeployedIndex already contains (and which it does not), one must
		...
		// completed Operations with updateTime equal or before this sync time are
		...
		// the index might be deployed to any ip ranges under the provided VPC network.
		//
	...
	#automatic_resources: close({
		// The maximum number of replicas this DeployedModel may be deployed on when the
		// traffic against it increases. If maxReplicaCount is not set, the default
		// value is minReplicaCount. The max allowed replica count is 1000.
		//
		// The maximum number of replicas this DeployedModel may be deployed on when the
		...
		// the model to that many replicas is guaranteed (barring service outages). If
		// traffic against the DeployedModel increases beyond what its replicas at
		// maximum may handle, a portion of the traffic will be dropped. If this value
		// is not provided, a no upper bound for scaling under heavy traffic will be
		// assume, though Vertex AI may be unable to scale beyond certain replica
		// number.
		max_replica_count?: number

		// The minimum number of replicas this DeployedModel will be always deployed on.
		// If minReplicaCount is not set, the default value is 2 (we don't provide SLA
		// when minReplicaCount=1).
		//
	...
	_#defs: "/$defs/dedicated_resources/$defs/machine_spec": close({
		...
		// this field is optional, and the default value is n1-standard-2. For
