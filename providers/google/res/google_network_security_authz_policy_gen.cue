...
google_network_security_authz_policy: {
	...
	close({
		...
		// 1. If there is a CUSTOM policy that matches the request, the CUSTOM policy is
		...
		// 3. If there are no ALLOW policies for the resource or if any of the ALLOW
		// policies match the request, the request is allowed.
		// 4. Else the request is denied by default if none of the configured
		// AuthzPolicies with ALLOW action match the request. Possible values:
		// ["ALLOW", "DENY", "CUSTOM"]
		action!: string
		...
		// Defines the type of authorization being performed. 'REQUEST_AUTHZ' applies to
		// request authorization. CUSTOM
		// authorization policies with Authz extensions will be allowed with ext_authz
		// or ext_proc protocols. Extensions are
		// invoked only once when the request headers arrive. 'CONTENT_AUTHZ' applies to
		// content security, sanitization, etc.
		// Only CUSTOM action is allowed in this policy profile. AuthzExtensions in the
		// custom provider must support ext_proc
		// protocol and be capable of receiving all ext_proc events (REQUEST_HEADERS,
		// REQUEST_BODY, REQUEST_TRAILERS,
		// RESPONSE_HEADERS, RESPONSE_BODY, RESPONSE_TRAILERS) with FULL_DUPLEX_STREAMED
	...
	#http_rules: close({
		...
		// The result of the CEL expression is ANDed with the from and to. Refer to the
		// CEL language reference for a list of available attributes.
		when?: string
	})
	...
	#target: close({
		// Required when targeting forwarding rules and secure web proxy. Must not be
		// specified when targeting Agent
		// Gateway. All resources referenced by this policy and extensions must share
		// the same load balancing scheme.
		...
		// A list of references to the Forwarding Rules or Secure Web Proxy Gateways or
		// Agent Gateways on which this
	...
	_#defs: "/$defs/http_rules/$defs/from/$defs/not_sources/$defs/principals": close({
		...
		// If not specified, the PrincipalSelector is CLIENT_CERT_URI_SAN. Default
		// value: "CLIENT_CERT_URI_SAN" Possible values:
		// ["PRINCIPAL_SELECTOR_UNSPECIFIED", "CLIENT_CERT_URI_SAN",
	...
	_#defs: "/$defs/http_rules/$defs/from/$defs/not_sources/$defs/principals/$defs/principal": close({
		...
		// match is not allowed, please use regex instead.
		// Examples:
		...
		// insensitive. For example, the matcher data will match both input string Data
		// and data if set to true.
		ignore_case?: bool

		// The input string must have the prefix specified here. Note: empty prefix is
		// not allowed, please use regex instead.
		// Examples:
	...
	_#defs: "/$defs/http_rules/$defs/from/$defs/not_sources/$defs/resources/$defs/tag_value_id_set": close({
		...
		// follows AND semantics which means all the ids must match.
	...
	_#defs: "/$defs/http_rules/$defs/to/$defs/not_operations": close({
		...
		// A list of HTTP methods to match against. Each entry must be a valid HTTP
		// method name (GET, PUT, POST, HEAD, PATCH, DELETE, OPTIONS). It only allows
		// exact match and is always case sensitive.
		methods?: [...string]
	})
	...
	_#defs: "/$defs/http_rules/$defs/to/$defs/operations/$defs/mcp": close({
		...
		// Default value: "SKIP_BASE_PROTOCOL_METHODS" Possible values:
	...
	_#defs: "/$defs/http_rules/$defs/to/$defs/operations/$defs/mcp/$defs/methods": close({
		...
		// 1) “tools”, “prompts”, “resources” - these will match against all sub methods
