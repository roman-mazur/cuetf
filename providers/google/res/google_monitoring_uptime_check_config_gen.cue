...
google_monitoring_uptime_check_config: {
	...
	close({
		...
		// Possible values: ["STATIC_IP_CHECKERS", "VPC_CHECKERS"]
		checker_type?: string
		...
		// 'UptimeCheckConfig' objects. The field can contain up to 64 entries. Each
		// key and value is limited to 63 Unicode characters or 128 bytes, whichever is
	...
	#content_matchers: close({
		...
		// compared to the content string when the check is run. Default value:
		// "CONTAINS_STRING" Possible values: ["CONTAINS_STRING",
		// "NOT_CONTAINS_STRING", "MATCHES_REGEX", "NOT_MATCHES_REGEX",
	...
	#http_check: close({
		...
		// 'Content-Length' header via the 'headers' field or the API will do so. If
		// the 'request_method' is 'GET' and 'body' is not empty, the API will return
		// an error. The maximum byte size is 1 megabyte. Note - As with all bytes
		// fields JSON representations are base64 encoded. e.g. 'foo=bar' in
		// URL-encoded form is 'foo%3Dbar' and in base64 encoding is
		// 'Zm9vJTI1M0RiYXI='.
		body?: string

		// The content type to use for the check. Possible values: ["TYPE_UNSPECIFIED",
		// "URL_ENCODED", "USER_PROVIDED"]
		content_type?: string
		...
		// desired values as described in [RFC 2616 (page
		// 31)](https://www.w3.org/Protocols/rfc2616/rfc2616.txt). Entering two
		...
		// (specified within the ['monitored_resource'](#nested_monitored_resource))
		// and path to construct the full URL. Optional (defaults to 80 without SSL, or
		// 443 with SSL).
		port?: number

		// The HTTP request method to use for the check. If set to 'METHOD_UNSPECIFIED'
		// then 'request_method' defaults to 'GET'. Default value: "GET" Possible
		// values: ["METHOD_UNSPECIFIED", "GET", "POST"]
		...
		// the Uptime check. Only applies to checks where 'monitored_resource' is set
		// to 'uptime_url'. If 'use_ssl' is 'false', setting 'validate_ssl' to 'true'
		// has no effect.
		validate_ssl?: bool
	})
	...
	#monitored_resource: close({
		...
		// descriptor. For example, Compute Engine VM instances use the labels
		...
		// 'gce_instance'. For a list of types, see [Monitoring resource
	...
	#tcp_check: close({
		...
		// The port to the page to run the check against. Will be combined with host
		// (specified within the 'monitored_resource') to construct the full URL.
		port!: number
	})
	...
	_#defs: "/$defs/content_matchers/$defs/json_path_matcher": close({
		// Options to perform JSONPath content matching. Default value: "EXACT_MATCH"
		// Possible values: ["EXACT_MATCH", "REGEX_MATCH"]
		json_matcher?: string
	...
	_#defs: "/$defs/http_check/$defs/accepted_response_status_codes": close({
		// A class of status codes to accept. Possible values: ["STATUS_CLASS_1XX",
