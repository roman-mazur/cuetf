...
google_storage_bucket: {
	...
	#autoclass: close({
		// While set to true, autoclass automatically transitions objects in your bucket
		// to appropriate storage classes based on each object's access pattern.
		enabled!: bool

		// The storage class that objects in the bucket eventually transition to if they
		// are not read for a certain length of time. Supported values include:
		// NEARLINE, ARCHIVE.
	...
	#custom_placement_config: close({
		// The list of individual regions that comprise a dual-region bucket. See the
		// docs for a list of acceptable regions. Note: If any of the data_locations
		...
		// bucket, if no encryption method is specified. You must pay attention to
		// whether the crypto key is available in the location that this bucket is
		// created in. See the docs for more details.
	...
	#logging: close({
		...
		// The object prefix for log objects. If it's not provided, by default Google
		// Cloud Storage sets this to this bucket's name.
		log_object_prefix?: string
	})

	#retention_policy: close({
		// If set to true, the bucket will be locked and permanently restrict edits to
		// the bucket's retention policy. Caution: Locking a bucket is an irreversible
		// action.
		is_locked?: bool

		// The period of time, in seconds, that objects in the bucket must be retained
		// and cannot be deleted, overwritten, or archived. The value must be less than
	...
	#soft_delete_policy: close({
		...
		// retained and cannot be permanently deleted. Default value is 604800.
	...
	_#defs: "/$defs/encryption/$defs/customer_managed_encryption_enforcement_config": close({
		...
		// Whether CMEK is restricted for new objects within the bucket. If
		// FullyRestricted, new objects can't be created using CMEK encryption. If
		// NotRestricted or unset, creation of new objects with CMEK encryption is
		// allowed.
	...
	_#defs: "/$defs/encryption/$defs/customer_supplied_encryption_enforcement_config": close({
		...
		// Whether CSEK is restricted for new objects within the bucket. If
		// FullyRestricted, new objects can't be created using CSEK encryption. If
		// NotRestricted or unset, creation of new objects with CSEK encryption is
		// allowed.
		restriction_mode!: string
	})
	...
	_#defs: "/$defs/encryption/$defs/google_managed_encryption_enforcement_config": close({
		...
		// Whether GMEK is restricted for new objects within the bucket. If
		// FullyRestricted, new objects can't be created using GMEK encryption. If
		// NotRestricted or unset, creation of new objects with GMEK encryption is
		// allowed.
		restriction_mode!: string
	})
	...
	_#defs: "/$defs/lifecycle_rule/$defs/action": close({
		...
		// Supported values include: MULTI_REGIONAL, REGIONAL, NEARLINE, COLDLINE,
		// ARCHIVE.
		storage_class?: string
	...
	_#defs: "/$defs/lifecycle_rule/$defs/condition": close({
		...
		// Relevant only for versioned objects. The number of newer versions of an
		// object to satisfy this condition.
		num_newer_versions?: number

		// While set true, age value will be sent in the request even for zero value of
		// the field. This field is only useful for setting 0 value to the age field.
		// It can be used alone or together with age.
		send_age_if_zero?: bool

		// While set true, days_since_custom_time value will be sent in the request even
		// for zero value of the field. This field is only useful for setting 0 value
		// to the days_since_custom_time field. It can be used alone or together with
		// days_since_custom_time.
		send_days_since_custom_time_if_zero?: bool

		// While set true, days_since_noncurrent_time value will be sent in the request
		// even for zero value of the field. This field is only useful for setting 0
		// value to the days_since_noncurrent_time field. It can be used alone or
		// together with days_since_noncurrent_time.
		send_days_since_noncurrent_time_if_zero?: bool

		// While set true, num_newer_versions value will be sent in the request even for
		// zero value of the field. This field is only useful for setting 0 value to
		// the num_newer_versions field. It can be used alone or together with
		// num_newer_versions.
		...
		// Match to live and/or archived objects. Unversioned buckets have only live
		// objects. Supported values include: "LIVE", "ARCHIVED", "ANY".
		with_state?: string
	})
}
