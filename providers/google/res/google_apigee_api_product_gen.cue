...
google_apigee_api_product: {
	...
	close({
		...
		// resources through other API proxies.
		...
		// For example, a quota of 50, for a quotaInterval of 12 and a quotaTimeUnit of
		...
		// operation does not have dedicated quota set for itself. Possible values:
		...
		// Time unit defined for the quotaInterval. Valid values include second, minute,
		...
		// resource will be the Organization.
		space?: string
	})
	...
	#graphql_operation_group: close({
		...
		// associated with the API product. Possible values: ["proxy", "remoteservice"]
	...
	_#defs: "/$defs/graphql_operation_group/$defs/operation_configs/$defs/quota": close({
		...
		// Time unit defined for the interval. Valid values include second, minute,
		// hour, day, month or year. If limit and interval are valid, the default value
		// is hour; otherwise, the default is null.
		time_unit?: string
	})
	...
	_#defs: "/$defs/grpc_operation_group/$defs/operation_configs": close({
		...
		// the gRPC service defined on the proxy.
		//
		// Example: Given a proxy that is configured to serve com.petstore.PetService,
		// the methods com.petstore.PetService.ListPets and
		// com.petstore.PetService.GetPet would be specified here as simply
		// ["ListPets", "GetPet"].
		//
		// Note: Currently, you can specify only a single GraphQLOperation. Specifying
