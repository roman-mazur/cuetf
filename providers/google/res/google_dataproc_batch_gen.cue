...
google_dataproc_batch: {
	...
	close({
		...
		// A batch UUID (Unique Universal Identifier). The service generates this value
	...
	#pyspark_batch: close({
		...
		// properties, such as --conf, since a collision can occur that causes an
	...
	_#defs: "/$defs/environment_config/$defs/execution_config": close({
		...
		// work to finish. If ttl is not specified for a batch workload, the workload
	...
	_#defs: "/$defs/runtime_config/$defs/autotuning_config": close({
		// Optional. Scenarios for which tunings are applied. Possible values: ["AUTO",
