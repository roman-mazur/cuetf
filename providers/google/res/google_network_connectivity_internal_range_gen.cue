...
google_network_connectivity_internal_range: {
	...
	close({
		...
		// Optional. List of IP CIDR ranges to be excluded. Resulting reserved Internal
		// Range will not overlap with any CIDR blocks mentioned in this list.
		...
		// internal range. Possible values: ["OVERLAP_ROUTE_RANGE",
		// "OVERLAP_EXISTING_SUBNET_RANGE"]
		overlaps?: [...string]

		// The type of peering set for this internal range. Possible values:
		// ["FOR_SELF", "FOR_PEER", "NOT_SHARED"]
		peering!: string

		// An alternate to ipCidrRange. Can be set when trying to create a reservation
		// that automatically finds a free range of the given size.
		// If both ipCidrRange and prefixLength are set, there is an error if the range
		...
		// The type of usage set for this InternalRange. Possible values: ["FOR_VPC",
		// "EXTERNAL_TO_VPC", "FOR_MIGRATION"]
		usage!: string

		// Output only. The list of resources that refer to this internal range.
		// Resources that use the internal range for their range allocation are referred
		// to as users of the range.
		// Other resources mark themselves as users while doing so by creating a
		// reference to this internal range. Having a user, based on this reference,
		// prevents deletion of the internal range referred to. Can be empty.
		users?: [...string]
	})

	#allocation_options: close({
		// Optional. Sets the strategy used to automatically find a free range of a size
		// given by prefixLength. Can be set only when trying to create a reservation
		// that automatically finds the free range to reserve. Possible values:
		// ["RANDOM", "FIRST_AVAILABLE", "RANDOM_FIRST_N_AVAILABLE",
		// "FIRST_SMALLEST_FITTING"]
		allocation_strategy?: string

		// Must be set when allocation_strategy is RANDOM_FIRST_N_AVAILABLE, otherwise
		// must remain unset. Defines the size of the set of free ranges from which
