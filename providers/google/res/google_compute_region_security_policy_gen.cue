...
google_compute_region_security_policy: {
	...
	close({
		...
		// requests before they hit the origin servers.
		// - CLOUD_ARMOR_EDGE: Cloud Armor edge security policies can be configured to
		// filter incoming HTTP requests targeting backend services (including Cloud
		...
		// This field can be set only at resource creation time. Possible values:
	...
	#advanced_options_config: close({
		...
		// "STANDARD_WITH_GRAPHQL". Possible values: ["DISABLED", "STANDARD",
		// "STANDARD_WITH_GRAPHQL"]
		json_parsing?: string

		// Logging level. Supported values include: "NORMAL", "VERBOSE". Possible
		// values: ["NORMAL", "VERBOSE"]
		log_level?: string
	...
	#ddos_protection_config: close({
		...
		// - STANDARD: basic always-on protection for network load balancers, protocol
		// forwarding, or VMs with public IP addresses.
		// - ADVANCED: additional protections for Managed Protection Plus subscribers
		// who use network load balancers, protocol forwarding, or VMs with public IP
		// addresses.
		// - ADVANCED_PREVIEW: flag to enable the security policy in preview mode.
		// Possible values: ["ADVANCED", "ADVANCED_PREVIEW", "STANDARD"]
	...
	#rules: close({
		...
		//
		// * rate_based_ban: limit client traffic to the configured threshold and ban
		// the client if the traffic exceeds the threshold. Configure parameters for
		// this action in RateLimitOptions. Requires rateLimitOptions to be set.
		//
		...
		// parameters for this action in rateLimitOptions. Requires rateLimitOptions to
		// be set for this.
		action!: string
	...
	#user_defined_fields: close({
		...
		// - TCP: Points to the beginning of the TCP header, skipping over any IPv4
		// options or IPv6 extension headers. Not present for non-first fragments.
		// - UDP: Points to the beginning of the UDP header, skipping over any IPv4
		// options or IPv6 extension headers. Not present for non-first fragments.
		// Possible values: ["IPV4", "IPV6", "TCP", "UDP"]
		base!: string
	...
	_#defs: "/$defs/rules/$defs/match": close({
		...
		// SRC_IPS_V1 - must specify the corresponding srcIpRange field in config.
		// Possible values: ["SRC_IPS_V1"]
	...
	_#defs: "/$defs/rules/$defs/preconfigured_waf_config/$defs/exclusion/$defs/request_cookie": close({
		...
		// EQUALS_ANY: The operator matches if the field value is any value. Possible
		// values: ["CONTAINS", "ENDS_WITH", "EQUALS", "EQUALS_ANY", "STARTS_WITH"]
		operator!: string
	...
	_#defs: "/$defs/rules/$defs/rate_limit_options": close({
		...
		// If specified, determines the time (in seconds) the traffic will continue to
		// be banned by the rate limit after the rate falls below the threshold.
		...
		// this rule. This is the default value if "enforceOnKey" is not configured.
		// * IP: The source IP address of the request is the key. Each IP has this limit
		// enforced separately.
		...
		// specified in the list of IPs under X-Forwarded-For HTTP header. If no such
		// header is present or the value is not a valid IP, the key defaults to the
		// source IP address of the request i.e. key type IP.
		...
		// * SNI: Server name indication in the TLS session of the HTTPS request. The
		// key value is truncated to the first 128 bytes. The key type defaults to ALL
		// on a HTTP session.
		...
		// from it, the key type defaults to IP. Possible values: ["ALL", "IP",
		// "HTTP_HEADER", "XFF_IP", "HTTP_COOKIE", "HTTP_PATH", "SNI", "REGION_CODE",
		// "TLS_JA3_FINGERPRINT", "TLS_JA4_FINGERPRINT", "USER_IP"]
		...
		// threshold, to deny with a specified HTTP response code.
