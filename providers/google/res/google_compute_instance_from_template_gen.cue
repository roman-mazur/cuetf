...
google_compute_instance_from_template: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/google_compute_instance_from_template")
	close({
		...
		boot_disk?: matchN(1, [#boot_disk, list.MaxItems(1) & [...#boot_disk]])
		...
		// setting this field, the update will fail.
		...
		// This could be one of the following values: PROVISIONING, STAGING, RUNNING,
		// STOPPING, SUSPENDING, SUSPENDED, REPAIRING, and TERMINATED.
		// For more information about the status of the instance, see [Instance life
		...
		// A custom hostname for the instance. Must be a fully qualified DNS name and
		// RFC-1035-valid. Valid format is a series of labels 1-63 characters long
		// matching the regular expression [a-z]([-a-z0-9]*[a-z0-9]), concatenated with
		// periods. The entire hostname must not exceed 253 characters. Changing this
		...
		// The ID of the project in which the resource belongs. If self_link is
		// provided, this value is ignored. If neither self_link nor project are
	...
	#attached_disk: close({
		...
		// A 256-bit customer-supplied encryption key, encoded in RFC 4648 base64 to
		// encrypt this disk. Only one of kms_key_self_link, disk_encryption_key_rsa
		// and disk_encryption_key_raw may be set.
		disk_encryption_key_raw?: string

		// Specifies an RFC 4648 base64 encoded, RSA-wrapped 2048-bit customer-supplied
		// encryption key to either encrypt or decrypt this resource. Only one of
		// kms_key_self_link, disk_encryption_key_rsa and disk_encryption_key_raw may
		// be set.
		...
		// you will receive an error. Setting this parameter cause VM recreation.
		force_attach?: bool

		// The self_link of the encryption key that is stored in Google Cloud KMS to
		// encrypt this disk. Only one of kms_key_self_link, disk_encryption_key_rsa
		// and disk_encryption_key_raw may be set.
	...
	#boot_disk: close({
		...
		// A 256-bit customer-supplied encryption key, encoded in RFC 4648 base64 to
		// encrypt this disk. Only one of kms_key_self_link, disk_encryption_key_raw
		// and disk_encryption_key_rsa may be set.
		disk_encryption_key_raw?: string

		// Specifies an RFC 4648 base64 encoded, RSA-wrapped 2048-bit customer-supplied
		// encryption key to either encrypt or decrypt this resource. Only one of
		// kms_key_self_link, disk_encryption_key_raw and disk_encryption_key_rsa may
		// be set.
		disk_encryption_key_rsa?: string
		...
		// The disk interface used for attaching this disk. One of SCSI or NVME. (This
		// field is shared with attached_disk and only used for specific cases, please
	...
	#network_interface: close({
		...
		// VIRTIO_NET, IDPF, MRDMA, IRDMA and IDPF
		nic_type?: string
	...
	#scheduling: close({
		...
		// Specifies if the instance should be restarted if it was terminated by Compute
		// Engine (not a user).
	...
	_#defs: "/$defs/boot_disk/$defs/initialize_params": close({
		...
		// A list of self_links of resource policies to attach to the instance's boot
		// disk. Modifying this list will cause the instance to recreate. Currently a
		// max of 1 resource policy is supported.
		resource_policies?: [...string]
	...
	_#defs: "/$defs/boot_disk/$defs/initialize_params/$defs/source_image_encryption_key": close({
		// The self link of the encryption key that is stored in Google Cloud KMS. Only
		// one of kms_key_self_link, rsa_encrypted_key and raw_key may be set.
		kms_key_self_link?: string
	...
	_#defs: "/$defs/network_interface/$defs/alias_ip_range": close({
		...
		// to allocate the IP CIDR range for this alias IP range.
	...
	_#defs: "/$defs/network_interface/$defs/ipv6_access_config": close({
		...
		// The name of this access configuration. In ipv6AccessConfigs, the recommended
