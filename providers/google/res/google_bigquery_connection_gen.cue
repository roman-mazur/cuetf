...
google_bigquery_connection: {
	...
	close({
		...
		// with following exceptions: Cloud SQL us-central1 maps to BigQuery US, Cloud
		// SQL europe-west1 maps to BigQuery EU.
	...
	#azure: close({
		...
		// identity will be used to access the user's Azure Active Directory
		// Application.
		identity?: string
	...
	#cloud_spanner: close({
		...
		// Cloud Spanner database role for fine-grained access control. The Cloud
		// Spanner admin should have provisioned the database role with appropriate
		// permissions, such as 'SELECT' and 'INSERT'. Other users should only use
		// roles provided by their Cloud Spanner admins. The database role name must
		// start with a letter, and can only contain letters, numbers, and underscores.
		// For more details, see https://cloud.google.com/spanner/docs/fgac-about.
		...
		// 'useParallelism' and 'useDataBoost' must be set when setting max
		// parallelism.
		max_parallelism?: number

		// If set, the request will be executed via Spanner independent compute
		// resources. 'use_parallelism' must be set when using data boost.
		use_data_boost?: bool
	...
	#cloud_sql: close({
		...
		// When the connection is used in the context of an operation in BigQuery, this
		// service account will serve as the identity being used for connecting to the
		// CloudSQL instance specified in this connection.
		service_account_id?: string
	...
	#configuration: close({
		...
		// 'google-cloudsql-postgres', and other connector IDs supported by the BigQuery
		// Connector framework.
		connector_id!: string
	})
	...
	_#defs: "/$defs/spark/$defs/metastore_service_config": close({
		// Resource name of an existing Dataproc Metastore service in the form of
	...
	_#defs: "/$defs/spark/$defs/spark_history_server_config": close({
		...
		// Server for the connection if the form of
