...
google_alloydb_cluster: {
	...
	close({
		...
		// The type of cluster. If not set, defaults to PRIMARY. Default value:
		// "PRIMARY" Possible values: ["PRIMARY", "SECONDARY"]
		cluster_type?: string
		...
		// The database engine major version. This is an optional field and it's
		// populated at the Cluster creation time.
		// Note: Changing this field to a higer version results in upgrading the AlloyDB
		// cluster which is an irreversible change.
	...
	#automated_backup_policy: close({
		...
		// failed.
		//
		// The backup window must be at least 5 minutes long. There is no upper bound on
	...
	#continuous_backup_config: close({
		...
		// The numbers of days that are eligible to restore from using PITR. To support
	...
	#network_config: close({
		// The name of the allocated IP range for the private IP AlloyDB cluster. For
		// example: "google-managed-services-default".
		// If set, the instance IPs for this cluster will be created in the allocated range.
		allocated_ip_range?: string

		// The resource link for the VPC network in which cluster resources are created
		// and from which they are accessible via Private IP. The network must belong
		// to the same project as the cluster.
	...
	#psc_config: close({
		...
		// The project number that needs to be allowlisted on the network attachment to
		// enable outbound connectivity, if the network attachment is configured to
		// ACCEPT_MANUAL connections.
		// In case the network attachment is configured to ACCEPT_AUTOMATIC, this
		// project number does not need to be allowlisted explicitly.
	...
	#restore_backupdr_pitr_source: close({
		// The name of the BackupDR data source that this cluster is restore from. It
	...
	_#defs: "/$defs/automated_backup_policy/$defs/weekly_schedule": close({
		...
		// The days of the week to perform a backup. At least one day of the week must
		// be provided. Possible values: ["MONDAY", "TUESDAY", "WEDNESDAY", "THURSDAY",
