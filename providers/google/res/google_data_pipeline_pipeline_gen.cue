...
google_data_pipeline_pipeline: {
	...
	close({
		...
		// The display name of the pipeline. It can contain only letters ([A-Za-z]),
		...
		// google.cloud.location.Locations.ListLocations. Note that the Data Pipelines
		// service is not available in all regions. It depends on Cloud Scheduler, an
		// App Engine application, so it's only available in App Engine regions."
		// "PIPELINE_ID is the ID of the pipeline. Must be unique for the selected project and location."
		name!: string

		// The sources of the pipeline (for example, Dataplex). The keys and values are
		// set by the corresponding sources during pipeline creation.
		...
		// The state of the pipeline. When the pipeline is created, the state is set to
		// 'PIPELINE_STATE_ACTIVE' by default. State changes can be requested by
		// setting the state to stopping, paused, or resuming. State cannot be changed
		// through pipelines.patch requests.
		// https://cloud.google.com/dataflow/docs/reference/data-pipelines/rest/v1/projects.locations.pipelines#state
		// Possible values: ["STATE_UNSPECIFIED", "STATE_RESUMING", "STATE_ACTIVE",
		// "STATE_STOPPING", "STATE_ARCHIVED", "STATE_PAUSED"]
		state!: string

		// The type of the pipeline. This field affects the scheduling of the pipeline
		// and the type of metrics to show for the pipeline.
		// https://cloud.google.com/dataflow/docs/reference/data-pipelines/rest/v1/projects.locations.pipelines#pipelinetype
		// Possible values: ["PIPELINE_TYPE_UNSPECIFIED", "PIPELINE_TYPE_BATCH",
	...
	_#defs: "/$defs/workload/$defs/dataflow_flex_template_request/$defs/launch_parameter": close({
		...
		// The job name to use for the created job. For an update job request, the job
		// name should be the same as the existing running job.
		job_name!: string
	...
	_#defs: "/$defs/workload/$defs/dataflow_flex_template_request/$defs/launch_parameter/$defs/environment": close({
		...
		// follow the restrictions specified in the labeling restrictions page. An
		...
		// Possible values: ["FLEXRS_UNSPECIFIED", "FLEXRS_SPEED_OPTIMIZED",
		...
		// workerZone. If neither workerRegion nor workerZone is specified, default to
		...
		// workerRegion. If neither workerRegion nor workerZone is specified, a zone in
		// the control plane's region is chosen based on available capacity. If both
		// workerZone and zone are set, workerZone takes precedence.
		worker_zone?: string

		// The Compute Engine availability zone for launching worker instances to run
		// your pipeline. In the future, workerZone will take precedence.
		zone?: string
	})
