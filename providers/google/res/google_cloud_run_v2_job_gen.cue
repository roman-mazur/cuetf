...
google_cloud_run_v2_job: {
	...
	close({
		...
		// diagnostics information in case the Job does not reach its desired state.
		...
		// The generation of this Job. See comments in reconciling for additional
		// information on reconciliation process in Cloud Run.
		observed_generation?: string

		// Returns true if the Job is currently being acted upon by the system to bring
		...
		// process, observedGeneration and latest_succeeded_execution, will have
		// transient values that might mismatch the intended state: Once reconciliation
		// is over (and this field is false), there are two possible outcomes:
		// reconciliation succeeded and the state matches the Job, or there was an
		// error, and reconciliation failed. This state can be found in
		// terminalCondition.state.
		//
		// If reconciliation succeeded, the following fields will match:
		// observedGeneration and generation, latest_succeeded_execution and
		// latestCreatedExecution.
		//
		// If reconciliation failed, observedGeneration and latest_succeeded_execution
		// will have the state of the last succeeded execution or empty for newly
		// created Job. Additional information on the failure can be found in
		...
		// Server assigned unique identifier for the Execution. The value is a UUID4
	...
	#template: close({
		...
		// or break down billing charges by team, component, environment, state, etc.
		...
		// given time. Must be <= taskCount. When the job is run, if this field is 0 or
		// unset, the maximum possible value will be used for that execution. The
	...
	_#defs: "/$defs/template/$defs/template/$defs/node_selector": close({
		...
		// https://cloud.google.com/run/docs/configuring/jobs/gpu for configuring GPU.
		accelerator!: string
	})
	...
	_#defs: "/$defs/template/$defs/template/$defs/volumes/$defs/gcs": close({
		// Name of the cloud storage bucket to back the volume. The resource service
		// account must have permission to access the bucket.
		bucket!: string
