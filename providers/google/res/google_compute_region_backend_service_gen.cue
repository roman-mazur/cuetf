...
#google_compute_region_backend_service: {
	...
	#connection_tracking_policy: close({
		...
		// persist on
		// unhealthy backends only for connection-oriented protocols (TCP
		// and SCTP)
		// and only if the Tracking Mode is PER_CONNECTION (default
		// tracking mode)
		...
		// unhealthy
		// backend. They are always diverted to newly selected healthy
		// backends
		// (unless all backends are unhealthy).
		//
		// If set to 'ALWAYS_PERSIST', existing connections always persist
		// on
		// unhealthy backends regardless of protocol and session affinity.
		// It is
		// generally not recommended to use this mode overriding the
		// default. Default value: "DEFAULT_FOR_PROTOCOL" Possible
		// values: ["DEFAULT_FOR_PROTOCOL", "NEVER_PERSIST",
		// "ALWAYS_PERSIST"]
		...
		// hours.
		//
		// For NLB the minimum(default) is 60 seconds and the maximum is
		// 16 hours.
		idle_timeout_sec?: number

		// Specifies the key used for connection tracking. There are two
		// options:
		// 'PER_CONNECTION': The Connection Tracking is performed as per
		// the
		// Connection Key (default Hash Method) for the specific protocol.
		//
		// 'PER_SESSION': The Connection Tracking is performed as per the
		// configured Session Affinity. It matches the configured Session
		// Affinity. Default value: "PER_CONNECTION" Possible values:
