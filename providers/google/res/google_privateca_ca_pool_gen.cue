...
google_privateca_ca_pool: {
	...
	#issuance_policy: close({
		...
		// The maximum lifetime allowed for issued Certificates. Note that if the
		// issuing CertificateAuthority
		// expires before a Certificate's requested maximumLifetime, the effective
	...
	#publishing_options: close({
		...
		// When true, publishes each CertificateAuthority's CA certificate and includes
		// its URL in the "Authority Information Access"
		// X.509 extension in all issued Certificates. If this is false, the CA
		// certificate will not be published and the corresponding
		// X.509 extension will not be written in issued certificates.
		publish_ca_cert!: bool

		// When true, publishes each CertificateAuthority's CRL and includes its URL in
		// the "CRL Distribution Points" X.509 extension
		// in all issued Certificates. If this is false, CRLs will not be published and
		// the corresponding X.509 extension will not
		// be written in issued certificates. CRLs will expire 7 days from their
		// creation. However, we will rebuild daily. CRLs are
	...
	_#defs: "/$defs/issuance_policy/$defs/identity_constraints": close({
		...
		// If this is set, the SubjectAltNames extension may be copied from a
		// certificate request into the signed certificate.
		// Otherwise, the requested SubjectAltNames will be discarded.
		allow_subject_alt_names_passthrough!: bool

		// If this is set, the Subject field may be copied from a certificate request
	...
	_#defs: "/$defs/issuance_policy/$defs/identity_constraints/$defs/cel_expression": close({
		...
		// Title for the expression, i.e. a short string describing its purpose. This
