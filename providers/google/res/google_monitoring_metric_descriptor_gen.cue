...
google_monitoring_metric_descriptor: {
	...
	close({
		...
		// type and a MonitoredResourceDescriptor, that is associated with this metric
		// type can only be associated with one of the monitored resource types listed
		// here. This field allows time series to be associated with the intersection
		// of this metric type and the monitored resource types in this list.
		...
		// must have only upper and lower-case letters, digits, '/' and underscores '_'
		// are allowed. Additionally, the maximum number of characters allowed for the
		...
		// combinations of metricKind and valueType might not be supported. Possible
		// values: ["BOOL", "INT64", "DOUBLE", "STRING", "DISTRIBUTION"]
		value_type!: string
	})
	...
	#labels: close({
		...
		// The key for this label. The key must not exceed 100 characters. The first
		// character of the key must be an upper- or lower-case letter, the remaining
		// characters must be letters, digits or underscores, and the key must match
		// the regular expression [a-zA-Z][a-zA-Z0-9_]*
		key!: string

		// The type of data that can be assigned to the label. Default value: "STRING"
	...
	#metadata: close({
		...
		// smaller sampling period. In '[duration
		// format](https://developers.google.com/protocol-buffers/docs/reference/google.protobuf?&_ga=2.264881487.1507873253.1593446723-935052455.1591817775#google.protobuf.Duration)'.
