...
google_network_services_edge_cache_service: {
	...
	close({
		...
		// Disables HTTP/2.
		//
		// HTTP/2 (h2) is enabled by default and recommended for performance. HTTP/2
		// improves connection re-use and reduces connection setup overhead by sending
		// multiple streams over the same connection.
		//
		// Some legacy HTTP clients may have issues with HTTP/2 connections due to
		// broken HTTP/2 implementations. Setting this to true will prevent HTTP/2 from
		...
		// The IPv4 addresses associated with this service. Addresses are static for the
		// lifetime of the service.
		ipv4_addresses?: [...string]

		// The IPv6 addresses associated with this service. Addresses are static for the
		// lifetime of the service.
		...
		// [a-zA-Z][a-zA-Z0-9_-]* which means the first character must be a letter,
	...
	#log_config: close({
		...
		// value is 1.0, and the value of the field must be in [0, 1].
		//
	...
	_#defs: "/$defs/routing/$defs/host_rule": close({
		...
		//
		// Host patterns must be valid hostnames. Ports are not allowed. Wildcard hosts
		// are supported in the suffix or prefix form. * matches any string of
		// ([a-z0-9-.]*). It does not match the empty string.
		//
		...
		// Notes:
		//
		// The wildcard will not match the empty string. e.g. ''*-bar.foo.com'' will
		// match ''baz-bar.foo.com'' but not ''-bar.foo.com''. The longest wildcards
		// match first. Only a single host in the entire service can match on ''*''. A
		// domain must be unique across all configured hosts within a service.
		//
		// Hosts are matched against the HTTP Host header, or for HTTP/2 and HTTP/3, the
		// ":authority" header, from the incoming request.
		//
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule": close({
		...
		// The Origin resource that requests to this route should fetch from when a
		// matching response is not in cache. Origins can be defined as short names
		// ("my-origin") or fully-qualified resource URLs - e.g.
		...
		// 5, 9, 12, 16 is a valid series of priority numbers
		// to which you could add rules numbered from 6 to 8, 10 to 11, and 13 to 15 in
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/header_action/$defs/response_header_to_remove": close({
		...
		//
		// Response headers are only sent to the client, and do not have an effect on
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/match_rule/$defs/header_match": close({
		...
		// If set to false (default), the headerMatch is considered a match if the match
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/route_action": close({
		...
		// Setting the compression mode to automatic enables dynamic compression for
		// every eligible response.
		//
		// When dynamic compression is enabled, it is recommended to also set a cache
		// policy to maximize efficiency. Possible values: ["DISABLED", "AUTOMATIC"]
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/route_action/$defs/cdn_policy": close({
		...
		// Cache modes allow users to control the behaviour of the cache, what content
		// it should cache automatically, whether to respect origin headers, or whether
		// to unconditionally cache all responses.
		//
		// For all cache modes, Cache-Control headers will be passed to the client. Use
		// clientTtl to override what is sent to the client. Possible values:
		...
		// used by the edge caches. Leaving this empty will use the same cache TTL for
		// both the CDN and the client-facing response.
		//
		// - The TTL must be > 0 and <= 86400s (1 day)
		// - The clientTtl cannot be larger than the defaultTtl (if set)
		// - Fractions of a second are not allowed.
		//
		// Omit this field to use the defaultTtl, or the max-age set by the origin, as
		// the client-facing TTL.
		//
		...
		//
		// Note that infrequently accessed objects may be evicted from the cache before
		// the defined TTL. Objects that expire will be revalidated with the origin.
		//
		// When the cache mode is set to "USE_ORIGIN_HEADERS" or "BYPASS_CACHE", you must omit this field.
		//
		// A duration in seconds terminated by 's'. Example: "3s".
		...
		// - Fractions of a second are not allowed.
		//
		// When the cache mode is set to "USE_ORIGIN_HEADERS", "FORCE_CACHE_ALL", or
		...
		// Sets a cache TTL for the specified HTTP status code. negativeCaching must be
		// enabled to configure negativeCachingPolicy.
		//
		// - Omitting the policy and leaving negativeCaching enabled will use the
		// default TTLs for each status code, defined in negativeCaching.
		// - TTLs must be >= 0 (where 0 is "always revalidate") and <= 86400s (1 day)
		//
		// Note that when specifying an explicit negativeCachingPolicy, you should take
		// care to specify a cache TTL for all response codes that you wish to cache.
		// The CDNPolicy will not apply any default negative caching when a policy
		// exists.
		...
		//
		// When set, a signed request is rejected if its expiration time is later than
		// now + signedRequestMaximumExpirationTtl, where now is the time at which the
		// signed request is first handled by the CDN.
		//
		// - The TTL must be > 0.
		// - Fractions of a second are not allowed.
		//
		// By default, signedRequestMaximumExpirationTtl is not set and the expiration
		...
		// means all content is public, and does not authorize access.
		//
		...
		// enabled) logged. Possible values: ["DISABLED", "REQUIRE_SIGNATURES",
		// "REQUIRE_TOKENS"]
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/route_action/$defs/cdn_policy/$defs/add_signatures": close({
		// The actions to take to add signatures to responses. Possible values:
		...
		// * 'Headers'
		//
		// You may specify up to 6 parameters to copy. A given parameter is be copied
		// only if the parameter exists in the verified token. Parameter names are
		// matched exactly as specified. The order of the parameters does not matter.
		...
		// GENERATE_TOKEN_HLS_COOKIELESS actions are specified. This field may not be
		// specified otherwise.
		keyset?: string
		...
		//
		// If specified, the name must be 1-64 characters long and match the regular
		// expression '[a-zA-Z]([a-zA-Z0-9_-])*' which means the first character must
		// be a letter, and all following characters must be a dash, underscore, letter
		// or digit.
		//
		// This field may only be set when the GENERATE_TOKEN_HLS_COOKIELESS or
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/route_action/$defs/cdn_policy/$defs/cache_key_policy": close({
		...
		//
		// Note: this should only be enabled if hosts share the same origin and content.
		// Removing the host from the cache key may inadvertently result in different
		// objects being cached than intended, depending on which route the first user
		// matched.
		exclude_host?: bool
		...
		//
		// Either specify includedQueryParameters or excludedQueryParameters, not both.
		...
		// Names of Cookies to include in cache keys. The cookie name and cookie value
		// of each cookie named will be used as part of the cache key.
		//
		// Cookie names:
		...
		//
		// Note that specifying several cookies, and/or cookies that have a large range
		// of values (e.g., per-user) will dramatically impact the cache hit rate, and
		// may result in a higher eviction rate and reduced performance.
		//
		// You may specify up to three cookie names.
		included_cookie_names?: [...string]

		// Names of HTTP request headers to include in cache keys. The value of the
		// header field will be used as part of the cache key.
		//
		// - Header names must be valid HTTP RFC 7230 header field values.
		// - Header field names are case insensitive
		// - To include the HTTP method, use ":method"
		//
		// Note that specifying several headers, and/or headers that have a large range
		// of values (e.g. per-user) will dramatically impact the cache hit rate, and
		// may result in a higher eviction rate and reduced performance.
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/route_action/$defs/cdn_policy/$defs/signed_token_options": close({
		...
		//
		// You may specify up to 3 signature algorithms to use. Possible values:
		// ["ED25519", "HMAC_SHA_256", "HMAC_SHA1"]
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/route_action/$defs/cors_policy": close({
		...
		// Specifies how long results of a preflight request can be cached by a client
		// in seconds. Note that many browser clients enforce a maximum TTL of 600s (10
		// minutes).
		//
		// - Setting the value to -1 forces a pre-flight check for all requests (not recommended)
		// - A maximum TTL of 86400s can be set, but note that (as above) some clients
		// may force pre-flight checks at a more regular interval.
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/route_action/$defs/url_rewrite": close({
		...
		// Prior to forwarding the request to the selected origin, the matching portion
	...
	_#defs: "/$defs/routing/$defs/path_matcher/$defs/route_rule/$defs/url_redirect": close({
		...
		// method will be retained. Possible values: ["MOVED_PERMANENTLY_DEFAULT",
