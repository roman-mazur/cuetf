...
google_dialogflow_generator: {
	...
	close({
		...
		// is triggered in a conversation. Possible values: ["END_OF_UTTERANCE",
		// "MANUAL_CALL", "CUSTOMER_MESSAGE", "AGENT_MESSAGE"]
	...
	#inference_parameter: close({
		...
		// random. High temperature = more random. If unset (or 0), uses a default
		// value of 0.
		temperature?: number
		...
		// that the next token is selected from among the 3 most probable tokens (using
		// temperature). For each token selection step, the top K tokens with the
		// highest probabilities are sampled. Then tokens are further filtered based on
		// topP with the final token selected using temperature sampling. Specify a
		// lower value for less random responses and a higher value for more random
		...
		// their probabilities equals the top-p value. For example, if tokens A, B, and
		// C have a probability of 0.3, 0.2, and 0.1 and the top-p value is 0.5, then
		// the model will select either A or B as the next token (using temperature)
		// and doesn't consider C. The default top-p value is 0.95. Specify a lower
	...
	_#defs: "/$defs/summarization_context/$defs/few_shot_examples/$defs/conversation_context/$defs/message_entries": close({
		...
		// Optional. Participant role of the message. Possible values: ["HUMAN_AGENT",
		// "AUTOMATED_AGENT", "END_USER"]
		role?: string
	...
	_#defs: "/$defs/summarization_context/$defs/few_shot_examples/$defs/summarization_section_list/$defs/summarization_sections": close({
		...
		// Optional. Type of the summarization section. Possible values: ["SITUATION",
		// "ACTION", "RESOLUTION", "REASON_FOR_CANCELLATION", "CUSTOMER_SATISFACTION",
		// "ENTITIES", "CUSTOMER_DEFINED", "SITUATION_CONCISE", "ACTION_CONCISE"]
		type?: string
	})
