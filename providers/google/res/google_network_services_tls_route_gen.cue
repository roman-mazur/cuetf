...
google_network_services_tls_route: {
	...
	close({
		...
		// Gateways defines a list of gateways this TlsRoute is attached to, as one of
		...
		// Meshes defines a list of meshes this TlsRoute is attached to, as one of the
		...
		// TargetProxies defines a list of target proxies this TlsRoute is attached to,
		// as one of the routing rules to route the requests served by the load
		// balancer.
	...
	_#defs: "/$defs/rules/$defs/matches": close({
		...
		// "http/1.1", "h2". At least one of sniHost and alpn is required. Up to 5
		// alpns across all matches can be set.
		alpn?: [...string]

		// SNI (server name indicator) to match against. SNI will be matched against all
		...
		// invalid. At least one of sniHost and alpn is required. Up to 5 sni hosts
