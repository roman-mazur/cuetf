...
google_netapp_volume: {
	...
	close({
		...
		// Create time of the volume. A timestamp in RFC3339 UTC "Zulu" format.
		...
		// Total hot tier data rounded down to the nearest GiB used by the volume. This
		...
		// Reports the CMEK policy resurce name being used for volume encryption.
		...
		// Name of the pool location. Usually a region name, expect for some STANDARD
		// service level pools which require a zone name.
		location!: string
		...
		// volumes supporting multiple IP endpoints.
		...
		// 'projects/{{project}}/global/networks/{{network}}'. Inherited from storage
		// pool.
		network?: string

		// The protocol of the volume. Allowed combinations are '['NFSV3']',
		// '['NFSV4']', '['SMB']', '['NFSV3', 'NFSV4']', '['SMB', 'NFSV3']' and
		// '['SMB', 'NFSV4']'. Possible values: ["NFSV3", "NFSV4", "SMB", "ISCSI"]
		protocols!: [...string]
		project?: string
		...
		// Use NTFS to use NTFS ACLs for file permissions. Can only be set for volumes
		// which use SMB together with NFS as protocol. Possible values: ["NTFS",
		// "UNIX"]
		security_style?: string

		// Service level of the volume. Inherited from storage pool. Supported values
		// are : PREMIUM, EXTREME, STANDARD, FLEX.
		service_level?: string

		// Share name (SMB) or export path (NFS) of the volume. Needs to be unique per location.
		share_name?: string

		// Settings for volumes with SMB access. Possible values: ["ENCRYPT_DATA",
		// "BROWSABLE", "CHANGE_NOTIFY", "NON_BROWSABLE", "OPLOCKS", "SHOW_SNAPSHOT",
		// "SHOW_PREVIOUS_VERSIONS", "ACCESS_BASED_ENUMERATION",
		// "CONTINUOUSLY_AVAILABLE"]
		smb_settings?: [...string]

		// If enabled, a NFS volume will contain a read-only .snapshot directory which
		// provides access to each of the volume's snapshots. Will enable "Previous
		// Versions" support for SMB.
		...
		// Name of the storage pool to create the volume in. Pool needs enough spare
		...
		// Unix permission the mount point will be created with. Default is 0770.
		// Applicable for UNIX security style volumes only.
		unix_permissions?: string

		// Used capacity of the volume (in GiB). This is computed periodically and it
	...
	#backup_config: close({
		// Specify a single backup policy ID for scheduled backups. Format:
		...
		// When set to true, scheduled backup is enabled on the volume. Omit if no
		// backup_policy is specified.
	...
	#block_devices: close({
		...
		// This field can't be changed after the block device is created. Possible
		// values: ["LINUX", "WINDOWS", "ESXI"]
		os_type!: string
	...
	#cache_parameters: close({
		...
		// ONTAP. A timestamp in RFC3339 UTC "Zulu" format. Examples:
		// "2023-06-22T09:13:01.617Z".
	...
	#hybrid_replication_parameters: close({
		// Optional. Name of source cluster location associated with the replication.
		...
		// replication which got reversed. Default is 'MIGRATION'. Possible values:
		// ["MIGRATION", "CONTINUOUS_REPLICATION", "ONPREM_REPLICATION",
		...
		// Optional. If the source is a FlexGroup volume, this field needs to match the
		// number of constituents in the FlexGroup.
		...
		// Optional. Replication Schedule for the replication created. Possible values:
	...
	#tiering_policy: close({
		...
		// eligible for tiering, can be range from 2-183.
		// Default is 31.
		...
		// Default is PAUSED. Default value: "PAUSED" Possible values: ["ENABLED",
		// "PAUSED"]
		tier_action?: string
	})
	...
	_#defs: "/$defs/export_policy/$defs/rules": close({
		...
		// specification. Possible values: ["READ_ONLY", "READ_WRITE", "READ_NONE"]
		access_type?: string

		// Defines the client ingress specification (allowed clients) as a comma
		// separated list with IPv4 CIDRs or IPv4 host addresses.
		allowed_clients?: string

		// An integer representing the anonymous user ID. Range is 0 to 4294967295.
		// Required when 'squash_mode' is 'ALL_SQUASH'.
		anon_uid?: number

		// If enabled, the root user (UID = 0) of the specified clients doesn't get
		// mapped to nobody (UID = 65534). This is also known as no_root_squash.
		// Use either squash_mode or has_root_access, but never both at the same time.
		...
		// the 'allowedClients' specification. It enables nfs clients to mount using
		// 'authentication' kerberos security mode. The 'kerberos5ReadOnly' value is
		...
		// If enabled (true) the rule defines read and write access for clients matching
		// the 'allowedClients' specification. It enables nfs clients to mount using
		// 'integrity' kerberos security mode. The 'kerberos5iReadOnly' value is
		// ignored if this is enabled.
		...
		// If enabled (true) the rule defines read and write access for clients matching
		// the 'allowedClients' specification. It enables nfs clients to mount using
		// 'privacy' kerberos security mode. The 'kerberos5pReadOnly' value is ignored
		// if this is enabled.
		...
		// an NFS export. It controls how the user identities (like root) are mapped to
		// anonymous users to limit access and enforce security.
		// Use either squash_mode or has_root_access, but never both at the same time.
		// These parameters are mutually exclusive. Possible values:
