...
helm_release: {
	...
	@jsonschema(id="https://github.com/roman-mazur/cuetf/schema/res/helm_release")
	close({
		...
		// The current revision of the write-only "set_wo" attribute. Incrementing this
		// integer value will cause Terraform to update the write-only value.
		...
		// If set, Helm will take ownership of resources not already annotated by this
		// release. Useful for migrations or recovery.
		take_ownership?: bool
		...
		// If true, the provider will install the release at the specified version even
		// if a release not controlled by the provider is present. This is equivalent
		// to running 'helm upgrade --install'. WARNING: this may not be suitable for
		// production use -- see the 'Upgrade Mode' note in the provider documentation.
		// Defaults to `false`.
		...
		// If wait is enabled, will wait until all Jobs have been completed before
